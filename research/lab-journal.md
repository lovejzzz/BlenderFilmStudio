...
## J-475 · F0.5 无鼠标渲染、受控中断与 receipt-only recovery PASS

Date: 2026-08-30 · Type: F0.5 HEADLESS RENDER AND RECEIPTS PASS · Host: macOS 25.6.0 / Apple M2 Max / 64 GiB · Formal product starts: 4 / 4 · Observed render calls: 2 / 2

attempt-01先以commit `528de05163aee900733e7767d3b71d3a7f474d78`预注册固定B01 `.blend`、EEVEE 640×360/16 spp PNG、Cycles CPU 640×360/32 spp half-ZIP multilayer EXR、160 GiB准入、120秒/8 GiB进程上限和三项负控；工具与formal-start分别推送至`d4352a905effa2df6e20002d49dc601be8b20be5`与`09f9806e2883da2448edc2e8484250ec5ec8b6ec`。insufficient-disk按实际free减80 GiB projection后低于100 GiB reserve，0 product start拒绝。第一个preview fresh admission后，真实产品在0.64秒/242,958,336-byte RSS退出：source Scene仍为`MULTI_LAYER_IMAGE`，harness未先切`IMAGE`便设置PNG，Blender enum只允许`OPEN_EXR_MULTILAYER`。失败发生在`bpy.ops.render`前，0 render、0正式图像；后续阶段全部按stop rule跳过。attempt-01 verdict保持FAIL，self hash为`e36afca30fc3567368ae72466c682a32ab54723300b2aa063667382b5b06c617`。

attempt-02在任何工具修正前以commit `3f70dc035de576b014744c612b6c520efc38bc84`建立fresh root并交叉绑定失败，只授权preview显式`media_type=IMAGE`、final显式`MULTI_LAYER_IMAGE`，以及用调用前marker区分expected/observed render calls；profile、threshold、顺序、source、product和OCIO均未改变。工具freeze为`b9b2c9cb0e8f634682807da374c4b1e5233a2260`，formal-start为`ecb4a688d9300d4176cbeba2c16d57bf28ef4451`。四次产品启动最低free 173,672,693,760 bytes，最大并发1：preview 4.58秒、controlled interruption 0.63秒、final recovery 1.88秒、independent audit 0.96秒；全链peak RSS 447,709,184 bytes，expected/observed render calls exact为2/2。

EEVEE生成231,205-byte PNG `9c2b1f98…`，decoded 640×360 RGBA、0 non-finite、dynamic range 1.0、143,887 nonzero RGB pixels。第二个renderer在`READY_FOR_CONTROLLED_INTERRUPT`后、`RENDER_CALL_BEGIN`前收到SIGTERM，observed render calls 0、无EXR或success receipt。篡改preview receipt的内存副本因self-hash mismatch以0 additional start拒绝；恢复先验证immutable preview与interruption receipt，preview rerender count保持0，只运行FINAL。Cycles生成1,533,576-byte EXR `849955e9…`；独立OIIO 3.1.13.1 / NumPy 2.3.4进程解码Combined、Depth、Normal及额外Vector/CryptoObject passes，Combined为640×360、0 non-finite、dynamic range 34.59375、230,400 nonzero RGB pixels。
...
## J-477 · F0.7 unsigned macOS package、隔离安装与双向 `.blend` 往返 PASS

Date: 2026-08-30 · Type: F0.7 PACKAGE / INSTALL / ROUND-TRIP PASS · Host: macOS 25.6.0 / Apple M2 Max / 64 GiB · Accepted formal product starts: 6 / 6 · Render/model/network calls: 0 / 0 / 0

F0.7在首次package/install mutation前冻结merged product、unsigned DMG、read-only mount、`/Users/mengyingli/Applications/Film Studio Engine F0.app` exact install target、官方与独立config/cache snapshots、installed official Blender negative control、两条`.blend` round-trip、missing optional metadata degradation与无secret的signing/notarization path。attempt-01因`hdiutil create`超过60秒上限保持`P02_DMG_CREATE_TIMEOUT_60S` FAIL；attempt-02完成可验证DMG后在首条往返的pre-save snapshot暴露depsgraph stale，并记录`OFFICIAL_CONFIG_EMPTY_DIRECTORY_DRIFT` secondary failure。attempts 03/04均完成六次round-trip product starts，却分别因`E08_INSTALL_RECEIPT_VARIABLE_NAME`与`E08_NORMALIZED_INSTALL_RECEIPT_VARIABLE_NAME`在最终receipt assembly fail-closed。四个失败root均未same-ID repair，verdict self hashes依次为`92b3fd06…`、`44189d51…`、`f84ef99b…`、`66953cf6…`。

attempt-05 preregistration/tool-freeze/formal-start commits为`0ce07512efbaf3ad1990a7d1d276ba6d6688df08`、`4b239d900e934dbf3d2bef3519addad79879e083`、`9440dae9fee6a27c510c8aa1b247a445b5800bc5`；工具冻结新增mandatory ESLint `no-undef`，直接覆盖前两次receipt variable-name失败模式。为保持160 GiB formal admission，只移除外部build roots中与最终app分离、可重新生成的intermediates，并留下三份host-space recovery receipts；源码、产品bundle、DMG和所有失败/正式证据均保留。

accepted package交叉绑定attempt-02已完成的341,069,106-byte UDZO DMG，SHA-256为`20a8aefd177fe41190b95ea7bbe7e75fb828ecd7ee5d9b7ffcd2c336a1f34a42`。`hdiutil verify`通过，read-only mounted payload与normalized staging payload均为5,487 files、digest `c0deb1c7…`。codesign evidence分类为`ADHOC_LINKER_SIGNED`；`spctl`按预期拒绝并保留日志，没有Developer ID、notarization submission、Gatekeeper bypass或public distribution claim。

两条往返各三次启动：official 5.2.0→F0 5.2.1→official 5.2.0，以及F0 5.2.1→official 5.2.0→F0 5.2.1。六个边界的core semantic SHA-256全部exact为`139852cf091bc79b78f7dd73430912d7988b0f54a3089b010dd87a26211a2300`；官方创建文件缺少Film Studio metadata时graceful open，F0创建的typed metadata SHA-256 `93daa8d5…`经官方重存后由F0 exact恢复，disposition为`PRESERVED_EXACT`。全过程0 render、0 mouse、0 network、0 model call。
...
## J-531 · PB.7 response recorder and independent auditor frozen

Date: 2026-08-31 · Type: TOOL FREEZE · Formal human responses / engine edits / builds / Blender starts / renders: 0 / 0 / 0 / 0 / 0
...
## J-578 · PC6 causal contract productization PASS; filmic realism still open

Date: 2026-09-01 · Type: ACCEPTED PRODUCT CAPABILITY + DIRECT VISUAL LIMIT · Native builds / product starts / scene mutations / review renders: 1 / 3 / 1 / 3

Product commit `5f3b981a…` adds exactly three paths / 702 additions: a strict CausalSceneSpec module, Film Studio workspace operators and top-bar Physics controls. The product accepts only five allowlisted factories, gives the actor initial-condition authority only before release, forbids target/final-pose authority, and derives shot framing from evaluated semantic world bounds. Attempts 01–03 are retained pre-build dependency/gitlink failures; attempt-04 completed a native build but retained the missing accepted post-build app rename. Fresh attempt-05 completed the clean arm64 build, rejected nine authority attacks, performed one legal Bullet scene build, rendered three review stills, saved and reopened exactly. Receipt/C4 self hashes are `dc0ecc5f…` / `5161f7d0…`; binary SHA-256 is `b43b0dce…`.
...
## J-582 · PC7 evaluated-motion filmic physics PASS

Date: 2026-09-01 · Type: ACCEPTED PRIMARY PHYSICS + DIRECT STILL/CLIP REVIEW · Native builds / product starts / stills / clip frames / saves: 1 / 3 / 3 / 24 / 1
