  ...
  return (
    <main className="cost-page">
      ...
      <section className="section cost-measured">
        ...
        <div className="b49-cost-update b51-cost-update"><header><span>NEW · B51-D1 + D2 NATIVE BACKEND</span><b>热态 Metal 改变吞吐数量级；用户 Cycles cache 不是 108 秒反例的充分原因</b></header><div><article><strong>≈37×</strong><span>qemu → native CPU</span></article><article><strong>≈7.17×</strong><span>CPU → warm Metal</span></article><article><strong>0.57–0.72 s</strong><span>warm Metal / frame</span></article><article><strong>+0.21 s</strong><span>exact cache-absent penalty</span></article></div><p>四项均为 512×288 简单场景实测，不是 2K/4K、长序列、电力或美元价格。D2 证明移走精确 `~/.cache/cycles` 只增加约 0.21 秒，不能把 D1 的 108.31 秒直接记为普通 cache warm-up；生产合同需要独立 pre-job canary。</p><Link href="/native-backend-v0-1">查看 D1 三后端与 D2 可逆因果干预 →</Link></div>
      </section>
