  ...
  return (
    <main className="cost-page">
      ...
      <section className="section cost-measured">
        ...
        <div className="b49-cost-update b51-cost-update"><header><span>NEW · B51-D1 NATIVE BACKEND</span><b>热态 Metal 改变吞吐数量级，冷同步仍改变首帧成本</b></header><div><article><strong>≈37×</strong><span>qemu → native CPU</span></article><article><strong>≈7.17×</strong><span>CPU → warm Metal</span></article><article><strong>0.57–0.72 s</strong><span>warm Metal / frame</span></article><article><strong>108.31 s</strong><span>first-cell synchronization</span></article></div><p>四项均为两个 512×288 简单场景的实测，不是 2K/4K、长序列、电力或美元价格。当前成本模型必须把 cache warm-up 作为一次性作业成本单列，不能把冷值和热值混成一个“平均 GPU 帧价”。</p><Link href="/native-backend-v0-1">查看八次真实 Blender 进程与像素边界 →</Link></div>
      </section>
