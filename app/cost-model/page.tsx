  ...
  return (
    <main className="cost-page">
      ...
      <section className="section cost-measured">
        ...
        <div className="b49-cost-update b51-cost-update"><header><span>NEW · B52-D2 ADAPTIVE PRODUCTION HOLDOUT</span><b>最多省 44.79%；完整 production contract 仍无可选点</b></header><div><article><strong>54 / 54</strong><span>fresh CPU renders</span></article><article><strong>5 / 8</strong><span>profiles ≥20%</span></article><article><strong>48 / 48</strong><span>Depth semantic pass</span></article><article><strong>0 / 8</strong><span>eligible profiles</span></article></div><p>显式 0.01/min0/max128 baseline 有效，成本曲线也被确认；但 INTERIOR beauty 从 0.015 起失败，Cryptomatte 与 exact Normal/Vector 又阻断全部 48 个 candidate cells。结论是 valid NOT_SUPPORTED，不是“adaptive 无效”。下一成本研究必须先建立 task-specific payload semantics，并以新 seed/未见帧测试 scene-conditioned threshold。</p><Link href="/adaptive-cpu-v0-1#d2">查看 8-profile 曲线、quality/payload 阻断与 22/22 attacks →</Link></div>
        <div className="b49-cost-update b51-cost-update"><header><span>NEW · B52-D3 ZERO-RERENDER TASK DERIVATION</span><b>新增渲染成本为零；仍没有可购买的质量—成本点</b></header><div><article><strong>0</strong><span>new renders</span></article><article><strong>820 KB</strong><span>formal output</span></article><article><strong>2 / 16</strong><span>Crypto classifier</span></article><article><strong>0 / 16</strong><span>Normal + Vector</span></article></div><p>D3 证明“先复用 EXR、再缩小下一次渲染矩阵”可以显著降低研究成本：54 个既有 EXR 支撑了 16 对任务级测量与 18 张诊断图。但结果没有产生 future candidate，不能据此购买更松的 adaptive profile。Normal 有实质 relighting 反例；Vector 则是幅值过门、nonzero-count 失败，下一步只需做冻结的 magnitude/downstream 派生，不应先花钱重渲染。</p><Link href="/adaptive-cpu-v0-1#d3">查看零重渲染证据、固定标尺诊断图与 D4 边界 →</Link></div>
        <div className="b49-cost-update b51-cost-update"><header><span>NEW · B52-D4 REAL COMPOSITOR TASK</span><b>约 73 秒机器时间换来一个重要 stop：静态 oracle 不值得继续烧渲染</b></header><div><article><strong>36 / 36</strong><span>Blender processes</span></article><article><strong>72.76 s</strong><span>cumulative wall time</span></article><article><strong>51.6 MB</strong><span>formal EXR outputs</span></article><article><strong>INVALID</strong><span>baseline effect</span></article></div><p>D4 没有启动新的 Cycles 光线渲染，只用 CPU compositor 复用 D2 的 54 个 EXR。候选 blur 输出虽与 baseline decoded-identical，但两个 baseline 场景都没有一个像素越过 `1/65536` task-effect 门；继续扩大候选矩阵只会花钱测量一个不敏感的 oracle。下一笔预算应先用于极小的 controlled-motion calibration，过门后才允许 fresh adaptive holdout。</p><Link href="/adaptive-cpu-v0-1#d4">查看 36-process 证据、保留的软件失败与 D5 stop/go 门 →</Link></div>
      </section>
