  ...
  return (
    <main className="cost-page">
      ...
      <section className="section cost-hardware" id="hardware">
        ...
        <div className="cost-section-heading dark-heading"><div><p className="eyebrow dark"><span /> BUY A NEW EVIDENCE DOMAIN · NOT A FASTER DUPLICATE</p><h2>当前机器够研究。<br />下一台必须<span>增加对照维度。</span></h2></div><p>本机实测为 M4 Max、16 CPU cores、40 GPU cores、48 GB unified memory；当前内盘仅约 100 GiB 可用。它足以继续编译器与 Metal 实验。B51-D5 已发现 exact CPU data 的 sample floor 仍为 128 spp，因此 H2 前先做 D6 语义门；最先逼近的资源仍是证据存储，不是计算。</p></div>
        ...
        <div className="hardware-decision"><b>CURRENT DECISION</b><p><strong>存储：现在有理由购买。</strong> RTX 5090 节点：有明确科研价值，但不是继续 D6/H2 的前置条件；如果预算允许，它是下一项最有效的硬件投入。当前 exact split path 还要支付完整 128-spp CPU data + Metal，并未表现成本优势。RTX PRO 6000：没有实测触发证据，暂不购买。</p></div>
      </section>
