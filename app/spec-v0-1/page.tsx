...
export const metadata: Metadata = {
  ...
  description: '首个可执行的 AI → Blender 镜头合同：JSON Schema、语义规则、安全边界、20 个测试样例与研究母版。',
  ...
  twitter: { card: 'summary_large_image', title: 'SceneSpec v0.1', description: '10 个合同块、20 个测试样例、三层验证与零任意代码。' },
};
...
const rootBlocks = [
  ...
  ['04', 'cameras', '焦段、传感器、光圈、焦距、快门角和显式变换', '摄影参数可重现'],
...
const validationLayers = [
  ...
  ['L4', 'Blender 编译前检查', '下一阶段：解析资产清单、API 合约、依赖闭包、资源预算与 dry-run 构建计划。', '尚未实现'],
];

const validFixtures = ['V01 基准镜头', 'V02 标题变体', 'V03 焦段变体', 'V04 新增道具', 'V05 新增补光', 'V06 合法角色', 'V07 合法接触', 'V08 偏移帧区间', 'V09 世界变体', 'V10 关闭降噪'];
const invalidFixtures = ['I01 未知根字段', 'I02 错误版本', 'I03 倒置帧区间', 'I04 重复 ID', 'I05 不存在的摄影机', 'I06 路径穿越', 'I07 开启网络', 'I08 缺失许可', 'I09 越界事件', 'I10 角色引用道具'];
...
const unresolved = [
  ...
  ['U03', '动画数据模型', 'v0.1 只定义事件，不定义完整 F-Curve、动作片段、retarget 与 performance layer。'],
  ['U04', 'USD 映射', 'SceneSpec 是意图合同，USD 是场景组合层；哪些字段进入 USD schema、哪些留在 manifest 尚待验证。'],
  ['U05', 'Blender API 合约', '需要把每个允许操作映射到可测试的 bpy 数据 API，并避免依赖 UI 上下文的 operators。'],
...
const sample = `{
  ...
  "render": {
    ...
    "previewEngine": "BLENDER_EEVEE_NEXT",
    "finalEngine": "CYCLES",
  ...
  return (
    <main className="spec-page">
      <header className="topbar">
        ...
        <nav aria-label="规格导航"><Link href="/">技术基线</Link><Link href="/blender-5-2">Blender 5.2</Link><Link href="/cost-model">成本</Link><Link href="/research-agenda">研究路线</Link><a href="#contract">合同</a><a href="#tests">测试</a></nav>
      ...
      <section className="spec-hero" id="top">
        ...
        <div className="spec-terminal" aria-label="规格验证摘要"><header><i /><i /><i /><span>validate:spec</span></header><code>PASS V01_BASE_B02</code><code>PASS V06_VALID_ACTOR</code><code>PASS I06_PATH_TRAVERSAL</code><code>PASS I10_ACTOR_USES_PROP</code><b>20 / 20 FIXTURES PASSED</b></div>
        <div className="spec-stats"><article><strong>10</strong><span>根合同块</span><small>shot → provenance</small></article><article><strong>20</strong><span>正反样例</span><small>10 接受 / 10 拒绝</small></article><article><strong>3</strong><span>已执行验证层</span><small>Schema · 语义 · 安全</small></article><article><strong>0</strong><span>任意代码权限</span><small>禁网 · 禁任意 Python</small></article></div>
      </section>

      <section className="section spec-verdict">
        <div className="section-index">00 / 当前状态</div>
        <div className="spec-verdict-grid"><div><p className="eyebrow dark"><span /> FIRST EXECUTABLE ARTIFACT</p><h2>研究从“可行”，<br />进入<span>可拒绝。</span></h2></div><div><b>已经证明</b><p>合同能够机械区分 20 个预设的合法与非法镜头变体，并给出结构或语义错误。</p><b>尚未证明</b><p>合同可以完整表达电影表演、能无损编译进 Blender，或能跨 GPU 得到已校准的像素一致性。</p></div></div>
      ...
      <section className="section spec-contract" id="contract">
        ...
        <div className="spec-flow"><article><span>01</span><b>Director Intent</b><small>自然语言 · 分镜 · 参考</small></article><i>→</i><article className="agent"><span>02</span><b>Codex Proposal</b><small>只提出 SceneSpec</small></article><i>→</i><article className="gate"><span>03</span><b>Validation Gates</b><small>Schema · 语义 · 安全</small></article><i>→</i><article><span>04</span><b>BuildPlan</b><small>不可变 · 可 diff</small></article><i>→</i><article><span>05</span><b>Blender Compiler</b><small>bpy data API · 下一阶段</small></article></div>
      ...
      <section className="section spec-validation">
        ...
        <div className="validation-layers">{validationLayers.map(([id, title, content, result], index) => <article className={index === 3 ? 'pending' : ''} key={id}><span>{id}</span><small>{index === 3 ? 'NEXT' : 'ACTIVE'}</small><h3>{title}</h3><p>{content}</p><b>{result}</b></article>)}</div>
      ...
      <section className="section spec-tests" id="tests">
        ...
        <div className="spec-heading dark-heading"><div><p className="eyebrow dark"><span /> 20 CONTRACT TESTS</p><h2>正例证明表达力，<br />反例证明边界。</h2></div><p>每个测试从相同 B02 基准文档派生，只改变目标字段。验证器必须同时匹配预期 valid/invalid 状态和关键错误代码。</p></div>
        <div className="fixture-board"><div className="valid"><header><span>ACCEPT</span><b>10 / 10</b></header>{validFixtures.map(item => <p key={item}><i>✓</i>{item}</p>)}</div><div className="invalid"><header><span>REJECT</span><b>10 / 10</b></header>{invalidFixtures.map(item => <p key={item}><i>×</i>{item}</p>)}</div></div>
      ...
      <section className="section spec-open">
        ...
        <div className="next-compiler"><span>NEXT MILESTONE</span><div><h3>SceneSpec → immutable BuildPlan → Blender 5.2</h3><p>下一步只实现 B01/B02 需要的最小编译器：清空受管集合、导入已锁定资产、建立摄影机和灯光、设置渲染、输出 scene manifest。两次净构建必须得到相同结构哈希。</p></div></div>
      </section>
      ...
      <footer><div><span className="brand-mark">BFS</span><b>SceneSpec v0.1</b></div><p>Executable artifact · Experimental · 20/20 fixtures passing</p><Link href="/research-agenda">返回研究路线 →</Link></footer>
    </main>
  );
}
