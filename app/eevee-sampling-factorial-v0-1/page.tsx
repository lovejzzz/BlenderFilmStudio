...
export default function EeveeSamplingFactorialPage() {
  return <main className="contact-page factorial-page">
    ...
    <section className="section contact-limits" id="next"><div className="section-index">03 / 下一可证伪边界</div><div className="contact-heading dark-heading"><div><p className="eyebrow dark"><span /> DOSE RESPONSE · QUALITY PRESERVATION</p><h2>找到参与因素之后，<br /><span>寻找边界与可用解。</span></h2></div><p>B18 应固定 dither，测试 samples 1/2/4/8/16/32 的 A/B 剂量响应。如果所有大于 1 的值都非 exact，就继续隔离执行调度；如果出现阈值，就缩小边界。画质必须作为独立响应保存。</p></div><div className="contact-artifacts"><a href={`${repo}experiments/eevee-sampling-factorial-v0-1/results.json`}><span>RESULT</span><b>4 cells · 12 attacks ↗</b></a><a href={`${repo}experiments/eevee-sampling-factorial-v0-1/evidence/S01-D0.sequence.comparison.json`}><span>EXACT CELL</span><b>144 OIIO pairs ↗</b></a><a href={`${repo}research/2026-08-26-b17-eevee-sampling-factorial-result.md`}><span>RESULT NOTE</span><b>causal boundary ↗</b></a><a href={`${repo}research/2026-08-26-b17-eevee-sampling-factorial-protocol.md`}><span>PROTOCOL</span><b>pre-registered matrix ↗</b></a></div></section>
