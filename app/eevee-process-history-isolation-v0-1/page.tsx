...
export default function ProcessHistoryIsolationPage() {
  return <main className="contact-page factorial-page">
    ...
    <section className="section contact-limits" id="next"><div className="section-index">04 / 下一可证伪边界</div><div className="contact-heading dark-heading"><div><p className="eyebrow dark"><span /> SCENE-LINEAR FLOAT · DISPLAY / PNG8</p><h2>变化发生在采样，<br /><span>还是发生在输出？</span></h2></div><p>下一实验应从同一次 render 同时固化内存 float Render Result 与 PNG8。若 float exact 而 PNG 不 exact，边界在显示/量化链；若 float 已不 exact，继续调查采样归约与 GPU 求值。</p></div><div className="contact-artifacts"><a href={`${repo}experiments/eevee-process-history-isolation-v0-1/results.json`}><span>RESULT</span><b>39 PIDs · 18 attacks ↗</b></a><a href={`${repo}experiments/eevee-process-history-isolation-v0-1/evidence/process-ledger.json`}><span>PROCESS LEDGER</span><b>39 unique PIDs ↗</b></a><a href={`${repo}research/2026-08-26-b20-eevee-process-history-isolation-result.md`}><span>RESULT NOTE</span><b>frame-level audit ↗</b></a><a href={`${repo}research/2026-08-26-b20-eevee-process-history-isolation-protocol.md`}><span>PREREGISTRATION</span><b>frozen before tools ↗</b></a></div></section>
