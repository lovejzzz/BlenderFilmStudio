...
export default function ReviewForm({ videoSrc }: { videoSrc: string }) {
  ...
  function download(event: React.FormEvent) {
    ...
    if (!ready) return;
    const payload = {
      documentType: 'BFS_HUMAN_REVIEW_RESPONSE', protocolVersion: '0.1.0', clipId: 'CLIP_A17F',
    ...
    anchor.href = url;
    anchor.download = `CLIP_A17F-${form.reviewerCode}.review.json`;
    anchor.click();
  ...
  return <>
    <section className="review-clip"><div><span>ANONYMOUS REVIEW ASSET</span><h1>CLIP_A17F</h1><p>请先以正常速度完整观看至少两次。不要逐帧暂停，也不要在提交前打开任何相关指标或实验报告。</p></div><video controls preload="metadata" playsInline aria-label="匿名交互审查视频"><source src={videoSrc} type="video/mp4" />浏览器无法播放此视频。</video></section>
    <form className="review-form" onSubmit={download}>
      <div className="review-intro"><span>PROTOCOL 0.1</span><h2>只判断你看到的交互。</h2><p>角色是技术代理。请判断动作关系，不评价造型是否好看。1 表示明显失败，5 表示对这个技术代理而言清楚可信。</p></div>
      ...
      {saved && <div className="review-saved"><b>RESPONSE SAVED LOCALLY</b><p>文件已下载。现在可以查看实验指标，不会再影响本次评分。</p><Link href="/contact-v0-1">查看 B04 机器证据 →</Link></div>}
    </form>
  </>;
}
