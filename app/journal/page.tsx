...
export default function JournalPage() {
  return <main className="contact-page">
    ...
    <section className="section contact-limits"><div className="section-index">02 / 最新结果</div><div className="contact-heading dark-heading"><div><p className="eyebrow dark"><span /> B44 → B46 → B47 · FROM FRAME TO PRODUCTION PASSES</p><h2>不只复现画面。<br /><span>后期需要的每一层也能复现。</span></h2></div><p>B47 用四个隔离 worker 从四份字节不同的 `.blend` 生成八个 32-bit multipart EXR。Combined、Depth、Normal、Vector 与三层 CryptoObject 共 28/28 跨构建通道对 exact；独立 audit 重开全部 EXR，并通过 18/18 预注册攻击。八个容器字节均不同，因此结论严格限定在解码后的通道数组与必要 metadata。</p></div><div className="contact-artifacts"><a href="https://github.com/lovejzzz/BlenderFilmStudio/blob/main/research/RESEARCH_CHARTER.md"><span>CHARTER</span><b>三道门与实验纪律 ↗</b></a><a href="https://github.com/lovejzzz/BlenderFilmStudio/blob/main/research/lab-journal.md"><span>FULL JOURNAL</span><b>失败与修正记录 ↗</b></a><Link href="/codex-worker-production-passes-v0-1"><span>B47 PRODUCTION PASSES</span><b>28/28 decoded pass pairs →</b></Link><Link href="/codex-worker-sequence-v0-1"><span>B46 SEQUENCE</span><b>ordered-frame predecessor →</b></Link></div></section>
