...
export default function TrajectoryV01Page() {
  return <main className="contact-page">
    ...
    <section className="section contact-limits" id="boundary"><div className="section-index">04 / 下一步</div><div className="contact-heading dark-heading"><div><p className="eyebrow dark"><span /> DO NOT LAUNDER THE SOURCE</p><h2>可重复播放，<br /><span>不是批准这条物理轨迹。</span></h2></div><p>正式 B07 仍需要 BuildPlan 集成，并要求源 solve 通过镜头证据与真实人类评审。当前状态值会跟随数据进入每一个 replay。</p></div><div className="contact-artifacts"><a href={`${repo}experiments/trajectory-v0-1/results.json`}><span>RESULT</span><b>双构建与反例 ↗</b></a><a href={`${repo}specs/benchmarks/B07.trajectory.json`}><span>TRAJECTORY</span><b>132 帧不可变数据 ↗</b></a><a href={`${repo}research/2026-08-26-b07-trajectory-result.md`}><span>RESULT NOTE</span><b>结论与非声明 ↗</b></a><a href={`${repo}specs/trajectory-spec.v0.1.schema.json`}><span>SCHEMA</span><b>TrajectorySpec v0.1 ↗</b></a></div></section>

    <footer><div><span className="brand-mark">BFS</span><b>B07 Immutable Trajectory</b></div><p>Exact replay pass · source physics unapproved · formal integration pending</p><Link href="/physics-v0-1">返回 B06 →</Link></footer>
  </main>;
}
