...
export default function WorkerLaunchContractPage() {
  return <main className="contact-page security-page">
    ...
    <section className="section contact-limits" id="boundary"><div className="section-index">05 / CONTRACT ≠ CONTAINMENT</div><div className="contact-heading dark-heading"><div><p className="eyebrow dark"><span/> NEXT · B39 DISPOSABLE LINUX WORKER</p><h2>下一次才运行 backend。<br/><span>而且必须先恢复磁盘安全线。</span></h2></div><p>B39 需要 digest-pinned Blender image、只读输入、单一输出、tmpfs work、禁网、non-root、cap drop、cgroup breach、真实 Blender canary 与 timeout recovery。已有 Colima/Alpine 只说明 testbed 候选存在，不说明 Blender 兼容或 containment 成立。</p></div><div className="contact-artifacts"><a href={`${repo}specs/worker-launch-contract.v0.1.json`}><span>FROZEN SPEC</span><b>11 env · 25 attacks · non-claims ↗</b></a><a href={`${repo}experiments/worker-launch-contract-v0-1/results.json`}><span>RESULT</span><b>3/3 canonical · host blocked ↗</b></a><a href={`${repo}experiments/worker-launch-contract-v0-1/audit.json`}><span>INDEPENDENT AUDIT</span><b>25/25 exact replay · PASS ↗</b></a><a href={`${repo}research/2026-08-26-b38-worker-launch-contract-result.md`}><span>RESULT NOTE</span><b>measured facts · B39 boundary ↗</b></a></div></section>

    <footer><div><span className="brand-mark">BFS</span><b>B38 Worker Launch Contract</b></div><p>logic support only · no child process · no backend claim</p><Link href="/worker-containment-v0-1">回看 B37 capability canary →</Link></footer>
  </main>;
}
