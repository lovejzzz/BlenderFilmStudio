"""Independently recompute every B34 scene-linear composite from bound source EXRs."""
...
def main() -> None:
    ...
    if spec["documentType"] != "BFS_HUMAN_QUADRATURE_REVIEW_SPEC" or manifest["studySpecSha256"] != sha256_file(args.study_spec):
        raise RuntimeError("B34 audit binding mismatch")
    observations = []
    ...
    report = {
        "documentType": "BFS_B34_INDEPENDENT_COMPOSITE_AUDIT", "version": spec["version"],
    ...
    print(f"BFS_B34_INDEPENDENT_COMPOSITE_AUDIT exact={report['frameCount']} max={report['maximumAbsoluteFloatError']} changed={report['totalChangedFloatValues']}")
...
if __name__ == "__main__":
    try:
        ...
        print(f"BFS_B34_INDEPENDENT_COMPOSITE_AUDIT_ERROR {error}", file=sys.stderr)
