"""Render one preregistered B35 six-second source process in Blender 5.2."""
...
def cell_controls(spec: dict[str, Any], cell: str) -> tuple[int, list[float] | None]:
    ...
    if cell == "NATURAL32":
        return design["natural32"]["samples"], None
    family, index_text = cell.split("_")
    definition = design["quadrature4" if family == "Q4" else "stratified8"]
    return definition["samplesPerComponent"], definition["points"][int(index_text) - 1]
...
def main() -> None:
    ...
    require_equal(spec["status"], "PREREGISTERED_BEFORE_B35_TOOLING_OUTPUTS_OR_RESPONSES", "study spec status")
    require_equal(review["documentType"], "BFS_REVIEW_RENDER_SPEC", "review spec type")

    scene = bpy.context.scene
    source, runtime = spec["source"], spec["runtime"]
    require_equal(sha256_file(args.review_spec), source["reviewRenderSpecSha256"], "review spec SHA")
    ...
    if args.output_dir.exists() and any(args.output_dir.iterdir()):
        raise RuntimeError("B35 source output directory must be empty")
    args.output_dir.mkdir(parents=True, exist_ok=True)
    args.report.parent.mkdir(parents=True, exist_ok=True)

    realization = spec["newVisualRealization"]
    if scene.camera is None or scene.camera.type != "CAMERA":
        raise RuntimeError("B35 active camera is missing")
    original_lens = float(scene.camera.data.lens)
    require_equal(original_lens, realization["expectedOriginalLensMm"], "original active-camera lens")
    scene.camera.data.lens = realization["intervenedLensMm"]
    require_equal(float(scene.camera.data.lens), realization["intervenedLensMm"], "intervened active-camera lens")
    ...
    observed_controls = {
        ...
        "cameraLensMm": float(scene.camera.data.lens),
    }
    require_equal(observed_controls, {
        ...
        "resolution": spec["renderDesign"]["resolution"],
        "cameraLensMm": realization["intervenedLensMm"],
    }, "frozen controls")

    frame_contract = spec["renderDesign"]["frames"]
    frames = list(range(frame_contract["start"], frame_contract["end"] + 1))
    require_equal(len(frames), frame_contract["count"], "frame count")
    outputs = []
    ...
    for frame in frames:
        ...
        require_equal(float(scene.camera.data.lens), realization["intervenedLensMm"], f"frame {frame} camera lens")
        outputs.append({
            "frame": frame, "name": output.name, "sha256": sha256_file(output),
            "bytes": output.stat().st_size,
            "renderSeconds": round(time.perf_counter() - frame_started, 6),
        })
        print(f"BFS_B35_FRAME_OK {args.cell} {frame}")

    report = {
        "documentType": "BFS_B35_HUMAN_QUADRATURE_SOURCE_RENDER",
        ...
        "visualRealization": {
            ...
            "originalLensMm": original_lens,
            "intervenedLensMm": float(scene.camera.data.lens),
    ...
    print(f"BFS_B35_PROCESS_OK {args.cell} renders={len(outputs)}")
...
if __name__ == "__main__":
    try:
        ...
        print(f"BFS_B35_RENDER_ERROR {error}", file=sys.stderr)
