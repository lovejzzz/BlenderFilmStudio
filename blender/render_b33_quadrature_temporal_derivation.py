"""Render one preregistered B33 consecutive-frame cell-replicate process."""
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--derivation-spec", type=Path, required=True)
...
def cell_controls(spec: dict[str, Any], cell: str) -> tuple[int, list[float] | None]:
    ...
    definition = spec["design"]["quadrature4" if family == "Q4" else "stratified8"]
    return definition["samplesPerComponent"], definition["points"][int(index_text) - 1]


def main() -> None:
    args = parse_args()
    spec = json.loads(args.derivation_spec.read_text(encoding="utf-8"))
    review = json.loads(args.review_spec.read_text(encoding="utf-8"))
    receipt = json.loads(args.receipt.read_text(encoding="utf-8"))
    require_equal(spec["documentType"], "BFS_QUADRATURE_TEMPORAL_DERIVATION_SPEC", "derivation spec type")
    require_equal(spec["status"], "PREREGISTERED_BEFORE_TOOLING_OR_OUTPUTS", "derivation spec status")
    ...
    if args.output_dir.exists() and any(args.output_dir.iterdir()):
        raise RuntimeError("B33 derivation output directory must be empty")
    ...
    for frame in frames:
        ...
        print(f"BFS_B33_DERIVATION_FRAME_OK {args.cell}_{args.replicate} {frame}")

    report = {
        "documentType": "BFS_B33_QUADRATURE_TEMPORAL_DERIVATION_RENDER",
        "version": spec["version"],
        "derivationSpecSha256": sha256_file(args.derivation_spec),
    ...
    print(f"BFS_B33_DERIVATION_PROCESS_OK {args.cell}_{args.replicate} renders={len(outputs)}")
...
if __name__ == "__main__":
    try:
        ...
        print(f"BFS_B33_DERIVATION_RENDER_ERROR {error}", file=sys.stderr)
