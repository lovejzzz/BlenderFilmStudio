...
FRAMES=(193,204,228,252,276,288); CONDITIONS=(("ORIGINAL","CAM_CLOSE_REFLECTION"),("CORRECTED","CAM_CLOSE_REFLECTION_CORRECTED_D4"))
...
def write(path,body):
    body=dict(body); body["reportHash"]=hashlib.sha256(canonical(body)).hexdigest(); path.parent.mkdir(parents=True,exist_ok=True); fd,tmp=tempfile.mkstemp(prefix=".b62-d4-audit-",dir=path.parent)
    try:
...
def state(owner):
    owner=getattr(owner,"original",None) or owner; mats=[m for m in getattr(getattr(owner,"data",None),"materials",[]) if m]
    rows=[]
    for m in mats:
...
def trace(scene,graph,origin,heading,maximum):
    ...
    for _ in range(64):
        hit,loc,_n,_f,owner,_m=scene.ray_cast(graph,cursor,direction,distance=maximum-travelled)
        if not hit or owner is None:return None,"MISS",False
        distance=float((loc-cursor).length); name,passthrough=state(owner)
...
def projection(scene,graph,camera):
    ...
    xs=[p[0] for p in points]; ys=[p[1] for p in points]; area=max(0,min(1,max(xs))-max(0,min(xs)))*max(0,min(1,max(ys))-max(0,min(ys)))
...
def geometry(scene,frame,condition,camera):
    scene.frame_set(frame); scene.camera=camera; graph=bpy.context.evaluated_depsgraph_get(); graph.update(); corners=camera.data.view_frame(scene=scene); left,right=min(c.x for c in corners),max(c.x for c in corners); bottom,top=min(c.y for c in corners),max(c.y for c in corners); z=sum(c.z for c in corners)/4; origin=camera.matrix_world.translation.copy(); rotation=camera.matrix_world.to_quaternion(); counts={}; groups={"CHARACTER":0,"CORE":0,"SCENE_OR_PROP":0,"OTHER":0,"MISS":0}
    for flat in range(32*18):
        y,x=divmod(flat,32); u,v=(x+.5)/32,(y+.5)/18; local=Vector((left+(right-left)*u,bottom+(top-bottom)*v,z)); name,g,exhausted=trace(scene,graph,origin,rotation@local,1000); req(not exhausted,"trace exhausted"); groups[g]+=1; counts[name]=counts.get(name,0)+1 if name else counts.get(name,0)
    visible=[]
    ...
    proj=projection(scene,graph,camera); helmet=counts.get("B62_HELMET",0)/576; character=groups["CHARACTER"]/576; feasible=FACE.issubset(visible) and helmet<=.70 and .20<=character<=.90 and .10<=proj["onScreenVertexFraction"]<=.60 and .35<=proj["clampedUnionAreaFraction"]<=.90 and len(visible)>=2
    return {"frame":frame,"condition":condition,"camera":camera.name,"objectCounts":dict(sorted((k,v) for k,v in counts.items() if k)),"groupCounts":groups,"helmetVisualBlockerShare":helmet,"characterVisualBlockerShare":character,"visibleAnchors":visible,"visibleAnchorCount":len(visible),"characterProjection":proj,"feasible":feasible}
def decode(path):
    image=oiio.ImageInput.open(str(path)); req(image is not None,"EXR open failed")
    try:
        matches=[]; sub=0
...
def main():
    a=args(); scene_path=Path(bpy.data.filepath).resolve(); req(sha(scene_path)==a.derived_sha256,"scene identity"); build=json.loads(a.build_report.read_text()); render=json.loads(a.render_report.read_text()); source=bpy.data.objects["CAM_CLOSE_REFLECTION"]; corrected=bpy.data.objects["CAM_CLOSE_REFLECTION_CORRECTED_D4"]; req(corrected.data.lens==65.0,"lens")
    bake=[]
    for frame in range(193,289):
        bpy.context.scene.frame_set(frame); graph=bpy.context.evaluated_depsgraph_get(); graph.update(); loc=source.evaluated_get(graph).matrix_world.translation; expected=TARGET+(ROTATION@(loc-TARGET))*2; actual=corrected.evaluated_get(graph).matrix_world.translation; bake.append({"frame":frame,"maxLocationError":max(abs(actual[i]-expected[i]) for i in range(3))})
    req(max(r["maxLocationError"] for r in bake)<=1e-6,"bake error")
    observations=[geometry(bpy.context.scene,f,c,bpy.data.objects[n]) for f in FRAMES for c,n in CONDITIONS]
    pixel=[]
    for row in render["renders"]:
        path=a.render_report.parent/row["exr"]["uri"]; decoded=decode(path); req(sha(path)==row["exr"]["sha256"] and decoded["sha256"]==row["combined"]["sha256"],"pixel mismatch"); pixel.append({"frame":row["frame"],"condition":row["condition"],"exrSha256":sha(path),"combined":decoded})
    corrected_pass=all(r["feasible"] for r in observations if r["condition"]=="CORRECTED"); original_fail=all(not r["feasible"] for r in observations if r["condition"]=="ORIGINAL")
    report=write(a.output,{"schemaVersion":"bfs.b62CameraQualityIndependentHoldoutAudit.v0.1","experimentId":"B62-Q1-D4","status":"PASS","derivedScene":{"sha256":a.derived_sha256},"buildReportHash":build["reportHash"],"renderReportHash":render["reportHash"],"bake":bake,"geometry":observations,"pixels":pixel,"outcome":{"correctedAllPass":corrected_pass,"originalAllFail":original_fail},"blender":{"version":bpy.app.version_string,"buildHash":bpy.app.build_hash.decode()},"operations":{"blenderStarts":1,"renderCalls":0,"modelCalls":0,"networkCalls":0,"dockerProcesses":0}}); print(f"BFS_B62_Q1_D4_INDEPENDENT PASS report={report['reportHash']}")
