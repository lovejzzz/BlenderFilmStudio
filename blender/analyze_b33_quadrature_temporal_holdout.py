"""Analyze preregistered B33 formal consecutive-frame temporal-error holdout outputs."""
...
def main() -> None:
    ...
    if spec.get("documentType") != "BFS_QUADRATURE_TEMPORAL_HOLDOUT_SPEC":
        raise RuntimeError("B33 holdout spec type mismatch")
    if index.get("documentType") != "BFS_B33_QUADRATURE_TEMPORAL_HOLDOUT_ANALYSIS_INDEX":
        raise RuntimeError("B33 holdout analysis index type mismatch")
    if index.get("holdoutSpecSha256") != sha256_file(args.spec):
        raise RuntimeError("B33 holdout index/spec binding mismatch")
    processes = index.get("processes", [])
    schedule = expected_schedule()
    if [item.get("replicateId") for item in processes] != schedule:
        raise RuntimeError("B33 holdout process schedule mismatch")
    if len({item.get("processId") for item in processes}) != spec["design"]["totalProcesses"]:
        raise RuntimeError("B33 holdout process IDs are missing or duplicated")
    ...
    temporal_exact = all(
        ...
        for item in transitions for domain in domains
    )
    denominators_valid = all(finite_positive(value) for value in all_denominators)
    q8_near_natural = all(
        item["ratios"][domain]["q8ToNaturalRmse"] <= gates["q8ToNaturalMaximumEveryTransition"][domain]
        for item in transitions for domain in domains
    )
    q8_dominance = (
        all(
            item["ratios"][domain]["q8ToQ4Rmse"] <= gates["q8ToQ4MaximumEveryTransitionEveryDomain"]
            for item in transitions for domain in domains
        )
        and all(
            ratio_summary[domain]["q8ToQ4Rmse"]["mean"] <= gates["q8ToQ4MeanMaximumEveryDomain"]
            for domain in domains
        )
    )
    repeatability = frame_exact and temporal_exact
    component_verdicts = {
        "reference": "REFERENCE_RELIABLE" if reference_reliable else "REFERENCE_UNRELIABLE",
        "repeatability": (
            "Q4_Q8_TEMPORAL_EXACT_REPEATABILITY_SUPPORT"
            if repeatability else "ENSEMBLE_TEMPORAL_REPEATABILITY_FAILURE"
        ),
        "q8NearNatural": (
            "Q8_NEAR_NATURAL_TEMPORAL_PROXY_SUPPORT"
            if q8_near_natural else "Q8_NEAR_NATURAL_TEMPORAL_PROXY_FAILURE"
        ),
        "q8Dominance": (
            "Q8_OVER_Q4_TEMPORAL_DOMINANCE_SUPPORT"
            if q8_dominance else "Q8_OVER_Q4_TEMPORAL_DOMINANCE_FAILURE"
        ),
    }
    all_positive = reference_reliable and repeatability and denominators_valid and q8_near_natural and q8_dominance
    ...
    result = {
        "documentType": "BFS_B33_QUADRATURE_TEMPORAL_HOLDOUT_ANALYSIS",
        ...
        "layout": layout,
        "valid": True,
        "status": "FORMAL_HOLDOUT",
        "decision": decision,
        "componentVerdicts": component_verdicts,
        "validityComponents": {
            ...
            "q8NearNatural": q8_near_natural,
            "q8Dominance": q8_dominance,
        },
    ...
    print(
        f"BFS_B33_HOLDOUT_ANALYZE_OK decision={decision} "
...
if __name__ == "__main__":
    try:
        ...
        print(f"BFS_B33_HOLDOUT_ANALYZE_ERROR {error}", file=sys.stderr)
