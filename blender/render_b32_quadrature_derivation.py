"""Render one fixed-offset component of the exploratory B32 4-point ensemble."""
...
POINTS = {
    "Q1": [-0.25, -0.25], "Q2": [-0.25, 0.25],
    "Q3": [0.25, -0.25], "Q4": [0.25, 0.25],
}
...
def main() -> None:
    ...
    if args.frames != [37, 72, 103]:
        raise RuntimeError("B32 derivation frames changed")
    ...
    observed = {"samples": int(scene.eevee.taa_render_samples), "jitter": [float(value) for value in scene["override_pixel_jitter_sample"]],
                "threadsMode": scene.render.threads_mode, "threads": int(scene.render.threads),
                "dither": float(scene.render.dither_intensity), "useFastGi": bool(scene.eevee.use_fast_gi),
                "useTaaReprojection": bool(scene.eevee.use_taa_reprojection)}
    expected = {"samples": 32, "jitter": jitter, "threadsMode": "FIXED", "threads": 8,
                "dither": 0.0, "useFastGi": True, "useTaaReprojection": True}
    if observed != expected:
        raise RuntimeError(f"B32 controls mismatch: {observed!r}")
    ...
    for frame in args.frames:
        ...
        if "FINISHED" not in result or not output.exists():
            raise RuntimeError(f"B32 frame {frame} failed")
    ...
    report = {
        "documentType": "BFS_B32_QUADRATURE_DERIVATION_RENDER", "version": "0.1.0",
        "status": "EXPLORATORY_DERIVATION_ONLY_NOT_CONFIRMATION", "point": args.point,
        "replicate": args.replicate, "jitter": jitter, "processId": os.getpid(), "frames": args.frames,
        "observedControls": observed, "outputs": outputs, "renderCalls": len(outputs),
        ...
        "runtime": {"blenderVersion": bpy.app.version_string, "buildHash": bpy.app.build_hash.decode("utf-8"),
    ...
    print(f"BFS_B32_QUADRATURE_RENDER_OK {args.point}_{args.replicate} seconds={report['totalRenderSeconds']}")
...
if __name__ == "__main__":
    try:
        ...
        print(f"BFS_B32_QUADRATURE_RENDER_ERROR {error}", file=sys.stderr)
