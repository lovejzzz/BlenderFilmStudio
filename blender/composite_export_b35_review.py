"""Average B35 source EXRs in scene-linear ACEScg and export pinned display PNGs."""
...
def main() -> None:
    ...
    for target in (args.composite_root, args.display_root):
        if target.exists() and any(target.iterdir()):
            raise RuntimeError(f"B35 composite/display output directory must be empty: {target}")
    ...
    for method in METHODS:
        ...
        for frame in frames:
            ...
            if any(not path.is_file() for path in source_paths):
                raise RuntimeError(f"Missing B35 source for {method} frame {frame}")
            ...
            print(f"BFS_B35_COMPOSITE_OK {method} {frame}")
        methods[method] = {"cells": cells, "weights": weights, "frameCount": len(outputs), "outputs": outputs}

    report = {
        "documentType": "BFS_B35_SCENE_LINEAR_COMPOSITE_AND_DISPLAY_EXPORT",
    ...
    print("BFS_B35_COMPOSITE_EXPORT_OK methods=3 frames=432")
...
if __name__ == "__main__":
    try:
        ...
        print(f"BFS_B35_COMPOSITE_EXPORT_ERROR {error}", file=sys.stderr)
