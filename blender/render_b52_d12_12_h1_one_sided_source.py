#!/usr/bin/env python3
"""Render one preregistered B52-D12.12-H1 Blender 5.2 source cell."""
...
def owner_material(spec: dict, owner_spec: dict) -> bpy.types.Material:
    ...
    prefix = f"BFS_D1212H1_{owner_spec['analyticOwnerId']}"
...
def key_transform(owner: bpy.types.Object, transforms: dict) -> None:
    ...
    if action is None:
        raise RuntimeError(f"missing D12.12-H1 action: {owner.name}")
...
def setup(spec: dict, fixture: dict, frame: int, repeat: int):
    ...
    scene.name = f"BFS_D1212H1_{fixture['id']}_F{frame}_R{repeat}"
    ...
    world = bpy.data.worlds.new("BFS_D1212H1_WORLD")
    ...
    camera_data = bpy.data.cameras.new("BFS_D1212H1_CAMERA_DATA")
    ...
    camera = bpy.data.objects.new("BFS_D1212H1_CAMERA", camera_data)
...
def main() -> None:
    ...
    if fixture is None or cli.report.exists() or (cli.output_exr and cli.output_exr.exists()):
        raise RuntimeError("D12.12-H1 fixture or output invalid")
    if not cli.probe_only and cli.output_exr is None:
        raise RuntimeError("D12.12-H1 output EXR required")
    runtime = spec["runtime"]
    if sha_file(Path(bpy.app.binary_path)) != runtime["blender"]["sha256"]:
        raise RuntimeError("D12.12-H1 Blender executable identity mismatch")
    if bpy.app.version_string != runtime["blender"]["version"] or bpy.app.build_hash.decode() != runtime["blender"]["buildHash"]:
        raise RuntimeError("D12.12-H1 Blender version identity mismatch")
    if sha_file(Path(os.environ["OCIO"])) != runtime["ocio"]["sha256"]:
        raise RuntimeError("D12.12-H1 OCIO identity mismatch")
    started = time.monotonic()
    scene, camera, owners = setup(spec, fixture, cli.frame, cli.repeat)
    layer = bpy.context.view_layer
    body = {
        "schemaVersion": "bfs.blenderMaterialOwnerOneSidedCurvatureHoldoutSourceReport.v0.1",
    ...
    if not cli.probe_only:
        ...
        if "FINISHED" not in outcome:
            raise RuntimeError(f"D12.12-H1 source render failed: {sorted(outcome)}")
    ...
    print(f"BFS_B52_D1212H1_SOURCE_OK fixture={fixture['id']} frame={cli.frame} repeat={cli.repeat} probe={cli.probe_only}")
