"""Verify one decoded B34 playback carrier against frozen display PNGs."""
...
def main() -> None:
    ...
    if spec.get("documentType") != "BFS_HUMAN_QUADRATURE_REVIEW_SPEC":
        raise RuntimeError("B34 study spec type mismatch")
    ...
    report = {
        "documentType": "BFS_B34_LOSSLESS_CARRIER_ROUNDTRIP",
    ...
    print(f"BFS_B34_CARRIER_ROUNDTRIP {args.method} exact={report['exactRgbFrames']}/{report['frameCount']} max={report['maximumAbsoluteRgbError']} changed={report['totalChangedRgbPixels']} alpha={report['allSourceAlphaOpaque']}")
    if report["exactRgbFrames"] != expected_count or not all_alpha_opaque or report["totalChangedRgbPixels"] != 0:
        raise RuntimeError("B34 carrier roundtrip is not exact")
...
if __name__ == "__main__":
    try:
        ...
        print(f"BFS_B34_CARRIER_ROUNDTRIP_ERROR {error}", file=sys.stderr)
