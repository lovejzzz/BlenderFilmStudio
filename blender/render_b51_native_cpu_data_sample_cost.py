"""Render one preregistered B51-D5 native CPU sample-dose cell."""
...
def main() -> None:
    ...
    variant = next((item for item in spec["variants"] if item["id"] == args.variant), None)
    if variant is None or args.samples not in spec["sampleLadder"] or args.repeat not in range(1, spec["repeatsPerVariantDose"] + 1):
        raise RuntimeError("unregistered D5 matrix cell")
    parent_spec_path = args.spec.parent / "native-metal-production-holdout.v0.1.json"
    ...
    device = configure_cpu(parent_spec)
    ...
    run_id = f"{variant['id']}_S{args.samples:03d}_R{args.repeat}"
    report = {
        "schemaVersion": "bfs.nativeCpuDataPassSampleCostWorkerReport.v0.1",
    ...
    print(f"BFS_B51_D5_RENDER_OK run={run_id} renderSeconds={report['renderSeconds']}", flush=True)
...
if __name__ == "__main__":
    try:
        ...
        print(f"BFS_B51_D5_RENDER_ERROR {error}", file=sys.stderr, flush=True)
