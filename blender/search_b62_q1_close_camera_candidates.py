...
FRAMES = (216, 240, 264)
SEALED = (193, 204, 228, 252, 276, 288)
ANGLES = (-180, -135, -90, -45, 0, 45, 90, 135)
SCALES = (1.0, 1.5, 2.0)
LENSES = (50.0, 65.0, 85.0, 100.0)
...
def persist(path, payload):
    ...
    descriptor, staging = tempfile.mkstemp(prefix=".b62-q1-d3-primary-", suffix=".tmp", dir=folder)
    try:
...
def classify(owner):
    ...
    for material in materials:
        outputs = sorted([node for node in material.node_tree.nodes if node.type == "OUTPUT_MATERIAL"], key=lambda node: node.name) if material.use_nodes and material.node_tree else []
        rows.append({"material": material.name, "usesNodes": bool(material.use_nodes), "outputCount": len(outputs), "surfaceLinked": any(bool(node.inputs.get("Surface") and node.inputs["Surface"].is_linked) for node in outputs), "volumeLinked": any(bool(node.inputs.get("Volume") and node.inputs["Volume"].is_linked) for node in outputs)})
    passthrough = bool(rows) and all(row["usesNodes"] and row["outputCount"] and row["volumeLinked"] and not row["surfaceLinked"] for row in rows)
    return {"object": original.name, "classification": "VOLUME_ONLY_PASS_THROUGH" if passthrough else "VISUAL_BLOCKER", "materials": rows}
...
def trace(scene, graph, origin, heading, maximum, inventory):
    ...
    skipped = 0
    for index in range(MAX_INTERSECTIONS):
...
def candidate_id(angle, scale, lens):
    angle_code = f"M{abs(angle):03d}" if angle < 0 else f"P{angle:03d}"
    return f"AZ_{angle_code}_R{round(scale * 100):03d}_L{round(lens):03d}"


def configure(camera, baseline_location, angle, scale, lens):
    relative = baseline_location - TARGET
    rotated = Matrix.Rotation(math.radians(angle), 4, "Z") @ relative
...
def measure(scene, graph, camera, inventory):
    ...
    return {"objectCounts": dict(sorted(owners.items())), "groupCounts": groups, "helmetVisualBlockerShare": helmet_share, "characterVisualBlockerShare": character_share, "skippedPassThroughIntersections": skipped, "anchors": anchors, "visibleAnchors": visible, "visibleAnchorCount": len(visible), "faceAnchorVisibleCount": sum(1 for name in visible if name in FACE), "characterProjection": projected, "feasible": feasible}
...
def main():
    ...
    require(original_camera is not None, "close camera missing")
    data = original_camera.data.copy()
    camera = bpy.data.objects.new("B62_Q1_D3_PRIMARY_CAMERA", data)
            ...
            for scale in SCALES:
                for lens in LENSES:
                    ...
                    for frame in FRAMES:
                        ...
                        configure(camera, baseline, angle, scale, lens)
                        frames.append({"frame": frame, **measure(scene, graph, camera, inventory)})
                    cost = abs(angle) / 180.0 + abs(scale - 1.0) + abs(lens - 100.0) / 100.0
                    candidates.append({"candidateId": candidate_id(angle, scale, lens), "azimuthDegrees": angle, "radialScale": scale, "lensMillimeters": lens, "interventionCost": cost, "frames": frames, "feasible": all(row["feasible"] for row in frames), "minimumVisibleAnchorCount": min(row["visibleAnchorCount"] for row in frames), "minimumFaceAnchorVisibleCount": min(row["faceAnchorVisibleCount"] for row in frames), "maximumHelmetVisualBlockerShare": max(row["helmetVisualBlockerShare"] for row in frames)})
    finally:
        bpy.data.objects.remove(camera, do_unlink=True)
        bpy.data.cameras.remove(data)
    require(len(candidates) == 96, "candidate roster mismatch")
    feasible = [row for row in candidates if row["feasible"]]
    feasible.sort(key=lambda row: (row["interventionCost"], -row["minimumVisibleAnchorCount"], row["maximumHelmetVisualBlockerShare"], -row["minimumFaceAnchorVisibleCount"], row["candidateId"]))
    baseline = next(row for row in candidates if row["azimuthDegrees"] == 0 and row["radialScale"] == 1.0 and row["lensMillimeters"] == 100.0)
    ...
    payload = {"schemaVersion": "bfs.b62CameraQualityBoundedCandidateObservation.v0.1", "experimentId": "B62-Q1-D3", "implementation": "PRIMARY", "status": "OBSERVED", "master": {"filepath": bpy.data.filepath, "expectedSha256": args.master_sha256}, "blender": {"version": bpy.app.version_string, "buildHash": bpy.app.build_hash.decode("utf-8")}, "derivationFramesEvaluated": list(FRAMES), "sealedHoldoutFramesNotEvaluated": list(SEALED), "materialAwareAtmosphere": atmosphere, "candidateCount": len(candidates), "feasibleCandidateCount": len(feasible), "baselineCandidateId": baseline["candidateId"], "baselineFeasible": baseline["feasible"], "selectedCandidateId": feasible[0]["candidateId"] if feasible else None, "candidates": candidates, "operations": {"blenderStarts": 1, "framesSet": len(FRAMES) * len(candidates), "renderCalls": 0, "modelCalls": 0, "networkCalls": 0, "dockerProcesses": 0}}
    persist(os.path.abspath(args.output), payload)
    print(f"BFS_B62_Q1_D3_PRIMARY OBSERVED candidates=96 feasible={len(feasible)} render=0")
