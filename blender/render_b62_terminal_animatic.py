...
def main():
    ...
    production_file_format=scene.render.image_settings.file_format;req(production_file_format=="OPEN_EXR_MULTILAYER","production file format")
    ...
    for frame in range(1,289):
        ...
        path=out/f"frame-{frame:04d}.png";req(not path.exists(),"frame exists");t=time.perf_counter();result=bpy.ops.render.render(write_still=False);elapsed=time.perf_counter()-t;req("FINISHED" in result,f"render result {frame}");image=bpy.data.images.get("Render Result");req(image is not None,f"Render Result absent {frame}");image.save_render(filepath=str(path),scene=output_scene)
