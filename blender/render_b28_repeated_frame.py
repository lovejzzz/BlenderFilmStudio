"""Render frame 38 twelve times in one persistent Blender process for B28."""
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--b28-spec", type=Path, required=True)
...
def camera_snapshot(scene: bpy.types.Scene, frames: list[int]) -> dict[str, Any]:
    ...
    return {"scene": scene.name, "cameraObject": camera.name, "cameraData": camera.data.name, "frameStart": scene.frame_start, "frameEnd": scene.frame_end, "fps": scene.render.fps, "fpsBase": stable_number(scene.render.fps_base), "samples": samples}


def main() -> None:
    args = parse_args()
    spec = json.loads(args.b28_spec.read_text(encoding="utf-8"))
    review = json.loads(args.review_spec.read_text(encoding="utf-8"))
    receipt = json.loads(args.receipt.read_text(encoding="utf-8"))
    scene = bpy.context.scene
    require_equal(spec["documentType"], "BFS_REPEATED_FRAME_MODE_SWITCH_SPEC", "B28 spec type")
    require_equal(spec["status"], "pre-registered", "B28 status")
    ...
    if re.fullmatch(r"P(0[1-9]|1[0-2])", args.replicate) is None:
        raise RuntimeError(f"Invalid B28 replicate {args.replicate!r}")
    ...
    if args.output_dir.exists() and any(args.output_dir.iterdir()):
        raise RuntimeError("B28 output directory must be empty")
    ...
    observed_controls = {
        ...
        "renderSamples": int(scene.eevee.taa_render_samples), "ditherIntensity": float(scene.render.dither_intensity),
    ...
    require_equal(observed_controls, {
        "threadsMode": constants["threadsMode"], "threads": constants["threads"], "renderSamples": constants["renderSamples"],
    ...
    frame_set_count = 1
    frame_after_set = scene.frame_current
    started = time.perf_counter()
    outputs = []
    for ordinal in range(1, calls + 1):
        if scene.frame_current != target:
            raise RuntimeError(f"Frame changed before render {ordinal}: {scene.frame_current}")
        call_started = time.perf_counter()
        output = args.output_dir / f"render-{ordinal:02d}.png"
        scene.render.filepath = str(output.resolve())
        result = bpy.ops.render.render(write_still=True)
        if "FINISHED" not in result or not output.exists():
            raise RuntimeError(f"Render {ordinal} failed: {sorted(result)}")
        if scene.frame_current != target:
            raise RuntimeError(f"Frame changed after render {ordinal}: {scene.frame_current}")
        item = {"callOrdinal": ordinal, "frame": target, "name": output.name, "sha256": sha256_file(output), "bytes": output.stat().st_size, "renderSeconds": round(time.perf_counter() - call_started, 6)}
        outputs.append(item)
        print(f"BFS_B28_RENDER_CALL_OK {args.replicate} {ordinal:02d} {item['sha256']} {item['renderSeconds']}")
    ...
    report = {
        "documentType": "BFS_B28_REPEATED_FRAME_RENDER", "version": "0.1.0", "b28SpecSha256": sha256_file(args.b28_spec),
        "replicate": args.replicate, "processId": os.getpid(), "classification": proxy["classification"],
        "source": {"receiptHash": source["receiptHash"], "executionIdentityHash": source["executionIdentityHash"], "planHash": scene["bfs_plan_hash"], "structureHash": scene["bfs_structure_hash"], "sceneBlendSha256": sha256_file(Path(bpy.data.filepath))},
        "runtime": {"blenderVersion": bpy.app.version_string, "blenderBuildHash": bpy.app.build_hash.decode("utf-8"), "renderEngine": scene.render.engine, "ocioConfigName": ocio.GetCurrentConfig().getName()},
        "profile": proxy, "observedControls": observed_controls,
        "identityBefore": identity_before, "identityAfter": identity_after, "cameraAndTimelineInvariant": identity_before == identity_after,
        "targetFrame": target, "frameSetCountBeforeRenders": frame_set_count, "frameAfterSet": frame_after_set,
        "renderOperatorCallCount": len(outputs), "callOrder": [item["callOrdinal"] for item in outputs],
        "frameObservedEveryCall": all(item["frame"] == target for item in outputs), "outputs": outputs, "outputFileCount": len(outputs),
        "totalBytes": sum(item["bytes"] for item in outputs), "totalRenderSeconds": round(time.perf_counter() - started, 6), "savedSourceBlend": False,
    }
    args.report.write_text(json.dumps(report, ensure_ascii=False, indent=2, sort_keys=True) + "\n", encoding="utf-8")
    print(f"BFS_B28_PROCESS_RENDER_OK {args.replicate} calls={len(outputs)} seconds={report['totalRenderSeconds']}")
...
if __name__ == "__main__":
    try:
        ...
        print(f"BFS_B28_RENDER_ERROR {error}", file=sys.stderr)
