...
FRAMES = (193, 204, 228, 252, 276, 288)
CONDITIONS = (("ORIGINAL", "CAM_CLOSE_REFLECTION"), ("CORRECTED", "CAM_CLOSE_REFLECTION_CORRECTED_D4"))
            ...
            for name in names:
                if name.endswith(".R") and name[:-2].split(".")[-1] == "Combined":
                    ...
                    if all(channel in positions for channel in wanted):
                        matches.append((subimage, spec.width, spec.height, spec.nchannels, prefix, wanted, [positions[channel] for channel in wanted]))
            subimage += 1
...
def main():
    ...
    require(scene_path.name == "B62_PHASE0_D4_CAMERA_CORRECTED.blend" and sha256_file(scene_path) == args.derived_sha256, "derived scene identity mismatch")
    ...
    for frame in FRAMES:
        ...
        corrected = next(row for row in rows if row["frame"] == frame and row["condition"] == "CORRECTED")
        pairs.append({"frame": frame, "originalCombinedSha256": original["combined"]["sha256"], "correctedCombinedSha256": corrected["combined"]["sha256"], "different": original["combined"]["sha256"] != corrected["combined"]["sha256"]})
    report = write_hashed(args.report, {"schemaVersion": "bfs.b62CameraQualityHoldoutRender.v0.1", "experimentId": "B62-Q1-D4", "status": "PASS", "derivedScene": {"filepath": str(scene_path), "sha256": args.derived_sha256}, "settings": {"engine": scene.render.engine, "device": scene.cycles.device, "resolution": [960, 540], "samples": 16, "denoise": True, "seed": 62004, "animatedSeed": False, "format": "OPEN_EXR_MULTILAYER", "pixelType": "HALF", "compression": "ZIP"}, "renders": rows, "pairs": pairs, "elapsedSeconds": time.perf_counter() - started, "blender": {"version": bpy.app.version_string, "buildHash": bpy.app.build_hash.decode("utf-8")}, "operations": {"blenderStarts": 1, "renderCalls": 12, "modelCalls": 0, "networkCalls": 0, "dockerProcesses": 0}}, "reportHash")
    print(f"BFS_B62_Q1_D4_RENDER PASS renders={len(rows)} seconds={report['elapsedSeconds']:.3f} report={report['reportHash']}")
