#!/usr/bin/env python3
"""Pass one B52-D11 Raw resolved EXR through the Blender 5.2 compositor."""
...
def main() -> None:
    ...
    tree = bpy.data.node_groups.new("BFS_D11_TREE", "CompositorNodeTree")
    scene.compositing_node_group = tree
    source = tree.nodes.new("CompositorNodeImage")
    source.name = "BFS_D11_EXTERNAL_SOURCE"
    source.image = image
    tree.interface.new_socket(name="Image", in_out="OUTPUT", socket_type="NodeSocketColor")
    output = tree.nodes.new("NodeGroupOutput")
    output.name = "BFS_D11_GROUP_OUTPUT"
    ...
    if not args.probe_only:
        camera_data = bpy.data.cameras.new("BFS_D11_BRIDGE_CAMERA_DATA")
        camera = bpy.data.objects.new("BFS_D11_BRIDGE_CAMERA", camera_data)
    ...
    body = {
        "schemaVersion": "bfs.blenderRealTexturedTemporalBridgeReport.v0.1",
    ...
    print(f"BFS_B52_D11_{'PREFLIGHT' if args.probe_only else 'BRIDGE'}_OK fixture={args.fixture} sourceRepeat={args.source_repeat} bridgeRepeat={args.bridge_repeat}")
