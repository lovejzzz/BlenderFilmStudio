#!/usr/bin/env python3
"""Render one preregistered B52-D12.11-I1 Material Index intervention cell."""
...
def owner_material(spec: dict, role: str, material_index: int) -> bpy.types.Material:
    definition = spec["sceneContract"]["materials"][role]
    prefix = f"BFS_D129_{role.upper()}"
    ...
    wave_u = math_node(nodes, "MULTIPLY", f"{prefix}_WAVE_U", definition["waveFrequency"][0])
    wave_v = math_node(nodes, "MULTIPLY", f"{prefix}_WAVE_V", definition["waveFrequency"][1])
...
def add_surface(spec: dict, owner_spec: dict, material_index: int) -> bpy.types.Object:
    surfaces = spec["sceneContract"]["surfaces"]
    if owner_spec["role"] == "background":
        width, height = surfaces["backgroundSizeWorld"]
        columns, rows = surfaces["backgroundSubdivisions"]
    else:
        width, height = surfaces["occluderSizeWorld"]
        columns, rows = surfaces["occluderSubdivisions"]
    vertices = []
    ...
    mesh = bpy.data.meshes.new(f"BFS_D129_{owner_spec['analyticOwnerId']}_MESH")
    mesh.from_pydata(vertices, [], faces)
    mesh.update()
    owner = bpy.data.objects.new(f"BFS_D129_{owner_spec['analyticOwnerId']}", mesh)
    owner.rotation_mode = spec["sceneContract"]["eulerOrder"]
    owner.pass_index = owner_spec["passIndex"]
    mesh.materials.append(owner_material(spec, owner_spec["role"], material_index))
    bpy.context.scene.collection.objects.link(owner)
    return owner


def key_transform(owner: bpy.types.Object, transforms: dict) -> None:
    for frame_text, transform in transforms.items():
    ...
    if action is None:
        raise RuntimeError(f"missing D12.11-I1 action: {owner.name}")
...
def setup(spec: dict, intervention: dict, fixture: dict, frame: int, repeat: int):
    bpy.ops.wm.read_factory_settings(use_empty=True)
    scene = bpy.context.scene
    scene.name = f"BFS_D129_{fixture['id']}_F{frame}_R{repeat}"
    ...
    world = bpy.data.worlds.new("BFS_D129_WORLD")
    ...
    camera_data = bpy.data.cameras.new("BFS_D129_CAMERA_DATA")
    ...
    camera = bpy.data.objects.new("BFS_D129_CAMERA", camera_data)
    ...
    for owner_spec in fixture["owners"]:
        token = intervention["materialOwnerTokens"]["assignments"][owner_spec["analyticOwnerId"]]
        owner = add_surface(spec, owner_spec, token)
...
def structure(owner: bpy.types.Object) -> dict:
...
def main() -> None:
    ...
    spec = json.loads(h1_spec_path.read_text())
    fixture = next((row for row in spec["fixtures"] if row["id"] == args.fixture), None)
    if fixture is None or args.report.exists() or (args.output_exr and args.output_exr.exists()):
        raise RuntimeError("D12.11-I1 fixture or output invalid")
    if not args.probe_only and args.output_exr is None:
        raise RuntimeError("D12.11-I1 output EXR required")
    if sha_file(Path(bpy.app.binary_path)) != spec["runtime"]["blender"]["sha256"]:
        raise RuntimeError("D12.11-I1 Blender executable identity mismatch")
    if bpy.app.version_string != spec["runtime"]["blender"]["version"] or bpy.app.build_hash.decode() != spec["runtime"]["blender"]["buildHash"]:
        raise RuntimeError("D12.11-I1 Blender version identity mismatch")
    if sha_file(Path(os.environ["OCIO"])) != spec["runtime"]["ocio"]["sha256"]:
        raise RuntimeError("D12.11-I1 OCIO identity mismatch")
    started = time.monotonic()
    scene, camera, owners = setup(spec, intervention, fixture, args.frame, args.repeat)
    layer = bpy.context.view_layer
    body = {
        "schemaVersion": "bfs.blenderMaterialIndexOwnerSourceReport.v0.1",
        ...
        "sceneStructure": {
            ...
            "owners": [structure(owner) for owner in owners],
        },
    ...
    if not args.probe_only:
        ...
        if "FINISHED" not in outcome:
            raise RuntimeError(f"D12.11-I1 source render failed: {sorted(outcome)}")
    ...
    body["operationCounts"] = {
        "blenderProcesses": 1,
        "blenderRenderCalls": 0 if args.probe_only else 1,
        "cyclesRayRenders": 0 if args.probe_only else 1,
        "modelCalls": 0,
        "networkCalls": 0,
    }
    ...
    print(f"BFS_B52_D1211_SOURCE_OK fixture={fixture['id']} frame={args.frame} repeat={args.repeat} probe={args.probe_only}")
