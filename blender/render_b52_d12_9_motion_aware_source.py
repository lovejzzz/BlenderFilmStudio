#!/usr/bin/env python3
"""Render one preregistered B52-D12.9-H1 multipart source cell."""
...
def owner_material(spec: dict, role: str) -> bpy.types.Material:
...
def add_surface(spec: dict, owner_spec: dict) -> bpy.types.Object:
    ...
    mesh.materials.append(owner_material(spec, owner_spec["role"]))
...
def key_transform(owner: bpy.types.Object, transforms: dict) -> None:
    ...
    if action is None:
        raise RuntimeError(f"missing D12.9-H1 action: {owner.name}")
...
def main() -> None:
    ...
    if not args.probe_only:
        ...
        if "FINISHED" not in outcome:
            raise RuntimeError(f"D12.9-H1 source render failed: {sorted(outcome)}")
    ...
    print(f"BFS_B52_D129_SOURCE_OK fixture={fixture['id']} frame={args.frame} repeat={args.repeat} probe={args.probe_only}")
