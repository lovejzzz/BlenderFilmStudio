"""Render one preregistered B33 formal consecutive-frame holdout cell-replicate process."""
...
def main() -> None:
    ...
    require_equal(spec["documentType"], "BFS_QUADRATURE_TEMPORAL_HOLDOUT_SPEC", "holdout spec type")
    ...
    if args.output_dir.exists() and any(args.output_dir.iterdir()):
        raise RuntimeError("B33 holdout output directory must be empty")
    ...
    for frame in frames:
        ...
        print(f"BFS_B33_HOLDOUT_FRAME_OK {args.cell}_{args.replicate} {frame}")

    report = {
        "documentType": "BFS_B33_QUADRATURE_TEMPORAL_HOLDOUT_RENDER",
    ...
    print(f"BFS_B33_HOLDOUT_PROCESS_OK {args.cell}_{args.replicate} renders={len(outputs)}")
...
if __name__ == "__main__":
    try:
        ...
        print(f"BFS_B33_HOLDOUT_RENDER_ERROR {error}", file=sys.stderr)
