#!/usr/bin/env python3
"""Render one preregistered B52-D12.8 multipart source cell."""
...
def add_surface(spec: dict, owner_spec: dict) -> bpy.types.Object:
    ...
    mesh = bpy.data.meshes.new(f"BFS_D128_{owner_spec['analyticOwnerId']}_MESH")
    mesh.from_pydata(vertices, [], faces)
    mesh.update()
    owner = bpy.data.objects.new(f"BFS_D128_{owner_spec['analyticOwnerId']}", mesh)
...
def key_transform(owner: bpy.types.Object, transforms: dict) -> None:
    ...
    if action is None:
        raise RuntimeError(f"missing D12.8 action: {owner.name}")
...
def setup(spec: dict, fixture: dict, frame: int, repeat: int):
    ...
    scene.name = f"BFS_D128_{fixture['id']}_F{frame}_R{repeat}"
    ...
    world = bpy.data.worlds.new("BFS_D128_WORLD")
    ...
    camera_data = bpy.data.cameras.new("BFS_D128_CAMERA_DATA")
    ...
    camera = bpy.data.objects.new("BFS_D128_CAMERA", camera_data)
...
def main() -> None:
    ...
    if not args.probe_only:
        ...
        if "FINISHED" not in outcome:
            raise RuntimeError(f"D12.8 source render failed: {sorted(outcome)}")
    ...
    print(f"BFS_B52_D128_SOURCE_OK fixture={fixture['id']} frame={args.frame} repeat={args.repeat} probe={args.probe_only}")
