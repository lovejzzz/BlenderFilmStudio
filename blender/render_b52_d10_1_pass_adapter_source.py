#!/usr/bin/env python3
"""Render one frozen B52-D10.1 Blender multipart source cell."""
...
def main() -> None:
    ...
    if args.output_exr.exists() or args.report.exists() or args.output_exr.parent.exists():
        raise RuntimeError("refusing to overwrite D10.1 source cell")
    ...
    scene.name = f"BFS_D10_1_{args.fixture}_F{args.frame}_R{args.repeat}"
    ...
    world = bpy.data.worlds.new("BFS_D10_1_WORLD")
    ...
    if fixture["moverByFrame"] is not None:
        set_linear_location_keys(objects["BFS_F32_MOVER"], fixture["moverByFrame"])
    ...
    animation_structure = {"camera": action_rows(camera), "mover": action_rows(objects["BFS_F32_MOVER"])}
    ...
    if "FINISHED" not in outcome:
        raise RuntimeError(f"D10.1 source render failed: {sorted(outcome)}")
    ...
    if not args.output_exr.is_file():
        raise RuntimeError("D10.1 source EXR absent")

    body = {
        "schemaVersion": "bfs.blenderMultipartTemporalAdapterF32SourceReport.v0.1",
    ...
    print(f"BFS_B52_D10_1_SOURCE_OK fixture={args.fixture} frame={args.frame} repeat={args.repeat} exr={body['output']['sha256']}", flush=True)
