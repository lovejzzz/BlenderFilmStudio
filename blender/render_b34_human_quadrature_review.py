"""Render one preregistered B34 six-second source process in Blender 5.2."""
...
def main() -> None:
    ...
    require_equal(sha256_file(args.review_spec), spec["evidenceBasis"]["reviewRenderSpecSha256"], "review spec SHA")
    ...
    if args.output_dir.exists() and any(args.output_dir.iterdir()):
        raise RuntimeError("B34 source output directory must be empty")
    ...
    for frame in frames:
        ...
        print(f"BFS_B34_FRAME_OK {args.cell} {frame}")

    report = {
        "documentType": "BFS_B34_HUMAN_QUADRATURE_SOURCE_RENDER",
    ...
    print(f"BFS_B34_PROCESS_OK {args.cell} renders={len(outputs)}")
...
if __name__ == "__main__":
    try:
        ...
        print(f"BFS_B34_RENDER_ERROR {error}", file=sys.stderr)
