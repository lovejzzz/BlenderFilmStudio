...
const packageRoot = '/Users/mengyingli/Documents/ChatGPT/BlenderFilmStudio-F0-workspace/F0.7-packages';
...
const createResult = await runTimed({ command: '/usr/bin/hdiutil', args: createArgs, prefix: createPrefix, timeoutSeconds: maxSeconds });
const createStatus = createResult.exitCode === 0 && !createResult.timeout && existsSync(dmg) && createResult.timing.realSeconds <= maxSeconds ? 'PASS' : 'FAIL';
