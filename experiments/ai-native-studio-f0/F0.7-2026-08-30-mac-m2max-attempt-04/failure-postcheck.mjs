...
if (!repository || !evidenceRelative) throw new Error('Usage: failure-postcheck.mjs <repository-root> <evidence-root-relative>');
...
check('installTargetAbsentAfterCompletedUninstall', !existsSync(installApp));
...
check('officialSandboxRetained', treeIdentity(officialSandbox).state === 'PRESENT');
...
const body = {
  schemaVersion: 'bfs.f0.7.failurePostcheck.v0.1', experimentId: basename(evidence),
...
if (failures.length) throw new Error(`F0.7 failure postcheck failed: ${failures.join(', ')}`);
console.log(`F0.7_FAILURE_POSTCHECK PASS checks=${Object.keys(checks).length} core=${core}`);
