...
const product = '/Users/mengyingli/Documents/ChatGPT/BlenderFilmStudio-F0-workspace/build-f0.3-workspace/bin/Film Studio Engine F0.app/Contents/MacOS/Blender';
const ocio = resolve(repository, 'color/ocio/cg-config-v4.0.0_aces-v2.0_ocio-v2.5.ocio');
const source = resolve(repository, 'experiments/ai-native-studio-f0/F0.4-2026-08-30-mac-m2max-attempt-03/b01/artifacts/scene.blend');
const renderScript = resolve(evidence, 'render-stage.py');
const auditScript = resolve(evidence, 'audit-f05.py');
...
async function runProduct({ sequence, id, script, scriptArgs, interruptMarker = null, renderer, renderCalls }) {
  ...
  const child = spawn('/usr/bin/time', args, {
    cwd: repository,
    detached: true,
    env: { ...process.env, OCIO: ocio, LANG: 'C', LC_ALL: 'C' },
  ...
  const status = interrupted
    ? markerObserved && interruptionDelivered && !interruptionError && result.exitCode !== 0 && !timeout ? 'INTERRUPTED' : 'FAIL'
    : result.exitCode === 0 && !timeout ? 'PASS' : 'FAIL';
  const body = {
...
for (const [path, expected] of [[product, '5a4538163d1fce6c531f2ad76a15232fc1d1d4bbff483d54778a290b4c37cb40'], [source, '648ba4e5c0be2620f0da85dd8fdc0a23d878c39054e61e69029221b5457da942'], [ocio, '24ec81841048fc5db160a7bad882263246183385c5d49d0e86e11464917ead15']]) {
...
const completionBody = {
  ...
  status: totalFormalBytes <= 268435456 && shaFile(source) === '648ba4e5c0be2620f0da85dd8fdc0a23d878c39054e61e69029221b5457da942' ? 'PASS' : 'FAIL',
  jobId: 'F05-B01-RENDER-001',
  formalProductStarts,
  rendererStarts,
  renderCalls: 2,
  ...
  totalFormalBytes,
  sourceBlendUnchanged: shaFile(source) === '648ba4e5c0be2620f0da85dd8fdc0a23d878c39054e61e69029221b5457da942',
...
console.log(`F05_RUN PASS starts=${formalProductStarts} renderers=${rendererStarts} renders=2 bytes=${totalFormalBytes}`);
