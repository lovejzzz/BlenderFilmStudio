...
p=argparse.ArgumentParser();p.add_argument('name');p.add_argument('--project',required=True);p.add_argument('--stills',default='S01,S02,S03,S04');p.add_argument('--width',type=int,default=1280);p.add_argument('--samples',type=int,default=48)
a=p.parse_args()
...
job={'action':'build','project':str(project),'output':str(wd/'output'),'stills':stills,'width':a.width,'samples':a.samples}
...
Path(env['HOME']).mkdir()
argv=[str(binary),'--background','--factory-startup','--disable-autoexec','--python-exit-code','2','--python',str(code/'blender_entry.py'),'--',str(jobpath)]
