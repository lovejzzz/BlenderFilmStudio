...
p=argparse.ArgumentParser();p.add_argument('name');p.add_argument('--project',default='last-signal.film.json');p.add_argument('--blend');p.add_argument('--action',choices=['build','editorial','inspect','exercise','render','resume_test','ui','encode','verify_cache','ui_scope'],default='build');p.add_argument('--maximum-new-frames',type=int,default=0);p.add_argument('--reference');p.add_argument('--frames');p.add_argument('--shots');p.add_argument('--stills',default='S01,S02,S03,S04');p.add_argument('--width',type=int,default=1280);p.add_argument('--samples',type=int,default=48)
a=p.parse_args()
...
if a.reference:
    reference=wd/'reference.json';shutil.copy2(a.reference,reference);job['reference']=str(reference)
