...
import bpy
from . import core,scene,rendering,sound,audio_master
...
def encode(sc,frames,output):
    ...
    audio_filter=audio_master.measured_filter(ffmpeg,wav,output)
    argv=[ffmpeg,'-nostdin','-n','-framerate','24','-i',str(frames/'frame-%05d.png'),'-i',str(wav),'-loop','1','-framerate','24','-i',str(title),'-filter_complex',filters,'-map','[v]','-map','1:a','-frames:v',str(len(p['frames'])),'-c:v','libx264','-preset','slow','-crf','17','-pix_fmt','yuv420p','-af',audio_filter,'-ar','48000','-c:a','aac','-b:a','256k','-movflags','+faststart','-shortest',str(movie)]
