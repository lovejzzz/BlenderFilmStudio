...
C2_SPEC = RESEARCH / "specs/ai-native-studio-rc6-real-impact-liquid-particle-radius-c20-c2.v1.06.json"
C2_EVIDENCE = RESEARCH / "experiments/physical-richness/RC6-2026-09-02-real-impact-liquid-particle-radius-c20-c2-audit-attempt-95"
...
def transformed_source():
    if sha(BASE) != EXPECTED_BASE_SHA256:
        raise RuntimeError("C20 C2 audit base identity mismatch")
...
EVIDENCE = RESEARCH / "experiments/physical-richness/RC6-2026-09-02-real-impact-liquid-particle-radius-c20-c2-audit-attempt-95"
...
freeze_paths = set(subprocess.run(["git", "diff-tree", "--no-commit-id", "--name-only", "-r", head], cwd=RESEARCH, capture_output=True, text=True, check=True).stdout.splitlines())'''
    ...
    check_extension = check_anchor + '''    "c2SpecSelfHash": c2_spec["specHash"] == self_hash(c2_spec, "specHash"),
    "c2ToolIdentity": sha(Path(__file__).resolve()) == c2_spec["toolSha256"],
    "c2FreezeCommitBound": c2_parent == c2_spec["researchParentBeforePreregistration"] and c2_freeze_paths == set(c2_spec["freezePaths"]),
    "retainedEvidenceRootExact": manifest(RETAINED_EVIDENCE)["manifestHash"] == c2_spec["retainedEvidenceManifestHash"],
'''
    audit_schema_old = '"bfs.rc6RealImpactLiquidParticleRadiusC20C1IndependentAudit.v0.1"'
    audit_schema_new = '"bfs.rc6RealImpactLiquidParticleRadiusC20C2AuditOnly.v0.1"'
    ...
    "counts": c2_spec["counts"],
    "claimCeiling": c2_spec["claimCeiling"],
'''
    ...
    replacements = (
        ...
        (check_anchor, check_extension, "C2 checks", 1),
        (audit_schema_old, audit_schema_new, "audit schema", 1),
        (audit_fields_anchor, audit_fields_extension, "C2 audit fields", 1),
        (output_old, output_new, "fresh audit output", 1),
        ("RC6_REAL_IMPACT_LIQUID_PARTICLE_RADIUS_C20_C1_AUDIT=", "RC6_REAL_IMPACT_LIQUID_PARTICLE_RADIUS_C20_C2_AUDIT=", "audit marker", 1),
        ("real-impact liquid C20 C1 independent audit failed", "real-impact liquid C20 C2 audit-only closure failed", "failure marker", 1),
    )
    for before, after, label, expected in replacements:
        if source.count(before) != expected:
            raise RuntimeError(f"C20 C2 {label} target mismatch: {source.count(before)} != {expected}")
        source = source.replace(before, after)
    return source


def main():
    c2_spec = json.loads(C2_SPEC.read_text())
    if c2_spec["specHash"] != self_hash(c2_spec, "specHash"):
        raise RuntimeError("C20 C2 spec self hash mismatch")
    if sha(Path(__file__).resolve()) != c2_spec["toolSha256"]:
        raise RuntimeError("C20 C2 tool identity mismatch")
    if subprocess.run(["git", "status", "--porcelain"], cwd=RESEARCH, capture_output=True, text=True, check=True).stdout:
        raise RuntimeError("C20 C2 worktree is not clean")
    c2_head = subprocess.run(["git", "rev-parse", "HEAD"], cwd=RESEARCH, capture_output=True, text=True, check=True).stdout.strip()
    c2_parent = subprocess.run(["git", "rev-parse", "HEAD^"], cwd=RESEARCH, capture_output=True, text=True, check=True).stdout.strip()
    c2_freeze_paths = set(subprocess.run(["git", "diff-tree", "--no-commit-id", "--name-only", "-r", "HEAD"], cwd=RESEARCH, capture_output=True, text=True, check=True).stdout.splitlines())
    if c2_parent != c2_spec["researchParentBeforePreregistration"] or c2_freeze_paths != set(c2_spec["freezePaths"]):
        raise RuntimeError("C20 C2 freeze commit mismatch")
    retained = RESEARCH / c2_spec["retainedEvidenceRoot"]
    retained_manifest = manifest(retained)
    if retained_manifest["manifestHash"] != c2_spec["retainedEvidenceManifestHash"]:
        raise RuntimeError("C20 C2 retained evidence drift")
    if C2_EVIDENCE.exists():
        raise RuntimeError("C20 C2 evidence root is not fresh")
    C2_EVIDENCE.mkdir(parents=True, exist_ok=False)
    scope = {
        "schemaVersion": "bfs.rc6RealImpactLiquidParticleRadiusC20C2Admission.v0.1",
        "status": "PASS",
        "executionCommit": c2_head,
        "retainedEvidenceManifestHash": retained_manifest["manifestHash"],
        "counts": c2_spec["counts"],
    }
    write_exclusive(C2_EVIDENCE / "admission.json", scope)
    exec(compile(transformed_source(), str(BASE) + "#RC6_REAL_IMPACT_LIQUID_PARTICLE_RADIUS_C20_C2", "exec"), globals(), locals())
    audit = json.loads((C2_EVIDENCE / "audit.json").read_text())
    receipt = {
        "schemaVersion": "bfs.rc6RealImpactLiquidParticleRadiusC20C2Receipt.v0.1",
        "status": audit["status"],
        "executionCommit": c2_head,
    ...
    write_exclusive(C2_EVIDENCE / "receipt.json", receipt)
    write_exclusive(C2_EVIDENCE / "evidence-manifest.json", manifest(C2_EVIDENCE))
    print("RC6_REAL_IMPACT_LIQUID_PARTICLE_RADIUS_C20_C2_RECEIPT=" + canonical(receipt), flush=True)
