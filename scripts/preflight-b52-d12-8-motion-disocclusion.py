...
def main():
 ...
 with tempfile.TemporaryDirectory(prefix="bfs-d128-c1-preflight-") as temporary:
  ...
  for producer,executable,tool in (("python",spec["runtime"]["python"]["executable"],repo/"scripts/reconstruct-b52-d12-8-motion-disocclusion.py"),("node",spec["runtime"]["node"]["executable"],repo/"scripts/reconstruct-b52-d12-8-motion-disocclusion.mjs")):
   output_dir=temp/"synthetic"/producer/"arrays";report=temp/"synthetic"/f"{producer}.json";row=run([executable,str(tool),"--spec",str(spec_path),"--fixture",fixture["id"],"--repeat","1","--input-dir",str(input_dir),"--adapter-report",str(adapter_path),"--output-dir",str(output_dir),"--report",str(report)],repo,base_env);reports[producer]=json.loads(report.read_text());synthetic[f"{producer}ExitZero"]=row["exitCode"]==0
