...
import {
  B40_C4_PREREG_COMMIT, B40_C4_SPEC_SHA256, analyzeB40C4Evidence, classifyB40C2Capacity,
  readB40C4Spec, roundTripB40C2, runB40C4Attacks,
} from './lib/b40-c4-projection-identity.mjs';

const root = resolve(repositoryRoot, 'experiments/worker-host-capacity-admission-v0-5');
const runnerPath = resolve(repositoryRoot, 'scripts/run-b40-c4-projection-identity.mjs');
const libraryPath = resolve(repositoryRoot, 'scripts/lib/b40-c4-projection-identity.mjs');
const auditPath = resolve(repositoryRoot, 'scripts/audit-b40-c4-projection-identity.mjs');
...
if (spawnSync('git', ['merge-base', '--is-ancestor', B40_C4_PREREG_COMMIT, 'HEAD'], { cwd: repositoryRoot }).status !== 0) throw new Error('B40-C4 preregistration is not an ancestor');
const toolFreezeCommit = probe('git', ['rev-parse', 'HEAD'], 'tool freeze identity');
if (probe('git', ['status', '--porcelain', '--untracked-files=no'], 'tracked status') !== '') throw new Error('B40-C4 tracked worktree must be clean');
if (process.version !== baseSpec.runtime.nodeVersion || process.execPath !== baseSpec.runtime.nodeBinary) throw new Error('B40-C4 Node identity differs');
const nodeBinarySha256 = await sha256File(process.execPath);
if (nodeBinarySha256 !== baseSpec.runtime.nodeBinarySha256) throw new Error('B40-C4 Node SHA differs');
const ancestryFiles = {
...
if (JSON.stringify(ancestry) !== JSON.stringify(baseSpec.ancestry)) throw new Error('B40-C4 ancestry differs');
...
const evidence = {
  schemaVersion: 'bfs.workerHostCapacityEvidence.v0.5', experimentId: 'B40-C4',
  status: 'REPLAY_STABLE_READ_ONLY_CAPACITY_ADMISSION_NO_RUNTIME_OPERATION',
  preregistration: { commit: B40_C4_PREREG_COMMIT, specSha256: B40_C4_SPEC_SHA256 },
  correctionChain: {
  ...
  projectionIdentityCorrection: {
    c3SpecSha256: c4Spec.c3Protocol.specSha256,
    c3ToolFreezeCommit: c4Spec.c3Protocol.toolFreezeCommit,
  ...
  tools: {
    runner: { uri: 'scripts/run-b40-c4-projection-identity.mjs', sha256: await sha256File(runnerPath) },
    library: { uri: 'scripts/lib/b40-c4-projection-identity.mjs', sha256: await sha256File(libraryPath) },
    audit: { uri: 'scripts/audit-b40-c4-projection-identity.mjs', sha256: await sha256File(auditPath) },
  },
...
const analysis = await analyzeB40C4Evidence(evidence, c4Spec, c2Spec, baseSpec);
const attacks = analysis.passed ? await runB40C4Attacks(evidence, c4Spec, c2Spec, baseSpec) : [];
const roundTripEvidence = roundTripB40C2(evidence);
const roundTripAnalysis = await analyzeB40C4Evidence(roundTripEvidence, c4Spec, c2Spec, baseSpec);
const roundTripAttacks = roundTripAnalysis.passed ? await runB40C4Attacks(roundTripEvidence, c4Spec, c2Spec, baseSpec) : [];
...
if (!replayPassed) { evidence.serializationPassed = false; evidence.evidenceHash = hashB40Evidence(evidence); }
const result = { ...evidence, analysis, attacks, attacksPassed, replayPassed, verdict: analysis.passed && attacksPassed && replayPassed ? c4Spec.acceptedVerdict : 'PROJECTION_IDENTITY_CORRECTION_FAILED', nonClaims: c4Spec.nonClaims };
await mkdir(root, { recursive: true });
await writeFile(resolve(root, 'results.json'), `${JSON.stringify(result, null, 2)}\n`);
process.stdout.write(`BFS_B40_C4_RESULT verdict=${result.verdict} blockers=${result.decision.reasons.join(',')} attacks=${attacks.filter(a => a.passed).length}/14 replay=${replayPassed ? 'PASS' : 'FAIL'} runtimeOps=0\n`);
