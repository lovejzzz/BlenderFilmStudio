#!/usr/bin/env python3
"""Scalar Python structural-validity and adaptive-risk consumer for B52-D12.9-H1."""
...
def exact_scaled(value: float, scale: int, label: str) -> int:
    ...
    integer = int(scaled)
    if scaled != integer:
        raise RuntimeError(f"D12.9-H1 non-canonical {label}: {value!r}")
    return integer
...
def main() -> None:
    ...
    if sha_file(args.spec) != SPEC_SHA256 or sha_file(Path(sys.executable)) != spec["runtime"]["python"]["sha256"] or np.__version__ != spec["runtime"]["python"]["numpy"]:
        raise RuntimeError("D12.9-H1 Python/spec identity mismatch")
    ...
    if fixture is None or args.output_dir.exists() or args.report.exists():
        raise RuntimeError("D12.9-H1 consumer fixture or output invalid")
    ...
    if adapter.get("reportHash") != canonical_hash(body_without_hash) or adapter.get("fixtureId") != args.fixture or adapter.get("repeat") != args.repeat:
        raise RuntimeError("D12.9-H1 adapter identity mismatch")
    width, height = fixture["resolution"]
    arrays = {}
    for name, (filename, channels) in INPUTS.items():
        ...
        if sha_bytes(payload) != adapter["arrays"][name]["sha256"]:
            raise RuntimeError(f"D12.9-H1 input hash mismatch: {name}")
    ...
    body = {
        "schemaVersion": "bfs.blenderMotionAwareCurvatureRiskConsumerReport.v0.1",
    ...
    print(f"BFS_B52_D129_CONSUMER_PY_OK fixture={args.fixture} repeat={args.repeat}")
