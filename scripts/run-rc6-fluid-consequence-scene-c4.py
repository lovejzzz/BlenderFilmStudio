#!/usr/bin/env python3
"""C4 scene adapter with exact World, launch and liquid mesh-cache compatibility corrections."""
...
if hashlib.sha256(BASE.read_bytes()).hexdigest() != EXPECTED_BASE_SHA256:
    raise RuntimeError("RC6 C4 base scene tool identity mismatch")
...
velocity_after = '''    # Two frames define only the launch initial condition; Bullet owns every later outcome.
    ...
    launch_action = ball.animation_data.action
...
cache_before = '    settings.cache_mesh_format = "OPENVDB"'
cache_after = '    settings.cache_mesh_format = "BOBJECT"'
for before, after, label in (
    (world_before, world_after, "World"),
    (velocity_before, velocity_after, "launch"),
    (cache_before, cache_after, "mesh cache"),
):
    if source.count(before) != 1:
        raise RuntimeError(f"RC6 C4 {label} correction target is not unique")
    source = source.replace(before, after)
exec(compile(source, str(BASE) + "#C4_COMPATIBILITY", "exec"), globals(), globals())
