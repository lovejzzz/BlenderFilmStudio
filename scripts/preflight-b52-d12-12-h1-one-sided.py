#!/usr/bin/env python3
"""Zero-render Blender 5.2 preflight for B52-D12.12-H1."""
...
def main():
    ...
    if sha_file(cli.spec) != SPEC_SHA256:
        raise RuntimeError("D12.12-H1 preflight spec identity mismatch")
    ...
    if cli.root.as_posix() != spec["freshness"]["preflightRoot"] or cli.root.exists() or Path(spec["freshness"]["formalRoot"]).exists():
        raise RuntimeError("D12.12-H1 preflight/formal freshness violation")
    ...
    syntax_commands = [
        ...
        [spec["runtime"]["node"]["executable"], "--check", "scripts/reconstruct-b52-d12-12-h1-one-sided.mjs"],
    ]
    ...
    for fixture in spec["fixtures"]:
        ...
        command = [
            ...
            "--python", "blender/render_b52_d12_12_h1_one_sided_source.py", "--",
    ...
    python_source = Path("scripts/reconstruct-b52-d12-12-h1-one-sided.py").read_text()
    node_source = Path("scripts/reconstruct-b52-d12-12-h1-one-sided.mjs").read_text()
    ...
    body = {
        "schemaVersion": "bfs.blenderMaterialOwnerOneSidedCurvatureHoldoutPreflight.v0.1",
    ...
    receipt_body = {
        "schemaVersion": "bfs.blenderMaterialOwnerOneSidedCurvatureHoldoutPreflightReceipt.v0.1",
        "experimentId": spec["experimentId"], "specSha256": SPEC_SHA256,
    ...
    print(f"BFS_B52_D1212H1_PREFLIGHT passed={passed} checks={sum(checks.values())}/{len(checks)} probes={len(probe_reports)}")
    if not passed:
        raise SystemExit(1)
