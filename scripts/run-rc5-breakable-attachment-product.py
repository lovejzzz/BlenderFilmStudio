...
def build():
    ...
    checks = {
        ...
        "secondaryCameraReadability": all(row["source"] == "BOUNDED_PROJECTED_SECONDARY_READABILITY" and row["candidateCount"] == 8 and row["candidateCameraObjectCount"] == 1 and row["candidateCameraObjectDeletions"] == 0 and row["allSemanticCentersInFrame"] is True and row["minimumSecondaryCenterSeparationNormalized"] >= 0.06 and row["losingCandidateCameraObjectsRetained"] == 0 for row in (contact_readability, effect_readability)),
