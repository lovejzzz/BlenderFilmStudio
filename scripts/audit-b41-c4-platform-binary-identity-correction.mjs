...
import { analyzeB41C4Evidence, readB41C4Spec } from './lib/b41-c4-platform-binary-identity-correction.mjs';

const root = resolve(repositoryRoot, 'experiments/linux-amd64-blender-runtime-canary-v0-5');
const result = JSON.parse(await readFile(resolve(root, 'results.json'), 'utf8'));
const [baseSpec, c1Spec, c2Spec, c3Spec, c4Spec] = await Promise.all([readB41Spec(), readB41C1Spec(), readB41C2Spec(), readB41C3Spec(), readB41C4Spec()]);
const analysis = analyzeB41C4Evidence(result, c4Spec, c3Spec, c2Spec, c1Spec, baseSpec);
...
const observedArtifacts = result.success?.artifacts
  ? { png: await pngInfo(resolve(root, 'success/canary.png')), blend: await fileInfo(resolve(root, 'success/canary.blend')) }
  : {};
...
const audit = { schemaVersion: 'bfs.linuxAmd64BlenderRuntimeIndependentAudit.v0.5', experimentId: 'B41-C4', analysis, toolsMatch, observedArtifacts, artifactsMatch, timeoutNonPromotable, passed: analysis.passed && toolsMatch && artifactsMatch && timeoutNonPromotable };
await writeFile(resolve(root, 'audit.json'), `${JSON.stringify(audit, null, 2)}\n`);
process.stdout.write(`BFS_B41_C4_AUDIT ${audit.passed ? 'PASS' : 'FAIL'} tools=${toolsMatch ? 'MATCH' : 'MISMATCH'} artifacts=${artifactsMatch ? 'MATCH' : 'MISMATCH'} timeoutPromotable=${result.timeout?.promotable}\n`);
