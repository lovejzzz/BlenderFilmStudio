#!/usr/bin/env python3
"""Run one 24-frame moving-liquid test with only particle_min at 12."""
...
def transformed_source():
    if hashlib.sha256(BASE.read_bytes()).hexdigest() != EXPECTED_BASE_SHA256:
        raise RuntimeError("moving-liquid particle-minimum runner base identity mismatch")
    ...
    replacements = [
        ('"""Run one 24-frame moving-liquid test with only effector distance at 2.0."""', '"""Run one 24-frame moving-liquid test with only particle_min at 12."""', "docstring"),
        ...
        ('"scripts/audit-rc6-moving-liquid-effector-distance.py"', '"scripts/audit-rc6-moving-liquid-particle-minimum.py"', "auditor"),
        ...
        ('"RC6_MOVING_LIQUID_EFFECTOR_DISTANCE="', '"RC6_MOVING_LIQUID_PARTICLE_MINIMUM="', "process marker"),
        ('"RC6_MOVING_LIQUID_EFFECTOR_DISTANCE_RUN="', '"RC6_MOVING_LIQUID_PARTICLE_MINIMUM_RUN="', "runner marker"),
        ('"bfs.rc6MovingLiquidEffectorDistanceAdmission.v0.1"', '"bfs.rc6MovingLiquidParticleMinimumAdmission.v0.1"', "admission schema"),
        ('"bfs.rc6MovingLiquidEffectorDistanceFailure.v0.1"', '"bfs.rc6MovingLiquidParticleMinimumFailure.v0.1"', "failure schema"),
        ('"bfs.rc6MovingLiquidEffectorDistanceReceipt.v0.1"', '"bfs.rc6MovingLiquidParticleMinimumReceipt.v0.1"', "receipt schema"),
        ('"PASS_MOVING_LIQUID_EFFECTOR_DISTANCE"', '"PASS_MOVING_LIQUID_PARTICLE_MINIMUM"', "pass verdict"),
        ('"FAIL_MOVING_LIQUID_EFFECTOR_DISTANCE"', '"FAIL_MOVING_LIQUID_PARTICLE_MINIMUM"', "fail verdict"),
    ...
    for before, after, label in replacements:
        ...
        if source.count(before) != expected:
            raise RuntimeError(f"moving-liquid particle-minimum runner {label} target mismatch")
