...
export function validateB34Response({ spec, specSha, manifest, sealed, response }) {
  if (response.documentType !== 'BFS_B34_BLINDED_RESPONSE' || response.version !== spec.version) return fail('RESPONSE_TYPE');
  ...
  const sealedSession = sealed.sessions.find(item => item.sessionId === response.sessionId);
  if (!publicSession || !sealedSession) return fail('SESSION_ID');
  if (response.mappingCommitment !== publicSession.mappingCommitment || response.mappingCommitment !== sealedSession.commitment) return fail('SESSION_COMMITMENT');
  if (sha256Canonical({ sessionId: sealedSession.sessionId, salt: sealedSession.salt, mapping: sealedSession.mapping }) !== sealedSession.commitment) return fail('SEALED_MAPPING');
  ...
  return { valid: true, reason: 'OK', publicSession, sealedSession };
}
...
export function analyzeB34Responses({ spec, specSha, manifest, sealed, responses }) {
  const validations = responses.map(response => validateB34Response({ spec, specSha, manifest, sealed, response }));
  ...
  const permutationCounts = Object.fromEntries(spec.blinding.sixOrderPermutations.map(label => [label, observations.filter(item => item.permutation === label).length]));
