...
const PREREGISTRATION_COMMIT = '0b69ca8';
const EXPECTED_ROOTS = {
  outputRoot: 'experiments/cinematic-render-repro-cost-preflight-v0-4',
  attemptRoot: 'experiments/cinematic-render-repro-cost-attempt-v0-4',
  formalRoot: 'experiments/cinematic-render-repro-cost-v0-4',
};
const TOOL_PATHS = [
  CONTRACT_URI,
...
async function validateCorrection(parsed) {
  const path = await resolveExistingRepositoryPath(CORRECTION_URI, 'B61 C3 correction');
  ...
  if (correction.status !== 'PREREGISTERED' || correction.authorizedRetryRoots.preflight !== parsed.outputRoot
    || correction.authorizedRetryRoots.attempt !== parsed.attemptRoot || correction.authorizedRetryRoots.formal !== parsed.formalRoot) throw new Error('B61 C3 retry-root binding mismatch');
  ...
  if (!isDeepStrictEqual(attemptTree, correction.failedFormalRun.attemptTree) || !isDeepStrictEqual(formalTree, correction.failedFormalRun.formalTree)) throw new Error('B61 v0.3 retained failure tree mismatch');
  const summaryPath = await resolveExistingRepositoryPath(correction.failedFormalRun.failureSummary.uri, 'B61 v0.3 failure summary');
  ...
  if (await sha256File(summaryPath) !== correction.failedFormalRun.failureSummary.sha256 || !validSelfHash(summary, 'failureHash')
    || summary.failureHash !== correction.failedFormalRun.failureSummary.failureHash || summary.rootCauseProven !== true) throw new Error('B61 v0.3 failure-summary binding mismatch');
  const resultPath = await resolveExistingRepositoryPath(correction.diagnosticEvidence.result.uri, 'B61 D4 result');
  const result = JSON.parse(await readFile(resultPath, 'utf8'));
  const receiptPath = await resolveExistingRepositoryPath(correction.diagnosticEvidence.receipt.uri, 'B61 D4 receipt');
  const receipt = JSON.parse(await readFile(receiptPath, 'utf8'));
  if (await sha256File(resultPath) !== correction.diagnosticEvidence.result.sha256 || !validSelfHash(result, 'resultHash')
    ...
    || receipt.receiptHash !== correction.diagnosticEvidence.receipt.receiptHash || receipt.status !== 'PASS') throw new Error('B61 D4 evidence mismatch');
  return { uri: CORRECTION_URI, sha256: await sha256File(path), attemptTree, formalTree, failureHash: summary.failureHash, pngDiagnosticResultHash: result.resultHash, pngDiagnosticReceiptHash: receipt.receiptHash };
}
