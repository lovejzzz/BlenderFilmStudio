#!/usr/bin/env python3
"""Run the frozen 110-process B52-D12.12-H1 formal matrix."""
...
def spawn(command: list[str], category: str, environment: dict[str, str]) -> dict:
    ...
    if process.returncode != 0:
        raise RuntimeError(f"D12.12-H1 child failed: {category} pid={process.pid}\n{stdout}\n{stderr}")
    return row
...
def main():
    ...
    if sha_file(cli.spec) != SPEC_SHA256:
        raise RuntimeError("D12.12-H1 spec identity mismatch")
    spec = json.loads(cli.spec.read_text())
    if cli.root.as_posix() != spec["freshness"]["formalRoot"] or cli.root.exists():
        raise RuntimeError("D12.12-H1 formal root must be exact and fresh")
    ...
    if not self_ok(preflight, "receiptHash") or not preflight.get("passed") or preflight.get("specSha256") != SPEC_SHA256:
        raise RuntimeError("D12.12-H1 passing preflight receipt required")
    tool_hashes = {uri: sha_file(Path(uri)) for uri in spec["freshness"]["newFormalToolPaths"]}
    if preflight.get("toolHashes") != tool_hashes:
        raise RuntimeError("D12.12-H1 preflight/tool freeze mismatch")
    ...
    if parent_trees_before != {
        ...
        raise RuntimeError("D12.12-H1 parent formal tree mismatch")
    ...
    if free_bytes < required_before:
        raise RuntimeError(f"D12.12-H1 disk reserve gate failed: {free_bytes} < {required_before}")
    ...
    source_tool = "blender/render_b52_d12_12_h1_one_sided_source.py"
    adapter_tool = "scripts/adapt-b52-d12-12-h1-one-sided-source.py"
    python_consumer = "scripts/reconstruct-b52-d12-12-h1-one-sided.py"
    node_consumer = "scripts/reconstruct-b52-d12-12-h1-one-sided.mjs"
    ...
    analyzer_command = [
        python, "scripts/analyze-b52-d12-12-h1-one-sided.py", "--spec", str(cli.spec), "--root", str(cli.root),
    ...
    if len(children) != 109:
        raise RuntimeError(f"D12.12-H1 pre-audit process roster mismatch: {len(children)}")

    execution_plan_path = cli.root / "execution-plan.json"
    audit_path = cli.root / "audit.json"
    audit_command = [
        python, "scripts/audit-b52-d12-12-h1-one-sided.py", "--spec", str(cli.spec), "--root", str(cli.root),
    ...
    plan_body = {
        "schemaVersion": "bfs.blenderMaterialOwnerOneSidedCurvatureHoldoutExecutionPlan.v0.1",
    ...
    execution_body = {
        "schemaVersion": "bfs.blenderMaterialOwnerOneSidedCurvatureHoldoutExecution.v0.1",
        "experimentId": spec["experimentId"], "specSha256": SPEC_SHA256,
    ...
    receipt_body = {
        "schemaVersion": "bfs.blenderMaterialOwnerOneSidedCurvatureHoldoutReceipt.v0.1",
        "experimentId": spec["experimentId"], "specSha256": SPEC_SHA256,
    ...
    print(f"BFS_B52_D1212H1_RUN receipt={receipt['receiptHash']} audit={receipt['auditVerdict']} verdict={receipt['scientificVerdict']} processes={len(children)}")
    if not final_valid:
        raise SystemExit(1)
