#!/usr/bin/env python3
"""Single-use formal runner for B52-D12.9-H1."""
...
def main():
    ...
    if sha_file(spec_path) != SPEC_SHA256 or preflight.get("preflightHash") != canon({key: value for key, value in preflight.items() if key != "preflightHash"}) or preflight.get("status") != "ACCEPTED":
        raise RuntimeError("D12.9-H1 identity/admission mismatch")
    if root.exists() or root != (repo / spec["diskAdmission"]["formalRoot"]).resolve():
        raise RuntimeError("refusing to reuse or redirect D12.9-H1 formal root")
    ...
    if tool_hashes != preflight["toolHashes"]:
        raise RuntimeError("D12.9-H1 tools differ from preflight")
    ...
    if available - projected < reserve:
        raise RuntimeError("D12.9-H1 disk admission rejected")
    ...
    def child(role: str, cell: str, argv: list[str], env: dict[str, str] | None = None):
        ...
        children.append(row)
        print(f"BFS_D129_CHILD role={role} cell={cell} pid={process.pid} exit={process.returncode}", flush=True)
        if process.returncode != 0:
            failure = {
                "schemaVersion": "bfs.blenderMotionAwareCurvatureRiskHoldoutFailure.v0.1",
            ...
            raise RuntimeError(f"D12.9-H1 child failed: {role} {cell}")

    blender = spec["runtime"]["blender"]["executable"]
    python = spec["runtime"]["python"]["executable"]
    node = spec["runtime"]["node"]["executable"]
    source = str(repo / "blender/render_b52_d12_9_motion_aware_source.py")
    adapter = str(repo / "scripts/adapt-b52-d12-9-motion-aware-source.py")
    py_consumer = str(repo / "scripts/reconstruct-b52-d12-9-motion-aware.py")
    node_consumer = str(repo / "scripts/reconstruct-b52-d12-9-motion-aware.mjs")
    typed_spec = str((repo / spec["parents"]["typedEnvelopeSpec"]["uri"]).resolve())
    typed_py = str((repo / spec["parents"]["typedEnvelopePython"]["uri"]).resolve())
    ...
    execution = {
        "schemaVersion": "bfs.blenderMotionAwareCurvatureRiskHoldoutExecution.v0.1",
        "experimentId": spec["experimentId"],
    ...
    child("ANALYZER", "FORMAL", [python, str(repo / "scripts/analyze-b52-d12-9-motion-aware.py"), "--spec", str(spec_path), "--root", str(root), "--preflight", str(preflight_path), "--execution", str(execution_path), "--output", str(root / "results.json")])
    result = json.loads((root / "results.json").read_text())
    child("AUDIT", "FORMAL", [python, str(repo / "scripts/audit-b52-d12-9-motion-aware.py"), "--spec", str(spec_path), "--root", str(root), "--result", str(root / "results.json"), "--execution", str(execution_path), "--output", str(root / "audit.json")])
    audit = json.loads((root / "audit.json").read_text())
    pids = [row["pid"] for row in children]
    expected = spec["processMatrix"]["totalUniqueChildProcessesIncludingAudit"]
    process_ok = len(children) == expected and len(set(pids)) == expected and all(row["exitCode"] == 0 for row in children)
    body = {
        "schemaVersion": "bfs.blenderMotionAwareCurvatureRiskHoldoutReceipt.v0.1",
        "experimentId": spec["experimentId"],
    ...
    if not process_ok or not audit["passed"]:
        raise RuntimeError("D12.9-H1 process/audit totality failure")
    print(f"BFS_B52_D129_FORMAL_COMPLETE verdict={result['verdict']} checks={result['checkPassed']}/{result['checkTotal']} attacks={result['mutationAttackPassed']}/{result['mutationAttackTotal']} audit={audit['checkPassed']}/{audit['checkTotal']} receipt={receipt['receiptHash']}")
