...
if (selfTestRequested) {
  ...
  const checks = [
    ...
    { id: 'CONTRACT_SCHEMA', pass: ['bfs.pb1ValidationOnlyC3AuthorizationRequest.v0.8', 'bfs.pb1ValidationOnlyC3Execution.v0.9'].includes(spec.schemaVersion) },
    { id: 'ENGINE_SYMLINK_BEFORE_CHECKOUT', pass: readFileSync(RUNNER_PATH, 'utf8').indexOf("symlinkSync(retainedObjects, localObjects, 'dir')") > 0 && readFileSync(RUNNER_PATH, 'utf8').indexOf("symlinkSync(retainedObjects, localObjects, 'dir')") < readFileSync(RUNNER_PATH, 'utf8').indexOf("'checkout exact publication head after objects link'") },
    { id: 'DEPENDENCY_SYMLINK_BEFORE_CHECKOUT', pass: readFileSync(RUNNER_PATH, 'utf8').indexOf("symlinkSync(retainedDependencyObjects, localDependencyObjects, 'dir')") > 0 && readFileSync(RUNNER_PATH, 'utf8').indexOf("symlinkSync(retainedDependencyObjects, localDependencyObjects, 'dir')") < readFileSync(RUNNER_PATH, 'utf8').indexOf("'checkout exact dependency after objects link'") },
  ...
  const output = { schemaVersion: 'bfs.pb1ValidationOnlyC3AuditorSelfTest.v0.9', status: failed.length ? 'FAIL' : 'PASS', checksPassed: checks.length - failed.length, checksTotal: checks.length, checks, failures: failed.map(item => item.id) };
...
add(checks, 'AUTHORIZATION_GRANTED_AND_BOUND', spec.authorization?.granted === true && spec.authorization?.exactTextZhCN === JSON.parse(readFileSync(resolve(REPOSITORY_ROOT, REQUEST_RELATIVE), 'utf8')).exactStandingAuthorizationTextZhCN);
...
add(checks, 'NEGATIVE_CONTROLS_10_OF_10', receipts['negative-controls.json'].checksPassed === 10 && receipts['negative-controls.json'].controls.every(item => item.pass));
...
add(checks, 'DEPENDENCY_OBJECTS_IS_EXACT_SYMLINK', lstatSync(dependencyObjects).isSymbolicLink() && command('/usr/bin/readlink', [dependencyObjects]) === spec.dependencyLfsCorrection.retainedObjectsSubtree);
const dependencyLfs = lfsInventory(dependency);
add(checks, 'DEPENDENCY_LFS_COUNT_BYTES_EXACT', dependencyLfs.count === spec.dependencyLfsCorrection.trackedPaths && dependencyLfs.bytes === spec.dependencyLfsCorrection.materializedContentBytes, { count: dependencyLfs.count, bytes: dependencyLfs.bytes }, { count: spec.dependencyLfsCorrection.trackedPaths, bytes: spec.dependencyLfsCorrection.materializedContentBytes });
add(checks, 'DEPENDENCY_LFS_HASHES_EXACT', dependencyLfs.mismatches.length === 0, dependencyLfs.mismatches, []);
add(checks, 'DEPENDENCY_LFS_MANIFEST_BOUND', dependencyLfs.manifestSha256 === dependencyReceipt.dependency.lfs.manifestSha256);
const retainedDependencyObjectsNow = treeIdentity(spec.dependencyLfsCorrection.retainedObjectsSubtree);
add(checks, 'RETAINED_DEPENDENCY_OBJECTS_UNCHANGED', canonicalJson(retainedDependencyObjectsNow) === canonicalJson(dependencyReceipt.dependency.retainedObjectsAfter) && retainedDependencyObjectsNow.files === spec.dependencyLfsCorrection.retainedObjectFiles && retainedDependencyObjectsNow.bytes === spec.dependencyLfsCorrection.retainedObjectBytes && retainedDependencyObjectsNow.manifestSha256 === spec.dependencyLfsCorrection.retainedObjectManifestSha256);
...
add(checks, 'AUTHORIZED_COUNTS_EXACT', counters.localEngineClones === 1 && counters.freshObjectsSymlinks === 1 && counters.localLfsMaterializations === 1 && counters.localDependencyClones === 1 && counters.freshDependencyObjectsSymlinks === 1 && counters.localDependencyLfsMaterializations === 1 && counters.nativeBuilds === 1 && counters.productStarts === 2);
...
const audit = writeJsonExclusive(auditPath, {
  schemaVersion: 'bfs.pb1ValidationOnlyC3IndependentAudit.v0.9',
  gate: 'PB.1',
  mode: 'VALIDATION_ONLY_C3',
  observedAt: new Date().toISOString(),
  status: failed.length ? 'FAIL' : 'PASS',
  auditor: { path: 'scripts/audit-ai-native-studio-pb1-validation-only-c3.mjs', sha256: sha256File(AUDITOR_PATH), importsRunner: false },
