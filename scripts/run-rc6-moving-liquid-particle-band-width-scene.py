#!/usr/bin/env python3
"""Change only particle narrow-band width from 3.0 to 4.0 on attempt-68 physics."""
...
def transformed_source():
    ...
    replacements = [
        ...
        (
            ...
            "abs(settings.particle_radius - 1.8) <= 1e-6 and settings.particle_number == 2 and settings.particle_min == 8 and settings.particle_max == 16 and abs(settings.particle_band_width - 4.0) <= 1e-6 and settings.use_fractions and abs(settings.fractions_threshold - 0.05) <= 1e-6 and abs(settings.fractions_distance - 0.25) <= 1e-6 and abs(settings.mesh_particle_radius - 2.5) <= 1e-6 and abs(effector.surface_distance - 2.0) <= 1e-6 and effector.subframes == 1 and settings.timesteps_min == 2 and settings.timesteps_max == 4 and abs(settings.cfl_condition - 2.0) <= 1e-6,",
            "configuration check",
        ),
        ("bfs.rc6MovingLiquidPreviewResult.v0.1", "bfs.rc6MovingLiquidParticleBandWidthResult.v0.1", "schema"),
        ("RC6_MOVING_LIQUID_PREVIEW=", "RC6_MOVING_LIQUID_PARTICLE_BAND_WIDTH=", "marker"),
        (
            ...
            "One 24-frame Preview-96 single-variable particle-band-width test on exact C5F96 motion with the attempt-68 baseline; no full tip, spill, impact, persistence, render or film-quality claim.",
            "claim ceiling",
        ),
    ]
    for before, after, label in replacements:
        if source.count(before) != 1:
            raise RuntimeError(f"moving-liquid particle-band-width {label} target mismatch")
