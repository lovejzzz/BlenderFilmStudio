...
const resultPath = resolve(workRoot, 'private-result.json');
...
const [spec, result, manifest, ledger, composite, sealed, b34Package, b34Composite] = await Promise.all([specPath, resultPath, manifestPath, ledgerPath, compositePath, sealedPath, b34PackagePath, b34CompositePath].map(json));
...
requireValue(result.packageStatus === 'PRIVATE_CARRIER_AND_INTERFACE_READY' && result.disclosureStatus === 'DELAYED_UNTIL_COLLECTION_CLOSE' && result.validPackage === true && result.humanReview.status === 'HUMAN_REVIEW_PENDING' && result.humanReview.formalResponseCount === 0, 'result classification mismatch');
requireValue(await sha256File(ledgerPath) === result.identities.sourceProcessLedgerSha256 && await sha256File(compositePath) === result.identities.compositeDisplayManifestSha256 && await sha256File(manifestPath) === result.identities.privatePackageManifestSha256, 'top-level artifact binding mismatch');
...
for (const cell of cells) {
  ...
  requireValue(cellManifest.visualRealization.originalLensMm === 50 && cellManifest.visualRealization.intervenedLensMm === 52 && cellManifest.visualRealization.sourceBlendSaved === false, `${cell} realization mismatch`);
...
for (const method of methods) {
  ...
  for (const item of record.outputs) {
    ...
    requireValue(item.displaySha256 !== b34Composite.methods[method].outputs[item.frame - 1].displaySha256, `${method} display reused B34 frame ${item.frame}`);
...
for (const carrier of manifest.carriers) {
  ...
  requireValue(carrier.sha256 !== b34Package.carriers.find(item => item.method === carrier.method).sha256, `${carrier.method} carrier reused B34 identity`);
...
const audit = {
  documentType: 'BFS_B35_INDEPENDENT_PRIVATE_PACKAGE_AUDIT', version: spec.version,
  auditedAtUtc: new Date().toISOString(), status: 'PRIVATE_PACKAGE_AUDIT_PASS',
  ...
  observations: { uniqueProcessIds: 13, sourceExrFiles: sourceFiles, sourceBytes, compositeExrFiles: compositeFiles, displayPngFiles: displayFiles, displayFramesDistinctFromB34: displayFiles, carrierFiles: 3, carriersDistinctFromB34: 3, exactRoundtripFrames: roundtripFrames, changedRgbPixels: changedPixels, observerSessions: 18, permutationCounts, sealedMappingNotInObserverPackages: true, formalHumanResponses: 0 },
...
process.stdout.write(`BFS_B35_INDEPENDENT_PACKAGE_AUDIT_PASS source=${sourceFiles} composites=${compositeFiles} display=${displayFiles} roundtrip=${roundtripFrames} sessions=18 human=0\n`);
