#!/usr/bin/env python3
"""C1 runner: fresh attempt-26 with post-assembly final scene correction."""
...
if hashlib.sha256(BASE.read_bytes()).hexdigest() != EXPECTED_BASE_SHA256:
    raise RuntimeError("RC6 source-clearance final C1 runner base identity mismatch")
...
replacements = (
    ...
    ("scripts/audit-rc6-liquid-source-clearance-final.py", "scripts/audit-rc6-liquid-source-clearance-final-c1.py", 1, "audit tool"),
...
for before, after, expected, label in replacements:
    if source.count(before) != expected:
        raise RuntimeError(f"RC6 source-clearance final C1 runner {label} target mismatch")
    source = source.replace(before, after)
exec(compile(source, str(BASE) + "#SOURCE_CLEARANCE_FINAL_C1_RUNNER_V01", "exec"), globals(), globals())
