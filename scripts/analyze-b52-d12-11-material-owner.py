#!/usr/bin/env python3
"""Independent scalar analyzer for B52-D12.11-I1 Material Index intervention."""
...
INPUTS = {
    ...
    "previousObjectIndex": ("previous-object-index.f32", 1, "<f4"),
    "currentObjectIndex": ("current-object-index.f32", 1, "<f4"),
...
def native(path: Path) -> dict:
    ...
    if not self_ok(value, "reportHash"):
        raise RuntimeError(f"D12.11-I1 report self-hash mismatch: {path}")
    return value
...
def load_array(path: Path, shape: tuple[int, ...], dtype: str):
    ...
    if len(payload) != math.prod(shape) * np.dtype(dtype).itemsize:
        raise RuntimeError(f"D12.11-I1 payload length mismatch: {path}")
...
def exact_scaled(value, factor, label):
    ...
    integer = int(scaled)
    if scaled != integer:
        raise RuntimeError(f"D12.11-I1 analyzer non-canonical {label}: {value!r}")
    return integer
...
def main():
    ...
    if args.output.exists():
        raise RuntimeError("refusing to overwrite D12.11-I1 result")
    intervention = json.loads(args.spec.read_text())
    h1_spec_path = Path(intervention["parents"]["h1Spec"]["uri"])
    if sha_file(args.spec) != SPEC_SHA256 or sha_file(h1_spec_path) != H1_SPEC_SHA256:
        raise RuntimeError("D12.11-I1 spec identity mismatch")
    h1_spec = json.loads(h1_spec_path.read_text())
    spec = json.loads(json.dumps(h1_spec))
    ...
    if sha_file(Path(sys.executable)) != h1_spec["runtime"]["python"]["sha256"] or not self_ok(preflight, "preflightHash") or preflight.get("status") != "ACCEPTED" or not self_ok(execution, "executionHash"):
        raise RuntimeError("D12.11-I1 formal identity mismatch")
    ...
    if tool_hashes != preflight["toolHashes"] or tool_hashes != execution["toolHashes"]:
        raise RuntimeError("D12.11-I1 tool identity mismatch")
    parent_checks = {name: sha_file(Path(row["uri"])) == row["sha256"] for name, row in intervention["parents"].items() if "uri" in row and "sha256" in row}
    if not all(parent_checks.values()):
        raise RuntimeError("D12.11-I1 parent identity mismatch")
    ...
    localization = json.loads(Path(intervention["parents"]["ownerLocalizationResult"]["uri"]).read_text())
    localization_by_cell = {row["cell"]: row for row in localization["cells"]}
    h1_result = json.loads(Path(intervention["parents"]["h1Result"]["uri"]).read_text())
    h1_measurement_by_cell = {row["cell"]: row for row in h1_result["measurements"]}
    ...
    for fixture in spec["fixtures"]:
        ...
        for repeat in (1, 2):
            ...
            for frame in (0, 1):
                ...
                expected_materials = [intervention["materialOwnerTokens"]["assignments"][owner["analyticOwnerId"]] for owner in fixture["owners"]]
                observed_owners = report.get("sceneStructure", {}).get("owners", [])
                cell_guards[f"source{frame}"] = (
                    report["output"]["sha256"] == sha_file(source_dir / f"frame-{frame}/source.exr")
                    and report["fixtureId"] == fixture_id
                    and report["repeat"] == repeat
                    and report.get("experimentId") == intervention["experimentId"]
                    and report.get("passState", {}).get("Material Index") is True
                    and [row.get("materialPassIndex") for row in observed_owners] == expected_materials
                    and [row.get("passIndex") for row in observed_owners] == [owner["passIndex"] for owner in next(row for row in h1_spec["fixtures"] if row["id"] == fixture_id)["owners"]]
                )
            ...
            h1_adapter_dir = h1_root / "adapters" / fixture_id / f"R{repeat}" / "arrays"
            paired_map = {
                ...
                "previousObjectIndex": "previous-owner.f32",
                "currentObjectIndex": "current-owner.f32",
            }
            cell_guards["pairedH1Payload"] = all(
                (adapter_dir / "arrays" / INPUTS[name][0]).read_bytes() == (h1_adapter_dir / h1_filename).read_bytes()
                for name, h1_filename in paired_map.items()
            )
            declared_tokens = {0.0, *(float(value) for value in intervention["materialOwnerTokens"]["assignments"].values())}
            cell_guards["materialTokenDomain"] = set(np.unique(arrays["previousOwner"]).astype(float)).issubset(declared_tokens) and set(np.unique(arrays["currentOwner"]).astype(float)).issubset(declared_tokens)
            ...
            h1_accepted_path = h1_root / "consumers" / "python" / fixture_id / f"R{repeat}" / "arrays" / "accepted.u8"
            h1_accepted = np.frombuffer(h1_accepted_path.read_bytes(), dtype="u1").reshape(height, width).astype(bool)
            true_owner_path = localization_root / "payloads" / fixture_id / f"R{repeat}" / "true-owner-bilinear.u8"
            true_owner_bilinear = np.frombuffer(true_owner_path.read_bytes(), dtype="u1").reshape(height, width).astype(bool)
            registered_alias = h1_accepted & ~true_owner_bilinear
            accepted_outside_true_owner = accepted & ~true_owner_bilinear
            accepted_on_registered_alias = accepted & registered_alias
            new_accepted = accepted & ~h1_accepted
            cell_guards["localizationCounts"] = int(registered_alias.sum()) == int(localization_by_cell[cell]["acceptedOutsideTrueOwnerBilinear"])
            cell_guards["newAcceptedSubset"] = not new_accepted.any()
            ...
            false_accept = int(accepted_outside_true_owner.sum())
            ...
            measurements.append({
                ...
                "pairedIntervention": {
                    "registeredH1AcceptedAliasPixels": int(registered_alias.sum()),
                    "acceptedOutsideTrueOwnerBilinearPixels": int(accepted_outside_true_owner.sum()),
                    "acceptedOnRegisteredAliasPixels": int(accepted_on_registered_alias.sum()),
                    "newAcceptedPixelsRelativeToH1": int(new_accepted.sum()),
    ...
    depth_ok = all(row["validHistoryDepthAgreement"]["sampleCount"] > 0 and row["validHistoryDepthAgreement"]["maximumRelative"] <= h1_spec["typedDepthDomains"]["validHistoryAgreement"]["maximum"] for row in measurements)
    ...
    stress_ok = True
    for row in primary:
        fixture = next(item for item in h1_spec["fixtures"] if item["id"] == row["fixtureId"])
        stress = fixture["requiredStress"]
        if stress:
            h1_row = h1_measurement_by_cell[row["cell"]]
            stress_ok = stress_ok and h1_row["reasonCounts"][stress["reason"]] >= stress["minimumPrimaryPixels"]
    same_index = next(row for row in primary if row["fixtureId"] == "SAME_INDEX_DEPTH_CROSSING_179X113")
    h1_same_index = h1_measurement_by_cell[same_index["cell"]]
    depth_rejection_ok = h1_same_index["expectedDepthRejectionPixels"] >= h1_spec["typedDepthDomains"]["expectedDepthRejection"]["minimumPrimaryPixels"] and h1_same_index["reasonCounts"]["INVALID_DEPTH"] >= gates["disocclusion"]["minimumOwnerOnlyWrongAcceptsInSameIndexPrimary"]
    curvature_stress_ok = h1_same_index["riskRejectedPixels"] >= gates["stress"]["minimumCurvatureRiskRejectedPixelsInSameIndexPrimary"]
    ...
    alias_ok = all(row["pairedIntervention"]["acceptedOutsideTrueOwnerBilinearPixels"] == 0 and row["pairedIntervention"]["acceptedOnRegisteredAliasPixels"] == 0 for row in measurements)
    critical_alias_roster_ok = all(
        row["pairedIntervention"]["registeredH1AcceptedAliasPixels"] == intervention["primaryEndpoints"]["registeredBaseline"]["perRepeatAcceptedOutsideTrueOwnerBilinearPixels"]
        and localization_by_cell[row["cell"]]["objectIndexBilinearAlias"] == intervention["primaryEndpoints"]["registeredBaseline"]["perRepeatObjectIndexBilinearAliasPixels"]
        for row in measurements if row["fixtureId"] == intervention["primaryEndpoints"]["registeredBaseline"]["fixture"]
    )
    no_new_accept_ok = all(row["pairedIntervention"]["newAcceptedPixelsRelativeToH1"] == 0 for row in measurements)
    paired_payload_ok = all(row.get("pairedH1Payload", False) for row in guards.values() if "pairedH1Payload" in row)
    token_domain_ok = all(row.get("materialTokenDomain", False) for row in guards.values() if "materialTokenDomain" in row)
    checks = [
        ("PARENT_IDENTITY", all(parent_checks.values())),
        ("PREFLIGHT_TOOL_IDENTITY", True),
        ("PROCESS_TOTALITY_BEFORE_AUDIT", process_ok),
        ("PAIRED_H1_PAYLOAD_IDENTITY", paired_payload_ok),
        ("MATERIAL_TOKEN_DOMAIN", token_domain_ok),
        ...
        ("PRIMARY_ALIAS_ROSTER", critical_alias_roster_ok),
        ("PRIMARY_ALIAS_ELIMINATION", alias_ok),
        ("NO_NEW_ACCEPTED_COORDINATES", no_new_accept_ok),
    ...
    hard_without_alias_pass = all(check_map[name] for name in hard_without_alias)
    all_pass = all(value for _, value in checks)
    decision = intervention["decision"]
    verdict = decision["supportedVerdict"] if all_pass else decision["boundedVerdict"] if hard_pass else decision["aliasFailureVerdict"] if hard_without_alias_pass else decision["rejectedVerdict"]
    targets = [f"{cell}:{name}" for cell, row in guards.items() for name in row] + [name for name, _ in checks] + [f"measurement:{index}:{key}" for index, row in enumerate(measurements) for key in ("falseAcceptedInvalidHistoryPixels", "riskUnderboundRgbSamples", "fallbackExact", "riskRejectedPixels", "pairedIntervention")]
    ...
    mutation = [{"id": f"M{index + 1:02d}", "target": target, "passed": canon({**base_projection, "mutationNonce": target}) != base_hash} for index, target in enumerate(targets[:max(56, intervention["attacks"]["minimumRegisteredAttacks"])])]
    body = {
        "schemaVersion": "bfs.blenderMaterialIndexOwnerIntegrationResult.v0.1",
        ...
        "nonClaims": intervention["nonClaims"],
    }
    result = {**body, "evidenceHash": canon(body)}
    args.output.parent.mkdir(parents=True, exist_ok=True)
    args.output.write_text(json.dumps(result, indent=2, sort_keys=True, allow_nan=False) + "\n")
    print(f"BFS_B52_D1211_ANALYSIS_OK verdict={verdict} checks={result['checkPassed']}/{result['checkTotal']} attacks={result['mutationAttackPassed']}/{result['mutationAttackTotal']}")
