...
def main() -> None:
    ...
    for fixture in spec["fixtures"]:
        ...
        for repeat in (1, 2):
            ...
            replay_rows.append({"fixtureId": fixture_id, "sourceRepeat": repeat, "adapterExact": adapter_exact, "quantizerExact": quantizer_checks[-1], "validPixels": int(validity.sum()), "resolvedSha256": hashlib.sha256(np.ascontiguousarray(resolved, dtype="<f4").tobytes()).hexdigest()})
    ...
    body = {"schemaVersion": "bfs.blenderNearestIntegerTemporalRecoveryAudit.v0.1", "experimentId": spec["experimentId"], "status": status, "auditorPid": os.getpid(), "spec": {"uri": str(args.spec), "sha256": sha(args.spec)}, "receipt": {"uri": str(receipt_path), "sha256": sha(receipt_path)}, "result": {"uri": str(result_path), "sha256": sha(result_path), "verdict": result["verdict"], "baseFailure": result["baseFailure"]}, "checks": checks, "replayedEvidence": evidence, "replay": replay_rows}
