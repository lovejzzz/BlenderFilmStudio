#!/usr/bin/env python3
"""Independent raw-payload/result audit for B52-D12.9-H1."""
...
def load(path: Path, shape, dtype):
    ...
    if len(payload) != math.prod(shape) * np.dtype(dtype).itemsize:
        raise RuntimeError(f"D12.9-H1 audit payload length mismatch: {path}")
...
def main():
    ...
    if args.output.exists():
        raise RuntimeError("refusing to overwrite D12.9-H1 audit")
    spec = json.loads(args.spec.read_text())
    result = json.loads(args.result.read_text())
    execution = json.loads(args.execution.read_text())
    if sha_file(args.spec) != SPEC_SHA256 or sha_file(Path(sys.executable)) != spec["runtime"]["python"]["sha256"]:
        raise RuntimeError("D12.9-H1 audit runtime identity mismatch")
    ...
    expected_verdict = decision["supportedVerdict"] if all_pass else decision["boundedVerdict"] if hard_pass else decision["rejectedVerdict"]
    ...
    attack_ok = len(attacks) >= spec["attacks"]["minimumRegisteredAttacks"] and len({row["id"] for row in attacks}) == len(attacks) and all(row.get("passed") is True for row in attacks) and result.get("mutationAttackPassed") == len(attacks) == result.get("mutationAttackTotal")
    ...
    body = {
        "schemaVersion": "bfs.blenderMotionAwareCurvatureRiskHoldoutAudit.v0.1",
    ...
    print(f"BFS_B52_D129_AUDIT_OK passed={audit['passed']} checks={audit['checkPassed']}/{audit['checkTotal']}")
