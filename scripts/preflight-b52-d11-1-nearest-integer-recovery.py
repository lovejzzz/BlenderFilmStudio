#!/usr/bin/env python3
"""Zero-formal-output frozen-tool admission for B52-D11.1."""
...
PREREGISTRATION_COMMIT = "08543cf17371966c9bb4963f8b78158906fe1f2f"
...
def main() -> None:
    ...
    if sha(args.spec) != SPEC_SHA256 or args.output.exists() or formal_root.exists():
        raise RuntimeError("B52-D11.1 preflight identity/output/formal-root mismatch")
    ...
    all_tools = len(tools) == 13 and all(item["match"] for item in tools.values())
    ...
    d11 = json.loads((root / spec["parents"]["d11Spec"]["uri"]).read_text())
    ...
    prior_names = d10_names | {item["name"] for fixture in d11["fixtures"] for item in fixture["objects"]}
    prior_ids = d10_ids | {int(item["passIndex"]) for fixture in d11["fixtures"] for item in fixture["objects"]}
    current_names = {item["name"] for fixture in spec["fixtures"] for item in fixture["objects"]}
    current_ids = {int(item["passIndex"]) for fixture in spec["fixtures"] for item in fixture["objects"]}
    freshness = all(
        (
            tuple(spec["scene"]["resolution"]) not in d9_resolutions,
            spec["scene"]["resolution"] != d10["sourceRender"]["resolution"],
            spec["scene"]["resolution"] != d11["sourceRender"]["resolution"],
            spec["scene"]["camera"]["orthoScale"] != d10["scene"]["camera"]["orthoScale"],
            spec["scene"]["camera"]["orthoScale"] != d11["scene"]["camera"]["orthoScale"],
            current_names.isdisjoint(prior_names),
            current_ids.isdisjoint(prior_ids),
            spec["scene"]["textureConstruction"] != d11["scene"]["textureConstruction"],
            spec["fixtures"] != d11["fixtures"],
        )
    )

    tests_argv = [spec["runtime"]["python"]["executable"], "-m", "unittest", "tests/test_b52_d11_1_nearest_integer_recovery_contract.py", "-v"]
    tests = subprocess.run(tests_argv, cwd=root, text=True, capture_output=True, check=False)
    test_ok = tests.returncode == 0 and "Ran 10 tests" in tests.stderr and "OK" in tests.stderr

    analyzer_source = (root / "scripts/analyze-b52-d11-1-nearest-integer-recovery.py").read_text()
    ...
    with tempfile.TemporaryDirectory(prefix="bfs-d11-1-preflight-") as temporary, tempfile.TemporaryDirectory(prefix="bfs-d11-1-config-") as config, tempfile.TemporaryDirectory(prefix="bfs-d11-1-scripts-") as scripts:
        ...
        probe_code = (
            ...
            "t=bpy.data.node_groups.new('BFS_D111_PREFLIGHT_TREE','CompositorNodeTree'); s.compositing_node_group=t; "
            "n=t.nodes.new('CompositorNodeImage'); n.name='BFS_D111_EXTERNAL_SOURCE'; t.interface.new_socket(name='Image',in_out='OUTPUT',socket_type='NodeSocketColor'); "
            "g=t.nodes.new('NodeGroupOutput'); g.name='BFS_D111_GROUP_OUTPUT'; t.links.new(n.outputs['Image'],g.inputs['Image']); "
    ...
    body = {
        "schemaVersion": "bfs.blenderNearestIntegerTemporalRecoveryFrozenToolPreflight.v0.1",
        ...
        "nonClaims": ["Preflight creates no D11.1 formal output.", "The Blender API probe performs zero renders.", "Development smoke artifacts cannot satisfy formal gates."],
    }
    ...
    print(f"BFS_B52_D11_1_PREFLIGHT_{body['status']} tools={len(tools)} tests={test_ok} probe={probe_ok} outputAbsent={body['formalRootAbsent']}")
    if not accepted:
        raise SystemExit(1)
