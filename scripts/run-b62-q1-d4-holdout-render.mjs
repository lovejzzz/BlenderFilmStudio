...
const SPEC_URI = "specs/b62-camera-quality-holdout-render-validation.v0.1.json",
  PROTOCOL_URI =
...
async function treeIdentity(uri) {
  ...
  async function walk(d) {
    for (const e of await readdir(d, { withFileTypes: true })) {
      ...
      else throw new Error(`special ${p}`);
    }
  }
  await walk(root);
  files.sort();
  ...
  for (const p of files) {
    ...
    bytes += c.length;
    material += `${relative(root, p).split("\\").join("/")}\0${hashBytes(c)}\n`;
  }
  return {
    files: files.length,
    bytes,
    treeSha256: hashBytes(Buffer.from(material)),
  };
}
function parse() {
...
async function processReceipt(root, id, command, args, result) {
  return writeHashed(
    resolve(root, "processes", `${id}.json`),
    {
...
async function child({ root, id, scene, tool, args, budget, maxFiles = 256 }) {
  ...
  const receipt = await processReceipt(
    ...
    command,
    full.map((v) =>
      v.startsWith(repositoryRoot)
        ? relative(repositoryRoot, v).split("\\").join("/")
        : v,
    ),
    result,
  );
...
async function main() {
  ...
  const correction = JSON.parse(
    await readFile(pathFor(CORRECTION_URI), "utf8"),
  );
  ...
  req(
    correction.correctionId === "B62-Q1-D4-C1" &&
      correction.statusBeforeToolChange === "PREREGISTERED" &&
      correction.retainedFailure.root === spec.output.formalRoot &&
      correction.authorizedChanges.retryRoot === RETAINED2_ROOT,
    "C1 correction",
  );
  req(
  ...
  const head = (await git(["rev-parse", "HEAD"])).trim(),
    origin = (await git(["rev-parse", "origin/main"])).trim();
  req(head === freeze && origin === freeze, "freeze");
  ...
  req(
    (await hashFile(pathFor(PROTOCOL_URI))) ===
      (await committedHash(PREREGISTRATION_COMMIT, PROTOCOL_URI)),
    "protocol drift",
  );
  req(
    (await hashFile(pathFor(CORRECTION_URI))) ===
      (await committedHash(CORRECTION_COMMIT, CORRECTION_URI)),
    "correction drift",
  );
  req(
    (await hashFile(pathFor(CORRECTION_PROTOCOL_URI))) ===
      (await committedHash(CORRECTION_COMMIT, CORRECTION_PROTOCOL_URI)),
    "correction protocol drift",
  );
  req(
    (await hashFile(pathFor(CORRECTION2_URI))) ===
      (await committedHash(CORRECTION2_COMMIT, CORRECTION2_URI)),
    "C2 drift",
  );
  req(
  ...
  req(
    canonical(retainedTree) === canonical(correction.retainedFailure.tree),
    "retained tree",
  );
  const retained2Tree = await treeIdentity(RETAINED2_ROOT);
  req(
  ...
  req(
    canonical(parentTree) === canonical(spec.parentEvidence.d3Retry.tree),
    "parent tree",
  );
  const parentReceipt = JSON.parse(
    await readFile(pathFor(spec.parentEvidence.d3Retry.receipt.uri), "utf8"),
  );
  req(
    (await hashFile(pathFor(spec.parentEvidence.d3Retry.receipt.uri))) ===
      spec.parentEvidence.d3Retry.receipt.sha256 &&
      parentReceipt.receiptHash ===
        spec.parentEvidence.d3Retry.receipt.receiptHash &&
      parentReceipt.selectedCandidateId ===
        spec.parentEvidence.d3Retry.selectedCandidateId,
    "parent receipt",
  );
  const master = pathFor(spec.parentEvidence.masterScene.uri),
    masterSha = await hashFile(master);
  req(
    masterSha === spec.parentEvidence.masterScene.sha256 &&
      (await hashFile(spec.runtime.blender.executable)) ===
        spec.runtime.blender.sha256,
    "runtime identity",
  );
  const fs = await statfs(repositoryRoot),
    available = Number(fs.bavail) * Number(fs.bsize);
  req(
    available - spec.processBudget.projectedWriteBytes >=
  ...
  await mkdir(resolve(root, "scene"), { mode: 0o700 });
  const admission = await writeHashed(
    resolve(root, "admission.json"),
    {
      ...
      bindings: {
        ...
        blenderSha256: await hashFile(spec.runtime.blender.executable),
        tools: toolHashes,
      },
      resources: {
        availableBytesBefore: available,
        projectedWriteBytes: spec.processBudget.projectedWriteBytes,
        minimumFreeReserveBytes: spec.processBudget.minimumFreeReserveBytes,
      },
      operations: {
        blenderStartsBeforeAdmission: 0,
        renderCalls: 0,
        modelCalls: 0,
        networkCalls: 0,
        dockerProcesses: 0,
  ...
  try {
    const derived = resolve(root, spec.output.derivedScene),
      buildReport = resolve(root, "build.json");
    const build = await child({
      ...
      budget: spec.processBudget.maximumWallSecondsBuild,
    });
    const buildDoc = JSON.parse(await readFile(buildReport, "utf8"));
    req(
      validSelf(buildDoc, "reportHash") &&
        buildDoc.status === "PASS" &&
        (await hashFile(derived)) === buildDoc.derived.sha256,
      "build output",
    );
    const renderReport = resolve(root, "render.json");
    const render = await child({
      root,
      id: "RENDER",
      scene: derived,
      tool: TOOLS[1],
      args: [
        "--output-dir",
        resolve(root, "renders"),
        "--report",
        renderReport,
        "--derived-sha256",
        buildDoc.derived.sha256,
      ],
      budget: spec.processBudget.maximumWallSecondsRender,
    });
    const renderDoc = JSON.parse(await readFile(renderReport, "utf8"));
    req(
      validSelf(renderDoc, "reportHash") && renderDoc.renders.length === 12,
      "render output",
    );
    const independentReport = resolve(root, "independent.json");
    const independent = await child({
      root,
      id: "INDEPENDENT",
      scene: derived,
      tool: TOOLS[2],
      ...
      budget: spec.processBudget.maximumWallSecondsIndependentAudit,
    });
    const independentDoc = JSON.parse(
      await readFile(independentReport, "utf8"),
    );
    req(
      validSelf(independentDoc, "reportHash") &&
        independentDoc.status === "PASS",
      "independent output",
    );
    const auditorArgs = [
        pathFor(TOOLS[4]),
      ...
      auditorResult = await runBudgetedProcess({
      ...
      auditor = await processReceipt(
        root,
        "AUDITOR",
        process.execPath,
        auditorArgs.map((v) =>
          v.startsWith(repositoryRoot) ? relative(repositoryRoot, v) : v,
        ),
        auditorResult,
      );
    req(
      auditorResult.outcome === "PASS" && auditorResult.child.exitCode === 0,
      "Node audit",
    );
    const auditPath = resolve(root, "audit.json"),
      audit = JSON.parse(await readFile(auditPath, "utf8"));
    req(
      validSelf(audit, "auditHash") && audit.status === "PASS",
      "audit invalid",
    );
    const receipt = await writeHashed(
      resolve(root, "receipt.json"),
      {
        schemaVersion: "bfs.b62CameraQualityHoldoutRenderReceipt.v0.1",
        experimentId: "B62-Q1-D4",
        status: "PASS",
        scientificVerdict: audit.scientificVerdict,
        humanReviewStatus: "PENDING",
        admission: {
          sha256: await hashFile(resolve(root, "admission.json")),
          admissionHash: admission.admissionHash,
        },
        derivedScene: {
          uri: `${ROOT_URI}/${spec.output.derivedScene}`,
          sha256: buildDoc.derived.sha256,
          bytes: buildDoc.derived.bytes,
        },
        reports: {
          build: {
            sha256: await hashFile(buildReport),
            reportHash: buildDoc.reportHash,
          },
          render: {
            sha256: await hashFile(renderReport),
            reportHash: renderDoc.reportHash,
          },
          independent: {
            sha256: await hashFile(independentReport),
            reportHash: independentDoc.reportHash,
          },
          audit: {
        ...
        processes: {
          build: {
            sha256: await hashFile(resolve(root, "processes/BUILD.json")),
            processHash: build.processHash,
          },
          render: {
            sha256: await hashFile(resolve(root, "processes/RENDER.json")),
            processHash: render.processHash,
          },
          independent: {
            sha256: await hashFile(resolve(root, "processes/INDEPENDENT.json")),
            processHash: independent.processHash,
          },
          auditor: {
            sha256: await hashFile(resolve(root, "processes/AUDITOR.json")),
            processHash: auditor.processHash,
          },
        },
        operations: {
          runnerProcesses: 1,
    ...
    console.log(
      `BFS_B62_Q1_D4 PASS ${receipt.scientificVerdict} HUMAN_PENDING ${receipt.receiptHash}`,
    );
  } catch (e) {
    if (!(await exists(resolve(root, "failure.json"))))
      await writeHashed(
        resolve(root, "failure.json"),
        {
