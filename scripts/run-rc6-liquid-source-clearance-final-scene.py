#!/usr/bin/env python3
"""Resolution-192 confirmation of the accepted 35 mm static geometry."""
...
if hashlib.sha256(BASE.read_bytes()).hexdigest() != EXPECTED_BASE_SHA256:
    raise RuntimeError("RC6 source-clearance final scene base identity mismatch")
...
replacements = (
    ...
    ("bfs.rc6LiquidSourceClearanceCell.v0.2", "bfs.rc6LiquidSourceClearanceFinalCell.v0.1", "schema"),
)
for before, after, label in replacements:
    if source.count(before) != 1:
        raise RuntimeError(f"RC6 source-clearance final scene {label} target mismatch")
    source = source.replace(before, after)
exec(compile(source, str(BASE) + "#SOURCE_CLEARANCE_FINAL_V01", "exec"), globals(), globals())
