...
C3_SPEC = RESEARCH / "specs/ai-native-studio-rc6-real-impact-liquid-particle-radius-c20-c3.v1.07.json"
C3_EVIDENCE = RESEARCH / "experiments/physical-richness/RC6-2026-09-02-real-impact-liquid-particle-radius-c20-c3-audit-attempt-96"
...
def transformed_source():
    if sha(BASE) != EXPECTED_BASE_SHA256:
        raise RuntimeError("C20 C3 audit base identity mismatch")
...
EVIDENCE = RESEARCH / "experiments/physical-richness/RC6-2026-09-02-real-impact-liquid-particle-radius-c20-c3-audit-attempt-96"
...
freeze_paths = set(subprocess.run(["git", "diff-tree", "--no-commit-id", "--name-only", "-r", head], cwd=RESEARCH, capture_output=True, text=True, check=True).stdout.splitlines())'''
    ...
    check_extension = check_anchor + '''    "c3SpecSelfHash": c3_spec["specHash"] == self_hash(c3_spec, "specHash"),
    "c3ToolIdentity": sha(Path(__file__).resolve()) == c3_spec["toolSha256"],
    "c3FreezeCommitBound": c3_parent == c3_spec["researchParentBeforePreregistration"] and c3_freeze_paths == set(c3_spec["freezePaths"]),
    "retainedEvidenceRootExact": manifest(RETAINED_EVIDENCE)["manifestHash"] == c3_spec["retainedEvidenceManifestHash"],
'''
    audit_schema_old = '"bfs.rc6RealImpactLiquidParticleRadiusC20C1IndependentAudit.v0.1"'
    audit_schema_new = '"bfs.rc6RealImpactLiquidParticleRadiusC20C3AuditOnly.v0.1"'
    ...
    "counts": c3_spec["counts"],
    "claimCeiling": c3_spec["claimCeiling"],
'''
    ...
    replacements = (
        ...
        ("RC6_REAL_IMPACT_LIQUID_PARTICLE_RADIUS_C20_C1_AUDIT=", "RC6_REAL_IMPACT_LIQUID_PARTICLE_RADIUS_C20_C3_AUDIT=", "audit marker", 1),
        ("real-impact liquid C20 C1 independent audit failed", "real-impact liquid C20 C3 audit-only closure failed", "failure marker", 1),
    )
    for before, after, label, expected in replacements:
        if source.count(before) != expected:
            raise RuntimeError(f"C20 C3 {label} target mismatch: {source.count(before)} != {expected}")
        source = source.replace(before, after)
    return source


def main():
    c3_spec = json.loads(C3_SPEC.read_text())
    if c3_spec["specHash"] != self_hash(c3_spec, "specHash"):
        raise RuntimeError("C20 C3 spec self hash mismatch")
    if sha(Path(__file__).resolve()) != c3_spec["toolSha256"]:
        raise RuntimeError("C20 C3 tool identity mismatch")
    if subprocess.run(["git", "status", "--porcelain"], cwd=RESEARCH, capture_output=True, text=True, check=True).stdout:
        raise RuntimeError("C20 C3 worktree is not clean")
    c3_head = subprocess.run(["git", "rev-parse", "HEAD"], cwd=RESEARCH, capture_output=True, text=True, check=True).stdout.strip()
    c3_parent = subprocess.run(["git", "rev-parse", "HEAD^"], cwd=RESEARCH, capture_output=True, text=True, check=True).stdout.strip()
    c3_freeze_paths = set(subprocess.run(["git", "diff-tree", "--no-commit-id", "--name-only", "-r", "HEAD"], cwd=RESEARCH, capture_output=True, text=True, check=True).stdout.splitlines())
    if c3_parent != c3_spec["researchParentBeforePreregistration"] or c3_freeze_paths != set(c3_spec["freezePaths"]):
        raise RuntimeError("C20 C3 freeze commit mismatch")
    retained = RESEARCH / c3_spec["retainedEvidenceRoot"]
    retained_manifest = manifest(retained)
    if retained_manifest["manifestHash"] != c3_spec["retainedEvidenceManifestHash"]:
        raise RuntimeError("C20 C3 retained evidence drift")
    if C3_EVIDENCE.exists():
        raise RuntimeError("C20 C3 evidence root is not fresh")
    C3_EVIDENCE.mkdir(parents=True, exist_ok=False)
    scope = {
        "schemaVersion": "bfs.rc6RealImpactLiquidParticleRadiusC20C3Admission.v0.1",
        "status": "PASS",
        "executionCommit": c3_head,
        "retainedEvidenceManifestHash": retained_manifest["manifestHash"],
        "counts": c3_spec["counts"],
    }
    write_exclusive(C3_EVIDENCE / "admission.json", scope)
    exec(compile(transformed_source(), str(BASE) + "#RC6_REAL_IMPACT_LIQUID_PARTICLE_RADIUS_C20_C3", "exec"), globals(), locals())
    audit = json.loads((C3_EVIDENCE / "audit.json").read_text())
    receipt = {
        "schemaVersion": "bfs.rc6RealImpactLiquidParticleRadiusC20C3Receipt.v0.1",
        "status": audit["status"],
        "executionCommit": c3_head,
    ...
    write_exclusive(C3_EVIDENCE / "receipt.json", receipt)
    write_exclusive(C3_EVIDENCE / "evidence-manifest.json", manifest(C3_EVIDENCE))
    print("RC6_REAL_IMPACT_LIQUID_PARTICLE_RADIUS_C20_C3_RECEIPT=" + canonical(receipt), flush=True)
