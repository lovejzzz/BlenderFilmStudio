...
const FREEZE_URI = 'specs/ai-native-studio-pc4-tool-freeze.v0.1.json';
const EVIDENCE_URI = 'experiments/ai-native-studio-post-pb7/PC.4-2026-08-31-mac-m2max-attempt-01';
const WORK = '/Users/mengyingli/Documents/ChatGPT/BlenderFilmStudio-PostPB7-workspace/PC.4-2026-08-31-mac-m2max-attempt-01';
...
async function runProcess(executable, argv, logPath, timeoutMs) {
  ...
  const child = spawn(executable, argv, { cwd: root, env: process.env, stdio: ['ignore', 'pipe', 'pipe'] });
  let output = '';
  child.stdout.on('data', chunk => { output += chunk; });
  child.stderr.on('data', chunk => { output += chunk; });
  const timer = setTimeout(() => child.kill('SIGTERM'), timeoutMs);
  const result = await new Promise((resolveResult, reject) => { child.once('error', reject); child.once('close', (code, signal) => resolveResult({ code, signal })); });
  clearTimeout(timer);
  await writeFile(logPath, output, { flag: 'wx', mode: 0o600 });
  return { ...result, wallSeconds: (Date.now() - started) / 1000, output };
}
...
async function main() {
  ...
  if (!validSelf(freeze, 'specHash') || freeze.status !== 'FROZEN_BEFORE_PC4_MUTATION' || freeze.preregistration.specHash !== spec.specHash) throw new Error('FREEZE');
  ...
  const common = ['--background'];
  const buildArgv = [...common, spec.source.path, '--disable-autoexec', '--python-exit-code', '1', '--python', resolve(root, 'scripts/build-ai-native-studio-pc4.py'), '--', '--spec', specPath, '--evidence-root', evidence, '--work-root', WORK];
  ...
  const auditArgv = [...common, buildRecord.derived.path, '--disable-autoexec', '--python-exit-code', '1', '--python', resolve(root, 'scripts/audit-ai-native-studio-pc4.py'), '--', '--spec', specPath, '--evidence-root', evidence, '--work-root', WORK];
  ...
  const receipt = await writeSelf(resolve(evidence, 'receipt.json'), {
    schemaVersion: 'bfs.pc4HeroRedesignReceipt.v0.1', status: 'MACHINE_PASS_VISUAL_REVIEW_REQUIRED', gate: 'PC.4',
  ...
  process.stdout.write(`BFS_PC4_RUN ${receipt.status} ${receipt.receiptHash}\n`);
}

main().catch(async error => { process.stderr.write(`BFS_PC4_RUN_REJECTED ${error.message}\n`); process.exitCode = 1; });
