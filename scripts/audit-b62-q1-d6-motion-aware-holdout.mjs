...
export async function audit(argv) {
  ...
  const spec = await json(SPEC_URI), correction = await json(CORRECTION_URI), admission = await json(`${ROOT_URI}/admission.json`), build = await json(`${ROOT_URI}/build.json`), render = await json(`${ROOT_URI}/render.json`), independent = await json(`${ROOT_URI}/independent.json`);
  ...
  req(admission.bindings.correction.sha256 === await hashFile(pathFor(CORRECTION_URI)) && admission.bindings.correctionProtocol.sha256 === await hashFile(pathFor(CORRECTION_PROTOCOL_URI)), 'correction binding');
  req(canonicalJson(admission.bindings.retainedFailureTree) === canonicalJson(correction.retainedFailure.tree) && admission.bindings.retainedFailureSha256 === correction.retainedFailure.failure.sha256 && admission.bindings.retainedBuildSha256 === correction.retainedFailure.build.sha256 && await hashFile(pathFor(correction.retainedFailure.failure.uri)) === correction.retainedFailure.failure.sha256 && await hashFile(pathFor(correction.retainedFailure.build.uri)) === correction.retainedFailure.build.sha256, 'retained binding');
  ...
  req(admission.bindings.tools[TOOLS[0]] === correction.frozenBeforeCorrection.builderSha256 && admission.bindings.tools[TOOLS[1]] === correction.frozenBeforeCorrection.renderSha256 && admission.bindings.tools[TOOLS[2]] === correction.frozenBeforeCorrection.independentSha256, 'frozen Blender tools');
  ...
  const checks = [
    ['SPEC_CORRECTION_ADMISSION_PARENT_BOUND', true], ['RETAINED_V0_1_FAILURE_BOUND', true], ['TOOL_FREEZE_HASHES_EXACT', true], ['THREE_FRESH_BLENDER_PROCESSES_PASS', true], ['BLENDER_5_2_IDENTITY_EXACT', true],
