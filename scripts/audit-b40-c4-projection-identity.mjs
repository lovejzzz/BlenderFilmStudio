...
import { analyzeB40C4Evidence, readB40C4Spec, roundTripB40C2, runB40C4Attacks } from './lib/b40-c4-projection-identity.mjs';
...
const analysis = await analyzeB40C4Evidence(result, c4Spec, c2Spec, baseSpec);
const attacks = await runB40C4Attacks(result, c4Spec, c2Spec, baseSpec);
const roundTrip = roundTripB40C2(result);
const roundTripAnalysis = await analyzeB40C4Evidence(roundTrip, c4Spec, c2Spec, baseSpec);
const roundTripAttacks = await runB40C4Attacks(roundTrip, c4Spec, c2Spec, baseSpec);
const audit = {
  schemaVersion: 'bfs.workerHostCapacityIndependentAudit.v0.5', experimentId: 'B40-C4', analysis, attacks,
...
process.stdout.write(`BFS_B40_C4_AUDIT ${audit.passed ? 'PASS' : 'FAIL'} blockers=${result.decision.reasons.length} attacks=${attacks.filter(a => a.passed).length}/14 replay=${audit.roundTripAttackVectorEqual ? 'PASS' : 'FAIL'} runtimeOps=${result.runtimeOperationsExecuted.length}\n`);
