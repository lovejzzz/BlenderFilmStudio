...
const root=resolve(REPO,SPEC.paths.evidenceRoot),auditPath=resolve(root,'audit-c1.json');if(existsSync(auditPath))throw new Error('C1 audit exists');
const failedAudit=JSON.parse(readFileSync(resolve(root,'audit.json'),'utf8'));
...
add('INDEPENDENT',!/^\\s*import .*run-ai-native-studio-pb1-validation-only-c4-runtime-recovery/m.test(readFileSync(SELF,'utf8')));
add('FAILED_AUDIT_RETAINED_SOLE_SELF_CHECK',failedAudit.status==='FAIL'&&failedAudit.checksPassed===13&&failedAudit.checksTotal===14&&failedAudit.failures.join(',')==='INDEPENDENT'&&receiptHash(failedAudit)===failedAudit.receiptHash);
...
const failed=checks.filter(x=>!x.pass),record={schemaVersion:'bfs.pb1ValidationOnlyC4IndependentAuditC1.v0.11',gate:'PB.1',observedAt:new Date().toISOString(),status:failed.length?'FAIL':'PASS',auditor:{path:'scripts/audit-ai-native-studio-pb1-validation-only-c4-runtime-recovery-c1.mjs',sha256:fileSha(SELF),importsRunner:false},checksPassed:checks.length-failed.length,checksTotal:checks.length,checks,failures:failed.map(x=>x.id),retainedFailedAuditReceiptHash:failedAudit.receiptHash,runtimeReceiptHash:runtime.receiptHash,verdictReceiptHash:verdict.receiptHash,externalMutationsPerformedByAuditor:0,stopRulePreserved:true};record.receiptHash=receiptHash(record);const fd=openSync(auditPath,'wx',0o600);try{writeFileSync(fd,JSON.stringify(record,null,2)+'\n')}finally{closeSync(fd)}process.stdout.write(JSON.stringify(record,null,2)+'\n');process.exit(record.status==='PASS'?0:1);
