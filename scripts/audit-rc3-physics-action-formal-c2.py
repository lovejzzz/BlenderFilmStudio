#!/usr/bin/env python3
"""Audit retained RC3 formal evidence with corrected inherited limits and visual binding."""
...
AUDIT_ROOT = ROOT / "experiments/physics-native-action/RC3-2026-09-01-formal-audit-c2"
...
PREREG_BASE = ROOT / "specs/ai-native-studio-rc3-physics-native-action-grammar-preregistration.v0.1.json"
FREEZE = ROOT / "specs/ai-native-studio-rc3-physics-action-formal-audit-c2.v0.1.json"
...
def main():
    if not AUDIT_ROOT.is_dir():
        raise RuntimeError("correction audit root must be preregistered and created")
    if (AUDIT_ROOT / "audit.json").exists():
        raise RuntimeError("correction audit output already exists")
    ...
    visual_review = load(AUDIT_ROOT / "direct-visual-review.json")
    failure = load(AUDIT_ROOT / "retained-audit-attempt-01-failure.json")
    prereg, prereg_base, freeze = load(PREREG), load(PREREG_BASE), load(FREEZE)
    process_names = (
    ...
    limits = prereg_base["resourceCeilings"]
    checks = {
        ...
        "admissionThreshold": admission["freeBytesBefore"] >= limits["minimumFreeBeforeNativeBuildGiB"] * 1024**3,
        "processIntegrity": process_integrity,
        "correctionFreezeSelfHash": freeze["freezeHash"] == self_hash(freeze, "freezeHash"),
        "correctionToolBindings": all(sha(ROOT / row["uri"]) == row["sha256"] for row in freeze["tools"]),
        "retainedAuditFailure": failure["failureHash"] == self_hash(failure, "failureHash") and failure["failureClass"] == "AUDITOR_INHERITED_LIMIT_LOOKUP_KEY_ERROR" and failure["formalReceiptHash"] == receipt["receiptHash"],
        "preregCorrectionBinding": prereg["inheritedRequirements"].startswith("All requirements") and prereg_base["specHash"] == "85e6aae4bc4604d9823c896e98a2e01ebee879fe050ec32a8cf870dcb3200ce2",
        ...
        "freshFormalDirectReview": visual_review["status"] == "PASS" and visual_review["answerCounts"] == {"YES": 10, "NO": 0} and visual_review["reviewHash"] == self_hash(visual_review, "reviewHash") and visual_review["formalReceiptHash"] == receipt["receiptHash"],
        "reviewArtifactBindings": all(sha(ROOT / row["uri"]) == row["sha256"] for row in visual_review["artifacts"]),
        ...
        "resourceCeilings": tree_bytes(WORK) <= limits["workspaceBytes"] and tree_bytes(EVIDENCE) <= limits["evidenceBytes"] and receipt["resources"]["freeBytesAfter"] >= limits["minimumFreeReserveGiB"] * 1024**3,
    }
    audit = {
        "schemaVersion": "bfs.rc3PhysicsActionFormalIndependentAudit.v0.2",
        "status": "PASS" if all(checks.values()) else "FAIL",
        "checks": checks,
        "checkCount": len(checks),
        "passCount": sum(checks.values()),
        "formalEvidence": str(EVIDENCE.relative_to(ROOT)),
    ...
    (AUDIT_ROOT / "audit.json").write_text(json.dumps(audit, indent=2, sort_keys=True) + "\n", encoding="utf-8")
