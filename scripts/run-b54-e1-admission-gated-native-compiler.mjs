...
const AUDITOR = 'scripts/audit-b54-e1-admission-gated-native-compiler.mjs';
...
async function writeAdmissionFailure(attemptRoot, attemptRead, error, gitChildren) {
  ...
  const failure = await writeHashed(failurePath, {
    schemaVersion: 'bfs.admissionGatedNativeCompilerAdmissionFailure.v0.1',
    experimentId: 'B54-E1',
    sequence: 2,
    status: 'REJECTED',
  ...
  await writeHashed(resolve(attemptRoot, 'receipt.json'), {
    schemaVersion: 'bfs.admissionGatedNativeCompilerAttemptReceipt.v0.1',
    experimentId: 'B54-E1',
    sequence: 3,
    status: 'REJECTED',
    scientificVerdict: null,
...
async function writeInvalidation(root, phase, error, context = {}) {
  ...
  await writeHashed(path, {
    schemaVersion: 'bfs.admissionGatedNativeCompilerInvalidation.v0.1',
    experimentId: 'B54-E1',
    status: 'INVALIDATED',
    phase,
...
async function main() {
  ...
  if (await pathState(attemptRoot) || await pathState(formalRoot)) throw new Error('B54-E1 attempt/formal root already exists; runner is single-use');
  ...
  const attempt = await writeHashed(attemptPath, {
    schemaVersion: 'bfs.admissionGatedNativeCompilerAttempt.v0.1',
    experimentId: 'B54-E1',
    sequence: 1,
    invocation: {
      spec: args.spec ?? 'specs/admission-gated-native-compiler-integration.v0.1.json',
  ...
  try {
    ...
    process.stdout.write(`BFS_B54_E1_FORMAL_ADMISSION_REJECTED reason=${error.reason ?? 'ADMISSION_EXCEPTION'} blender=0\n`);
    process.exitCode = 1;
    return;
  }
  ...
  const admissionRecord = await writeHashed(admissionPath, {
    schemaVersion: 'bfs.admissionGatedNativeCompilerAdmission.v0.1',
    experimentId: 'B54-E1',
    sequence: 2,
  ...
  try {
    ...
    const formalStart = await writeHashed(formalStartPath, {
      schemaVersion: 'bfs.admissionGatedNativeCompilerFormalStart.v0.1',
      experimentId: 'B54-E1',
      sequence: 4,
      runnerPid: process.pid,
    ...
    const planObservation = await writeHashed(planObservationPath, {
      schemaVersion: 'bfs.admissionGatedNativeCompilerPlanObservations.v0.1',
      experimentId: 'B54-E1',
      sequence: 5,
      observations: pair.observations,
      plans: planBindings,
    ...
    for (const benchmark of spec.inputs.benchmarks) {
      for (const suffix of ['A', 'B']) {
        ...
        runBindings.push({
          ...
          budget: { uri: `${runUri}/budget.report.json`, sha256: await sha256File(budgetPath), outcome: budget.outcome, nativeCommand: budget.command, nativeChildPid: budget.child?.pid ?? null },
    ...
    const nativePidBindingsAvailable = runBindings.every(row => Number.isInteger(row.budget.nativeChildPid));
    const operationDraft = await writeHashed(operationDraftPath, {
      schemaVersion: 'bfs.admissionGatedNativeCompilerOperationDraft.v0.1',
      experimentId: 'B54-E1',
      sequence: 6,
      runnerPid: process.pid,
      runnerProcesses: 1,
    ...
    const operation = await writeHashed(operationPath, {
      schemaVersion: 'bfs.admissionGatedNativeCompilerOperation.v0.1',
      experimentId: 'B54-E1',
      sequence: 7,
    ...
    const results = await writeHashed(resultsPath, {
      schemaVersion: 'bfs.admissionGatedNativeCompilerIntegrationResult.v0.1',
      experimentId: 'B54-E1',
      sequence: 8,
      status: 'COMPLETE',
      scientificVerdict,
    ...
    const receipt = await writeHashed(resolve(formalRoot, 'receipt.json'), {
      schemaVersion: 'bfs.admissionGatedNativeCompilerIntegrationReceipt.v0.1',
      experimentId: 'B54-E1',
      sequence: 9,
      status: 'COMPLETE',
      scientificVerdict,
    ...
    process.stdout.write(`BFS_B54_E1_FORMAL_COMPLETE verdict=${scientificVerdict} gates=${results.gatePassed}/${results.gateTotal} attacks=${results.semanticAttacks.passed}/${results.semanticAttacks.total} receipt=${receipt.receiptHash}\n`);
  } catch (error) {
