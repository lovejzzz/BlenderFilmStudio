#!/usr/bin/env python3
"""Independent frozen-tool and byte-exact replay audit for B52-D2."""
...
def main() -> None:
    ...
    with tempfile.TemporaryDirectory(prefix="bfs-b52-d2-audit-") as temporary:
        ...
        if process.returncode:
            raise RuntimeError(f"B52-D2 analyzer replay failed: {process.stdout}\n{process.stderr}")
        replay_exact = result_path.read_bytes() == replay.read_bytes()
        replay_observation = {
            "uri": "results.json",
            "originalSha256": sha256_file(result_path),
            "replaySha256": sha256_file(replay),
    ...
    bound_inputs = []
    observation_groups = [
        receipt["parentObservations"], receipt["sourceObservations"], receipt["sourcePostObservations"],
        receipt["referenceObservations"], receipt["referencePostObservations"], [receipt["specObservation"]],
    ]
    for item in [entry for group in observation_groups for entry in group]:
        path = root / item["uri"]
        observed = sha256_file(path) if path.is_file() else None
        observed_bytes = path.stat().st_size if path.is_file() else None
        expected_bytes = item.get("expectedBytes")
        bound_inputs.append({
            "kind": item["kind"],
            "uri": item["uri"],
            ...
            "match": observed == item["expectedSha256"] and (expected_bytes is None or observed_bytes == expected_bytes),
        })
    ...
    valid_verdicts = {spec["selectionRule"]["positiveVerdict"], spec["selectionRule"]["negativeVerdict"]}
    passed = (
        replay_exact
        and all(item["match"] for item in frozen_tools + bound_inputs + artifacts)
        and result["verdict"] in valid_verdicts
        and result["attacksPassed"] == len(spec["attacks"])
        and result["baseFailure"] is None
    )
    audit = {
        "schemaVersion": "bfs.nativeCpuAdaptiveProductionAudit.v0.1",
    ...
    print(f"BFS_B52_D2_AUDIT {audit['status']} replayExact={replay_exact} artifacts={sum(item['match'] for item in artifacts)}/{len(artifacts)}", flush=True)
    if not passed:
        raise SystemExit(1)
