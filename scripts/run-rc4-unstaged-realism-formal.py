#!/usr/bin/env python3
"""Clean-build formal RC4 validation in fresh immutable roots."""
...
PRODUCT_TOOL = RESEARCH / "scripts/run-rc4-unstaged-realism-product.py"
...
def command(index, name, argv, cwd=None):
    ...
    row = {"index": index, "name": name, "argv": [str(item) for item in argv], "cwd": str(cwd or RESEARCH), "exitCode": done.returncode, "wallSeconds": round(time.monotonic() - started, 6), "stdoutSha256": sha(stdout_path), "stderrSha256": sha(stderr_path)}
...
def main():
    if WORK.exists() or EVIDENCE.exists():
        raise RuntimeError("RC4 formal roots are not fresh")
    if output(["git", "rev-parse", "HEAD"], PRODUCT) != COMMIT or output(["git", "status", "--porcelain"], PRODUCT):
        raise RuntimeError("RC4 candidate source identity mismatch")
    ...
    if free_before < MINIMUM_FREE_BEFORE or free_before < MINIMUM_RESERVE + PROJECTED_WRITES:
        raise RuntimeError("RC4 formal resource admission failed")
    ...
    admission = {"schemaVersion": "bfs.rc4FormalAdmission.v0.1", "status": "PASS", "freeBytesBefore": free_before, "minimumFreeBeforeBytes": MINIMUM_FREE_BEFORE, "minimumReserveBytes": MINIMUM_RESERVE, "projectedWriteBytes": PROJECTED_WRITES, "workLimitBytes": WORK_LIMIT, "evidenceLimitBytes": EVIDENCE_LIMIT}
    ...
    if not built.is_dir() or bundle.exists():
        raise RuntimeError("formal bundle rename precondition")
    built.rename(bundle)
    ...
    if not binary.is_file() or not operator.is_file() or not all(path.is_file() for path in installed.values()):
        raise RuntimeError("formal installed product files missing")
    expected_installed = {
        "physicsAction": "a7a7b7e5359f032db0a6dd7d1cc7a4d48e5ef3c8c335608c4d5ab3594cc6af3e",
    ...
    processes.append(command(5, "r1-build", common + ["--action", "build", *tail]))
    blend = runtime / "RC4_R1_UNSTAGED_PHYSICAL_REALISM.blend"
    opened = [str(binary), "--background", "--disable-autoexec", "--offline-mode", str(blend), "--python", str(PRODUCT_TOOL), "--"]
    processes.append(command(6, "r1-reopen", opened + ["--action", "reopen", *tail]))
    processes.append(command(7, "d1-h1-regressions", common + ["--action", "regress", *tail]))
    processes.append(command(8, "negative-controls", common + ["--action", "negative", *tail]))
    processes.append(command(9, "r1-render", opened + ["--action", "render", *tail]))
    video = EVIDENCE / "contact-clip.mp4"
    processes.append(command(10, "contact-video", ["/opt/homebrew/bin/ffmpeg", "-hide_banner", "-loglevel", "error", "-y", "-framerate", "24", "-pattern_type", "glob", "-i", str(EVIDENCE / "clip/frame-*.png"), "-c:v", "libx264", "-pix_fmt", "yuv420p", "-movflags", "+faststart", str(video)]))

    build_row = json.loads((EVIDENCE / "R1-build.json").read_text())
    ...
    changed = output(["git", "diff", "--numstat", PARENT, COMMIT, "--", "scripts/modules/film_studio_physics_action.py", "scripts/modules/film_studio_physical_look.py"], source).splitlines()
    ...
    checks = {
        ...
        "sourceScope": changed == ["270\t0\tscripts/modules/film_studio_physical_look.py", "101\t19\tscripts/modules/film_studio_physics_action.py"],
        "installedHashes": installed_hashes == expected_installed,
        "r1Topology": result["topology"] == "GROUP_RESPONSE" and result["mechanism"]["activeRigidBodyCount"] == 4 and result["mechanism"]["rigidBodyConstraintCount"] == 0,
        "derivedPhysics": physics["contactFrame"] is not None and physics["firstResponseDelayFrames"] <= 2 and physics["respondingTargetCount"] >= 2 and physics["continuousActorMotionThroughContact"] and physics["settledWindowFrameCount"] >= 10,
        "settledBinding": result["cinematography"]["effect"]["frame"] == physics["settledGroupFrame"],
    ...
    receipt = {
        "schemaVersion": "bfs.rc4UnstagedRealismFormalReceipt.v0.1",
        ...
        "counts": {"cleanNativeBuilds": 1, "productStarts": 5, "sceneMutations": 3, "blendSaves": 1, "reopens": 1, "reviewStills": 3, "contactClipFrames": 48, "ffmpegProcesses": 1, "networkCalls": 0, "engineRemoteWrites": 0, "forcePushes": 0, "tags": 0, "releases": 0, "binaryDistribution": 0, "signing": 0, "notarization": 0},
    ...
    print("RC4_FORMAL=" + canonical(receipt))
    if receipt["status"] == "FAIL":
        raise RuntimeError("RC4 formal machine validation failed")
