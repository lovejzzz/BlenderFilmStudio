...
export async function audit(freeze) {
  ...
  req(correctionC3.correctionId === 'B62-T2-E1-C3' && correctionC3.retainedResult.root === RETAINED_C3_ROOT_URI && correctionC3.authorizedChanges.retryRoot === ROOT_URI, 'C3 mismatch');
  ...
  const operationsExact = render.operations.blenderStarts === 1 && render.operations.sceneSaves === 0 && render.operations.eeveeRenderCalls === 288 && render.operations.cyclesRenderCalls === 0 && render.source.sha256 === spec.parentEvidence.scene.sha256 && render.source.sha256AfterRender === spec.parentEvidence.scene.sha256 && render.source.unchanged === true && independent.operations.blenderStarts === 1 && independent.operations.renderCalls === 0 && [render, independent].every(row => row.operations.modelCalls === 0 && row.operations.networkCalls === 0 && row.operations.dockerProcesses === 0);
  const filesystem = await statfs(repositoryRoot), available = Number(filesystem.bavail) * Number(filesystem.bsize);
  const admissionExact = validSelf(admission, 'admissionHash') && admission.status === 'ADMITTED' && admission.correctionCommit === CORRECTION_COMMIT && admission.correctionC2Commit === CORRECTION_C2_COMMIT && admission.correctionC3Commit === CORRECTION_C3_COMMIT && admission.toolFreezeCommit === freeze && canonicalJson(admission.parentTree) === canonicalJson(parentTree) && canonicalJson(admission.retainedFailureTree) === canonicalJson(retainedTree) && canonicalJson(admission.retainedC2FailureTree) === canonicalJson(retainedC2Tree) && canonicalJson(admission.retainedC3ResultTree) === canonicalJson(retainedC3Tree) && canonicalJson(admission.bindings.tools) === canonicalJson(tools) && canonicalJson(admission.bindings.runtime) === canonicalJson(runtimeHashes) && admission.bindings.scene.sha256 === spec.parentEvidence.scene.sha256;
  const technical = { preregExact, retainedExact, retainedC2Exact, retainedC3Exact, parentExact, toolsExact, runtimeExact, admission: admissionExact, rootRosterExact, renderSelf: validSelf(render, 'reportHash') && render.status === 'PASS', independentSelf: validSelf(independent, 'reportHash') && independent.status === 'PASS', processes: Object.values(processChecks).every(Boolean), outputBudget: snapshot.bytes <= spec.processBudget.maximumOutputBytes, reserve: available >= spec.processBudget.minimumFreeReserveBytes };
  const gates = [
    ['G01_PREREGISTRATION_AND_T1_PARENT_EXACT', preregExact && retainedExact && retainedC2Exact && retainedC3Exact && parentExact],
