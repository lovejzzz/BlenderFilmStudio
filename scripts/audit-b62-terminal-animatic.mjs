...
export async function audit(freeze) {
  ...
  req(correctionC4.correctionId === 'B62-T2-E1-C4' && correctionC4.retainedFailure.root === RETAINED_C4_ROOT_URI && correctionC4.authorizedChanges.retryRoot === ROOT_URI, 'C4 mismatch');
  ...
  const renderFramesExact = render.frames.length === 288 && render.frames.every((row, index) => { const frame = index + 1, uri = `frames/frame-${String(frame).padStart(4, '0')}.png`, observed = filesByUri.get(uri), [marker, camera] = routeFor(frame); return row.frame === frame && row.contextScene === 'B62_PHASE0_MASTER' && row.contextFrame === frame && row.marker === marker && row.camera === camera && row.png.uri === `${ROOT_URI}/${uri}` && observed && observed.sha256 === row.png.sha256 && observed.bytes === row.png.bytes; });
