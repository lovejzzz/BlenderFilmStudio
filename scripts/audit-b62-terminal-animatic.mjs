...
const ROOT_URI = 'experiments/b62-terminal-animatic-continuity-v0-3';
...
export async function audit(freeze) {
  ...
  req(correctionC2.correctionId === 'B62-T2-E1-C2' && correctionC2.retainedFailure.root === RETAINED_C2_ROOT_URI && correctionC2.authorizedChanges.retryRoot === ROOT_URI, 'C2 mismatch');
  ...
  const admissionExact = validSelf(admission, 'admissionHash') && admission.status === 'ADMITTED' && admission.correctionCommit === CORRECTION_COMMIT && admission.correctionC2Commit === CORRECTION_C2_COMMIT && admission.toolFreezeCommit === freeze && canonicalJson(admission.parentTree) === canonicalJson(parentTree) && canonicalJson(admission.retainedFailureTree) === canonicalJson(retainedTree) && canonicalJson(admission.retainedC2FailureTree) === canonicalJson(retainedC2Tree) && canonicalJson(admission.bindings.tools) === canonicalJson(tools) && canonicalJson(admission.bindings.runtime) === canonicalJson(runtimeHashes) && admission.bindings.scene.sha256 === spec.parentEvidence.scene.sha256;
  const technical = { preregExact, retainedExact, retainedC2Exact, parentExact, toolsExact, runtimeExact, admission: admissionExact, rootRosterExact, renderSelf: validSelf(render, 'reportHash') && render.status === 'PASS', independentSelf: validSelf(independent, 'reportHash') && independent.status === 'PASS', processes: Object.values(processChecks).every(Boolean), outputBudget: snapshot.bytes <= spec.processBudget.maximumOutputBytes, reserve: available >= spec.processBudget.minimumFreeReserveBytes };
  const gates = [
    ['G01_PREREGISTRATION_AND_T1_PARENT_EXACT', preregExact && retainedExact && retainedC2Exact && parentExact],
  ...
  const auditRecord = await writeHashed(resolve(root, 'audit.json'), { schemaVersion: 'bfs.b62TerminalAnimaticAudit.v0.1', experimentId: spec.experimentId, status, scientificVerdict: verdict, humanReview: 'HUMAN_PENDING', preregistrationCommit: PREREGISTRATION_COMMIT, correctionCommit: CORRECTION_COMMIT, correctionC2Commit: CORRECTION_C2_COMMIT, toolFreezeCommit: freeze, gates, processChecks, technical, outcomes: { pixelsExact, temporalExact, cutsExact, geometryPass, causalExact }, bindings: { retainedFailureTree: retainedTree, retainedC2FailureTree: retainedC2Tree, admission: { sha256: await hashFile(resolve(root, 'admission.json')), admissionHash: admission.admissionHash }, render: { sha256: await hashFile(resolve(root, 'reports/render-report.json')), reportHash: render.reportHash }, independent: { sha256: await hashFile(resolve(root, 'reports/independent-audit.json')), reportHash: independent.reportHash }, metadata: { sha256: await hashFile(resolve(root, 'reports/video-metadata.json')), metadataHash: metadata.metadataHash }, video: { sha256: await hashFile(videoPath), bytes: videoBytes.length } }, rootBeforeAudit: snapshot, resources: { availableBytesAtAudit: available, maximumOutputBytes: spec.processBudget.maximumOutputBytes, minimumFreeReserveBytes: spec.processBudget.minimumFreeReserveBytes }, operations: { nodeAuditorProcesses: 1, priorBlenderStarts: 2, priorEeveeRenderCalls: 288, priorCyclesRenderCalls: 0, priorFfmpegProcesses: 1, priorFfprobeProcesses: 1, modelCalls: 0, networkCalls: 0, dockerProcesses: 0 }, nonClaims: spec.nonClaims }, 'auditHash');
