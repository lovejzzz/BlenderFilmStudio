...
const ROOT_URI = 'experiments/b62-terminal-animatic-continuity-v0-1';
...
export async function audit(freeze) {
  ...
  req(spec.experimentId === 'B62-T2-E1' && spec.output.formalRoot === ROOT_URI, 'spec mismatch');
  ...
  const preregExact = await hashFile(pathFor(SPEC_URI)) === await committedHash(PREREGISTRATION_COMMIT, SPEC_URI) && await hashFile(pathFor(PROTOCOL_URI)) === await committedHash(PREREGISTRATION_COMMIT, PROTOCOL_URI);
  ...
  const cameraFor = frame => frame <= 96 ? 'CAM_WIDE_APPROACH' : frame <= 192 ? 'CAM_MEDIUM_CONTACT' : 'CAM_CLOSE_MOTION_TERMINAL';
  const renderFramesExact = render.frames.length === 288 && render.frames.every((row, index) => { const frame = index + 1, uri = `frames/frame-${String(frame).padStart(4, '0')}.png`, observed = filesByUri.get(uri); return row.frame === frame && row.camera === cameraFor(frame) && row.png.uri === `${ROOT_URI}/${uri}` && observed && observed.sha256 === row.png.sha256 && observed.bytes === row.png.bytes; });
  ...
  const routingExact = independent.routing.length === 288 && independent.routing.every((row, index) => row.frame === index + 1 && row.camera === cameraFor(index + 1) && row.expectedCamera === row.camera && row.exact);
  ...
  const admissionExact = validSelf(admission, 'admissionHash') && admission.status === 'ADMITTED' && admission.toolFreezeCommit === freeze && canonicalJson(admission.parentTree) === canonicalJson(parentTree) && canonicalJson(admission.bindings.tools) === canonicalJson(tools) && canonicalJson(admission.bindings.runtime) === canonicalJson(runtimeHashes) && admission.bindings.scene.sha256 === spec.parentEvidence.scene.sha256;
  const technical = { preregExact, parentExact, toolsExact, runtimeExact, admission: admissionExact, rootRosterExact, renderSelf: validSelf(render, 'reportHash') && render.status === 'PASS', independentSelf: validSelf(independent, 'reportHash') && independent.status === 'PASS', processes: Object.values(processChecks).every(Boolean), outputBudget: snapshot.bytes <= spec.processBudget.maximumOutputBytes, reserve: available >= spec.processBudget.minimumFreeReserveBytes };
  const gates = [
    ['G01_PREREGISTRATION_AND_T1_PARENT_EXACT', preregExact && parentExact],
