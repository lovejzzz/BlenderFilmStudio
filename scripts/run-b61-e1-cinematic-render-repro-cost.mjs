...
const TOOL_PATHS = [CONTRACT_URI, 'specs/cinematic-render-repro-cost-c2-multilayer-exr-decoder-correction.v0.1.json', CORRECTION_URI, 'research/2026-08-29-b61-e1-cinematic-render-repro-cost-protocol.md', 'research/2026-08-29-b61-e1-c1-terminal-observability-correction.md', 'research/2026-08-29-b61-e1-c2-multilayer-exr-decoder-correction.md', 'research/2026-08-29-b61-e1-c3-isolated-png-review-correction.md', 'experiments/b61-exr-reopen-reconciliation-v0-1/result.json', 'experiments/b61-exr-reopen-reconciliation-v0-1/receipt.json', 'experiments/b61-png-export-context-diagnostic-v0-1/result.json', 'experiments/b61-png-export-context-diagnostic-v0-1/receipt.json', 'blender/render_b61_frames.py', 'blender/audit_b61_exr.py', 'scripts/preflight-b61-e1-cinematic-render-repro-cost.mjs', 'scripts/run-b61-e1-cinematic-render-repro-cost.mjs', 'scripts/audit-b61-e1-cinematic-render-repro-cost.mjs'];
...
export async function runB61(argv) {
  ...
  const contractPath = await resolveExistingRepositoryPath(CONTRACT_URI, 'B61 contract'); const contract = JSON.parse(await readFile(contractPath, 'utf8')); const correction = JSON.parse(await readFile(await resolveExistingRepositoryPath(CORRECTION_URI, 'B61 C3 correction'), 'utf8')); const preflight = JSON.parse(await readFile(preflightPath, 'utf8'));
  if (correction.status !== 'PREREGISTERED' || correction.authorizedRetryRoots.preflight !== parsed.preflightRoot || correction.authorizedRetryRoots.attempt !== parsed.attemptRoot || correction.authorizedRetryRoots.formal !== parsed.formalRoot) throw new Error('B61 C3 retry-root binding mismatch');
