...
const TOOL_PATHS = [CONTRACT_URI, 'research/2026-08-29-b61-e1-cinematic-render-repro-cost-protocol.md', 'blender/render_b61_frames.py', 'blender/audit_b61_exr.py', 'scripts/preflight-b61-e1-cinematic-render-repro-cost.mjs', 'scripts/run-b61-e1-cinematic-render-repro-cost.mjs', 'scripts/audit-b61-e1-cinematic-render-repro-cost.mjs'];
...
export async function runB61(argv) {
  ...
  const contractPath = await resolveExistingRepositoryPath(CONTRACT_URI, 'B61 contract'); const contract = JSON.parse(await readFile(contractPath, 'utf8')); const preflight = JSON.parse(await readFile(preflightPath, 'utf8'));
  ...
  await durableMkdir(formalPath); await durableMkdir(resolve(formalPath, 'runs')); await durableMkdir(resolve(attemptPath, 'processes')); await writeDurableHashed(resolve(formalPath, 'formal-start.json'), { schemaVersion: 'bfs.cinematicRenderReproCostFormalStart.v0.1', sequence: 4, status: 'AUTHORIZED', attemptReceiptHash: attemptReceipt.receiptHash, formalRoot: parsed.formalRoot }, 'formalStartHash');
  ...
  try {
    for (const shot of contract.shots) for (const repetition of contract.render.repetitions) {
      ...
      const result = await run('/usr/bin/time', ['-lp', contract.runtime.blenderBinary, '--background', source, '--disable-autoexec', '--python', resolve(repositoryRoot, 'blender/render_b61_frames.py'), '--', '--repository-root', repositoryRoot, '--contract', contractPath, '--shot', shot.label, '--repetition', repetition, '--output-dir', output], env, contract.resourceCeilings.perRenderProcessTimeoutSeconds * 1000);
      const record = processRecord(id, result, phaseGate(result.stdout, source, ocio)); await writeDurableJson(resolve(attemptPath, 'processes', `${id}.json`), record); processes.push(record);
    ...
    const reopenPath = resolve(formalPath, 'exr-reopen-audit.json'); const reopen = await run(contract.runtime.blenderBinary, ['--background', '--factory-startup', '--disable-autoexec', '--python', resolve(repositoryRoot, 'blender/audit_b61_exr.py'), '--', '--repository-root', repositoryRoot, '--contract', contractPath, '--formal-root', formalPath, '--output', reopenPath], env, 120000);
    await writeDurableJson(resolve(attemptPath, 'processes', 'EXR-AUDITOR.json'), processRecord('EXR-AUDITOR', reopen, { usingFrozenOcio: reopen.stdout.includes(`Using OCIO=${ocio}`) })); if (reopen.exitCode !== 0) throw new Error(`B61 EXR auditor failed: ${reopen.stderr || reopen.stdout}`);
    const auditPath = resolve(formalPath, 'audit.json'); const auditor = await run(process.execPath, ['scripts/audit-b61-e1-cinematic-render-repro-cost.mjs', '--preflight-root', parsed.preflightRoot, '--attempt-root', parsed.attemptRoot, '--formal-root', parsed.formalRoot, '--output', `${parsed.formalRoot}/audit.json`], { ...env, PATH: '/opt/homebrew/bin:/usr/bin:/bin' }, 120000); await writeDurableJson(resolve(attemptPath, 'processes', 'NODE-AUDITOR.json'), processRecord('NODE-AUDITOR', auditor, {})); if (auditor.exitCode !== 0) throw new Error(`B61 Node auditor failed: ${auditor.stderr || auditor.stdout}`);
