#!/usr/bin/env python3
"""Execute the immutable 81-process B52-D11.1 formal matrix exactly once."""
...
def fail(output_root: Path, stage: str, record: dict, runs: list[dict]) -> None:
    payload = {"schemaVersion": "bfs.blenderNearestIntegerTemporalRecoveryRunFailure.v0.1", "stage": stage, "failedCell": record, "completedRuns": runs}
...
def main() -> None:
    ...
    if preflight.get("status") != "ACCEPTED" or preflight.get("spec", {}).get("sha256") != SPEC_SHA256 or preflight.get("allFrozenToolsMatchGit") is not True:
        raise RuntimeError("B52-D11.1 frozen preflight not accepted")
    output_root = root / spec["formalOutputRoot"]
    if output_root.exists():
        raise RuntimeError("refusing to overwrite immutable D11.1 formal root")
    available = shutil.disk_usage(root).free
    projected_after = available - spec["projectedWriteBytes"]
    if projected_after < spec["diskReserveBytes"]:
        raise RuntimeError("B52-D11.1 disk admission rejected")
    ...
    for name in ("sources", "adapters", "quantizers", "accumulators", "encoders", "bridges"):
    ...
    tools = {
        "source": root / "blender/render_b52_d11_1_textured_source.py",
        "adapter": root / "scripts/adapt-b52-d11-1-multipart.py",
        "pythonQuantizer": root / "scripts/quantize-b52-d11-1-motion.py",
        "nodeQuantizer": root / "scripts/quantize-b52-d11-1-motion.mjs",
        "pythonAccumulator": root / "scripts/accumulate-b52-d11-1-temporal.py",
        "nodeAccumulator": root / "scripts/accumulate-b52-d11-1-temporal.mjs",
        "encoder": root / "scripts/encode-b52-d11-1-resolved.py",
        "bridge": root / "blender/render_b52_d11_1_resolved_passthrough.py",
        "analyzer": root / "scripts/analyze-b52-d11-1-nearest-integer-recovery.py",
    }
    runs: list[dict] = []
    ...
    for fixture in spec["fixtures"]:
        for source_repeat in (1, 2):
            for frame in (0, 1):
                ...
                with tempfile.TemporaryDirectory(prefix="bfs-d11-1-config-") as config, tempfile.TemporaryDirectory(prefix="bfs-d11-1-scripts-") as scripts:
    ...
    for fixture in spec["fixtures"]:
        for source_repeat in (1, 2):
            ...
            quantizer_records = {}
            for producer, executable, tool_key in (("python", python, "pythonQuantizer"), ("node", node, "nodeQuantizer")):
                quantizer_cell = output_root / "quantizers" / producer / cell_id
                quantized_motion = quantizer_cell / "motion-quantized.xy32"
                quantizer_report = quantizer_cell / "quantizer.report.json"
                argv = [executable, str(tools[tool_key]), "--spec", str(spec_path), "--fixture", fixture_id, "--repeat", str(source_repeat), "--input", str(adapter_arrays / "motion.xy32"), "--adapter-report", str(adapter_report), "--output", str(quantized_motion), "--report", str(quantizer_report)]
                completed = run(argv, root, environment(spec), quantizer_cell / "stdout.log", quantizer_cell / "stderr.log")
                record = record_report(f"QUANTIZER_{producer.upper()}", f"{producer}_{cell_id}", argv, completed, quantizer_report, root)
                record.update({"fixtureId": fixture_id, "sourceRepeat": source_repeat, "producer": producer, "outputUri": str(quantized_motion)})
                if completed.returncode != 0 or not quantized_motion.is_file() or not quantizer_report.is_file():
                    fail(output_root, record["stage"], record, runs)
                record["outputSha256"] = sha(quantized_motion)
                runs.append(record)
                quantizer_records[producer] = record
            ...
            for producer, executable, tool_key in (("python", python, "pythonAccumulator"), ("node", node, "nodeAccumulator")):
                ...
                quantizer_record = quantizer_records[producer]
                argv = [executable, str(tools[tool_key]), "--spec", str(spec_path), "--fixture", fixture_id, "--repeat", str(source_repeat), "--input-dir", str(adapter_arrays), "--adapter-report", str(adapter_report), "--quantized-motion", quantizer_record["outputUri"], "--quantizer-report", quantizer_record["reportUri"], "--output-dir", str(accumulator_arrays), "--report", str(accumulator_report)]
    ...
    counts = {stage: sum(row["stage"] == stage for row in runs) for stage in ("SOURCE", "ADAPTER", "QUANTIZER_PYTHON", "QUANTIZER_NODE", "ACCUMULATOR_PYTHON", "ACCUMULATOR_NODE", "ENCODER", "BRIDGE")}
    receipt_body = {
        "schemaVersion": "bfs.blenderNearestIntegerTemporalRecoveryRunReceipt.v0.1",
        "experimentId": spec["experimentId"],
        "runnerPid": os.getpid(),
        "preregistration": {"commit": "08543cf17371966c9bb4963f8b78158906fe1f2f", "specUri": str(spec_path.relative_to(root)), "specSha256": SPEC_SHA256},
    ...
    print(f"BFS_B52_D11_1_RUN_OK verdict={result['verdict']} result={sha(result_path)} receipt={sha(receipt_path)}")
