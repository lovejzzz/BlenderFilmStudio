#!/usr/bin/env python3
"""Independently analyze the frozen B52-D10 real-Blender adapter holdout."""
...
def object_location(spec: dict, fixture: dict, name: str, frame: int) -> list[float]:
    ...
    if name == "BFS_MOVER" and fixture["moverByFrame"] is not None:
...
def expected_action(values: dict[str, list[float]] | None) -> list[dict]:
    if values is None:
        return []
    return [
        {
            ...
            "dataPath": "location",
            "arrayIndex": axis,
            "keyframes": [{"frame": float(frame), "value": float(location[axis]), "interpolation": "LINEAR"} for frame, location in values.items()],
        }
        for axis in range(3)
    ]


def expected_scene_structure(spec: dict, fixture: dict, frame: int, repeat: int) -> dict:
    camera_spec = spec["scene"]["camera"]
    return {
        "sceneName": f"BFS_D10_{fixture['id']}_F{frame}_R{repeat}",
        "frame": frame,
        "camera": {"name": camera_spec["name"], "location": [float(v) for v in camera_location(spec, fixture, frame)], "rotationEuler": [float(v) for v in camera_spec["rotationEuler"]], "type": camera_spec["type"], "orthoScale": float(camera_spec["orthoScale"])},
        "objects": sorted([
            {"name": item["name"], "type": "MESH", "location": [float(v) for v in object_location(spec, fixture, item["name"], frame)], "scale": [float(v) for v in item["scale"]], "passIndex": int(item["passIndex"])}
...
def diagnostic_pixels(kind: str, current: dict) -> np.ndarray:
    if kind == "current-combined":
        rgb = np.clip(current["parts"]["BFS_MASTER.Combined"][..., :3], 0.0, 1.0)
    elif kind == "current-depth":
        depth = current["parts"]["BFS_MASTER.Depth"][..., 0].astype(np.float64)
        value = np.clip((depth - 6.0) / 4.0, 0.0, 1.0)
        rgb = np.stack([value, value * value, 1.0 - value], axis=-1)
    elif kind == "current-ownership":
        owner = current["parts"]["BFS_MASTER.Object Index"][..., 0].astype(np.int64)
        rgb = np.stack([((owner * 37) % 255) / 255.0, ((owner * 73) % 255) / 255.0, ((owner * 109) % 255) / 255.0], axis=-1)
    else:
        vector = current["parts"]["BFS_MASTER.Vector"][..., :2].astype(np.float64)
...
def main() -> None:
    ...
    if args.output.exists():
        raise RuntimeError("refusing to overwrite D10 formal result")
    ...
    for record in receipt["sourceRuns"]:
        ...
        source_state &= bool(
            report["runtime"] == {"engine": "CYCLES", "device": "CPU", "samples": 1, "seed": 20260827, "animatedSeed": False, "adaptiveSampling": False, "denoising": False, "motionBlur": False, "persistentData": False, "threadsMode": "FIXED", "threads": 4}
            and report["passState"] == {"viewLayer": "BFS_MASTER", "Combined": True, "Depth": True, "Vector": True, "Object Index": True, "passAlphaThreshold": 0.5}
        )
        scene_state &= report["sceneStructure"] == expected_scene_structure(spec, fixture, record["frame"], record["repeat"])
        action_state &= report["animationStructure"] == {"camera": expected_action(fixture["cameraByFrame"]), "mover": expected_action(fixture["moverByFrame"])}
        expected_shapes = {"BFS_MASTER.Combined": [height, width, 4], "BFS_MASTER.Depth": [height, width, 1], "BFS_MASTER.Vector": [height, width, 4], "BFS_MASTER.Object Index": [height, width, 1]}
    ...
    for fixture_id, fixture in fixtures.items():
        for repeat in (1, 2):
            for frame in (0, 1):
                ...
                owner = loaded["parts"]["BFS_MASTER.Object Index"][..., 0].astype(np.float64)
                ...
                one_orientation = centroids.get("BFS_TOP_MARKER", math.inf) < centroids.get("BFS_BOTTOM_MARKER", -math.inf)
                orientation_ok &= one_orientation

            current = source_cells[(fixture_id, 1, repeat)]["loaded"]
            motion = current["parts"]["BFS_MASTER.Vector"].astype(np.float64)
            owner = current["parts"]["BFS_MASTER.Object Index"][..., 0].astype(np.float64)
    ...
    for record in receipt["adapterRuns"]:
        ...
        current = source_cells[(fixture_id, 1, repeat)]["loaded"]
        expected = {
            "previousRgba": previous["parts"]["BFS_MASTER.Combined"],
            "currentRgba": current["parts"]["BFS_MASTER.Combined"],
            "previousDepth": previous["parts"]["BFS_MASTER.Depth"][..., 0],
            "currentDepth": current["parts"]["BFS_MASTER.Depth"][..., 0],
            "previousLayer": previous["parts"]["BFS_MASTER.Object Index"][..., 0],
            "currentLayer": current["parts"]["BFS_MASTER.Object Index"][..., 0],
            "motion": np.negative(current["parts"]["BFS_MASTER.Vector"][..., :2], dtype=np.float32),
        }
    ...
    if diagnostics_root.exists():
        raise RuntimeError("refusing to overwrite D10 diagnostics")
    ...
    for fixture_id in fixtures:
        ...
        for kind in spec["diagnostics"]["perFixture"]:
            ...
            record = {"schemaVersion": "bfs.blenderMultipartTemporalAdapterDiagnostic.v0.1", "experimentId": spec["experimentId"], "fixtureId": fixture_id, "kind": kind, "sourceExrSha256": source_sha, **write_png(png_path, pixels)}
    ...
    checks = {
        ...
        "DEVELOPMENT_OBSERVATION_IDENTITY": sha256_file(Path(spec["parents"]["d10DevelopmentObservation"]["uri"])) == spec["parents"]["d10DevelopmentObservation"]["sha256"],
    ...
    body = {
        "schemaVersion": "bfs.blenderMultipartTemporalAdapterHoldoutResult.v0.1",
        ...
        "checks": checks,
        "measurements": {"vector": vector_rows, "staticVector": static_rows, "depth": depth_rows, "ownership": ownership_rows, "sourceRepeats": repeat_rows, "adapterRepeats": adapter_repeat_rows},
    ...
    print(f"BFS_B52_D10_ANALYSIS verdict={verdict} failure={base_failure} attacks={sum(item['passed'] for item in attack_audit)}/{len(spec['attacks'])} result={sha256_file(args.output)}")
