#!/usr/bin/env python3
"""Independently audit the attempt-63 minimum-timesteps moving-liquid test."""
...
def transformed_source():
    if hashlib.sha256(BASE.read_bytes()).hexdigest() != EXPECTED_BASE_SHA256:
        raise RuntimeError("moving-liquid solver-timesteps auditor base identity mismatch")
    ...
    config_after = config_before + ' and result["configuration"]["timestepsMin"] == 2 and result["configuration"]["timestepsMax"] == 4 and abs(result["configuration"]["cflCondition"] - 2.0) <= 1e-6'
    replacements = [
        ('"""Independently audit the attempt-59 2.0-cell moving-liquid test."""', '"""Independently audit the attempt-63 minimum-timesteps moving-liquid test."""', "docstring"),
        ...
        ('"scripts/run-rc6-moving-liquid-effector-distance.py"', '"scripts/run-rc6-moving-liquid-solver-timesteps.py"', "runner"),
        ...
        ('"bfs.rc6MovingLiquidEffectorDistanceIndependentAudit.v0.1"', '"bfs.rc6MovingLiquidSolverTimestepsIndependentAudit.v0.1"', "audit schema"),
        ('"RC6_MOVING_LIQUID_EFFECTOR_DISTANCE_AUDIT="', '"RC6_MOVING_LIQUID_SOLVER_TIMESTEPS_AUDIT="', "audit marker"),
    ]
    for before, after, label in replacements:
        expected = 2 if label == "fresh roots" else 1
        if source.count(before) != expected:
            raise RuntimeError(f"moving-liquid solver-timesteps auditor {label} target mismatch")
