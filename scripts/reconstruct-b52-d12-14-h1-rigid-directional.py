#!/usr/bin/env python3
"""Independent scalar Python consumer for B52-D12.14-H1."""
...
def main() -> None:
    ...
    if sha_file(cli.spec) != SPEC_SHA256 or cli.output_dir.exists() or cli.report.exists():
        raise RuntimeError("D12.14-H1 spec identity or fresh consumer output violation")
    ...
    if sha_file(Path(sys.executable)) != runtime["sha256"] or np.__version__ != runtime["numpy"]:
        raise RuntimeError("D12.14-H1 Python runtime identity mismatch")
    ...
    if adapter.get("reportHash") != canonical_hash(adapter_body) or adapter.get("fixtureId") != cli.fixture or adapter.get("repeat") != cli.repeat:
        raise RuntimeError("D12.14-H1 adapter report mismatch")
    arrays = {}
    for name, (filename, channels) in INPUTS.items():
        ...
        if sha_bytes(payload) != adapter["arrays"][name]["sha256"]:
            raise RuntimeError(f"D12.14-H1 input hash mismatch: {name}")
    ...
    print(f"BFS_B52_D1214H1_PYTHON fixture={cli.fixture} repeat={cli.repeat} accepted={int(masks['accepted'].sum())}")
