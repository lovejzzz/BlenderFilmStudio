...
async function main() {
  ...
  req(correction.authorizedChanges.retryRoot === RETAINED_C2_ROOT_URI && correctionC2.correctionId === 'B62-T2-E1-C2' && correctionC2.statusBeforeToolChange === 'PREREGISTERED' && correctionC2.retainedFailure.root === RETAINED_C2_ROOT_URI && correctionC2.authorizedChanges.retryRoot === ROOT_URI, 'C2 mismatch');
  ...
  const scoped = await git(['status', '--porcelain=v1', '--', SPEC_URI, PROTOCOL_URI, CORRECTION_URI, CORRECTION_PROTOCOL_URI, CORRECTION_C2_URI, CORRECTION_C2_PROTOCOL_URI, ...TOOLS]);
  ...
  const admission = await writeHashed(resolve(root, 'admission.json'), {
    schemaVersion: 'bfs.b62TerminalAnimaticAdmission.v0.1', experimentId: experiment.experimentId, status: 'ADMITTED', preregistrationCommit: PREREGISTRATION_COMMIT, correctionCommit: CORRECTION_COMMIT, correctionC2Commit: CORRECTION_C2_COMMIT, toolFreezeCommit: freeze,
    parentTree, retainedFailureTree: retainedTree, retainedC2FailureTree: retainedC2Tree, bindings: { tools: toolHashes, scene: { uri: experiment.parentEvidence.scene.uri, sha256: await hashFile(scene) }, runtime: { blender: await hashFile(experiment.runtime.blender.executable), ffmpeg: await hashFile(experiment.runtime.ffmpeg.executable), ffprobe: await hashFile(experiment.runtime.ffprobe.executable), ocio: await hashFile(pathFor(OCIO_URI)) } },
