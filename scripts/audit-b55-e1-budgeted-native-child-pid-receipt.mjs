...
// This auditor intentionally does not import the B55 preflight or runner.
...
function deleteAtPath(record, path) {
  const copy = structuredClone(record);
  let target = copy;
  for (const segment of path.slice(0, -1)) target = target[segment];
...
function nativePidReceiptSchemaExact(budget, operationBinding, spec) {
  const wrapperPid = operationBinding?.budget?.restrictedWrapperPid;
  return budget.documentType === 'BFS_BUDGETED_PROCESS_RESULT'
    && budget.version === '0.2.0'
    && budget.outcome === 'PASS'
    && budget.command === spec.runtime.blender.executable
    && budget.args?.includes('--background')
    && budget.args?.includes('--factory-startup')
    && budget.args?.includes('--disable-autoexec')
    && Number.isSafeInteger(budget.child?.pid)
    && budget.child.pid > 0
    && Number.isSafeInteger(wrapperPid)
    && budget.child.pid !== wrapperPid
    && budget.child.exitCode === 0
    && budget.child.signal === null
    && budget.child.spawnError === null
    && budget.termination?.requested === false
    && budget.termination?.awaited === true
    && operationBinding?.budget?.documentType === budget.documentType
    && operationBinding?.budget?.version === budget.version
    && operationBinding?.budget?.nativeChildPid === budget.child.pid
    && operationBinding?.budget?.nativeExitCode === budget.child.exitCode
    && operationBinding?.budget?.nativeSignal === budget.child.signal
    && operationBinding?.budget?.nativeSpawnError === budget.child.spawnError
    && operationBinding?.budget?.terminationRequested === budget.termination.requested;
}

async function supervisorMinimalityProbe(repositoryRoot, gitChildren) {
  const uri = 'scripts/lib/budgeted-process.mjs';
  const beforeResult = await runGit(['show', `${PREREGISTRATION_COMMIT}:${uri}`], repositoryRoot, gitChildren, 'SUPERVISOR_BEFORE');
  if (beforeResult.exitCode !== 0) return { exact: false, reason: 'PREREGISTERED_SOURCE_UNAVAILABLE' };
  const before = beforeResult.stdout;
  const after = await readFile(resolve(repositoryRoot, uri), 'utf8');
  let expected = before;
  const replacements = [
    [
...
async function auditPreflightPidProbes(preflight, repositoryRoot) {
  const expected = new Map([
    ['PASS_SELF_REPORT', { outcome: 'PASS', exitCode: 0, child: true }],
    ['CHILD_FAILED_SELF_REPORT', { outcome: 'CHILD_FAILED', exitCode: 7, child: true }],
    ['WALL_TIME_SELF_REPORT', { outcome: 'BUDGET_EXCEEDED', breach: 'WALL_TIME', child: true, terminated: true }],
    ['SPAWN_ERROR_NULL_PID', { outcome: 'CHILD_FAILED', child: false }],
  ]);
  const rows = [];
  for (const binding of preflight.pidProbes?.cases ?? []) {
    const contract = expected.get(binding.id);
    const reportPath = resolve(repositoryRoot, binding.reportUri ?? 'missing');
    const report = JSON.parse(await readFile(reportPath, 'utf8'));
    const observationPath = binding.observationUri ? resolve(repositoryRoot, binding.observationUri) : null;
    const observation = observationPath ? JSON.parse(await readFile(observationPath, 'utf8')) : null;
    const fileBindingsExact = await sha256File(reportPath) === binding.reportSha256
      && deepExact(report, binding.report)
      && (observationPath ? await sha256File(observationPath) === binding.observationSha256 && deepExact(observation, binding.observation) : binding.observationSha256 === null);
    const childExact = contract?.child
      ? Number.isSafeInteger(report.child?.pid) && report.child.pid > 0
        && observation?.pid === report.child.pid
        && observation?.ppid === preflight.pidProbes.preflightPid
      : report.child?.pid === null && observation === null && typeof report.child?.spawnError === 'string';
    const outcomeExact = report.documentType === 'BFS_BUDGETED_PROCESS_RESULT'
      && report.version === '0.2.0'
      && report.outcome === contract?.outcome
      && (!Object.hasOwn(contract ?? {}, 'exitCode') || report.child?.exitCode === contract.exitCode)
      && (!contract?.breach || report.breach?.reason === contract.breach)
      && (!contract?.terminated || report.termination?.requested === true && report.termination?.awaited === true);
    rows.push({ id: binding.id, fileBindingsExact, childExact, outcomeExact, exact: Boolean(contract) && fileBindingsExact && childExact && outcomeExact });
  }
  return { rows, exact: rows.length === 4 && expected.size === 4 && rows.every(row => row.exact) };
}

async function main() {
  const args = parseArguments(process.argv.slice(2));
  const specPath = resolve(args.spec ?? 'specs/budgeted-native-child-pid-receipt-correction.v0.1.json');
  ...
  const supervisorMinimality = await supervisorMinimalityProbe(repositoryRoot, gitChildren);
  ...
  const parentAuditRead = await readHashed(resolve(repositoryRoot, parent.audit.uri), 'auditHash');
  const parentReceiptRead = await readHashed(resolve(repositoryRoot, parent.receipt.uri), 'receiptHash');
  const falseParentGates = Object.entries(parentResultRead.record.gates ?? {}).filter(([, value]) => value !== true).map(([key]) => key);
  const parentExact = parentResultRead.valid && parentAuditRead.valid && parentReceiptRead.valid
    && parentResultRead.sha256 === parent.results.sha256 && parentResultRead.record.resultHash === parent.results.resultHash
    && parentAuditRead.sha256 === parent.audit.sha256 && parentAuditRead.record.auditHash === parent.audit.auditHash
    && parentReceiptRead.sha256 === parent.receipt.sha256 && parentReceiptRead.record.receiptHash === parent.receipt.receiptHash
    && parentResultRead.record.scientificVerdict === parent.scientificVerdict;
  const parentSingleGapExact = parentExact
    && Object.keys(parentResultRead.record.gates ?? {}).length === parent.frozenObservation.gateCount
    && Object.values(parentResultRead.record.gates ?? {}).filter(Boolean).length === parent.frozenObservation.passingGateCount
    && falseParentGates.length === 1
    && falseParentGates[0] === parent.frozenObservation.onlyFalseGate
    && parentResultRead.record.processEvidence?.nativeCompilePidBindingsAvailable === false
    && parentResultRead.record.processEvidence?.limitation === parent.frozenObservation.limitation
    && parentAuditRead.record.gates?.[parent.frozenObservation.onlyFalseGate] === false;
  const unchangedProductionExact = spec.intervention.unchangedProductionFiles.every(row => toolChecks.find(check => check.uri === row.uri)?.observed === row.sha256);
  ...
  const pidProbeAudit = await auditPreflightPidProbes(preflight, repositoryRoot);
  ...
  const supervisorChangeMinimal = supervisorMinimality.exact
    && preflight.supervisorMinimality?.exact === true
    && preflight.supervisorMinimality?.afterSha256 === supervisorMinimality.afterSha256
    && preflight.toolHashes?.['scripts/lib/budgeted-process.mjs'] === supervisorMinimality.afterSha256;
    ...
    for (const suffix of ['A', 'B']) {
      ...
      runAudits.push({
        ...
        nativeInvocationSemanticExact: budget.outcome === 'PASS'
          ...
          && budget.child?.exitCode === 0,
        nativePidReceiptSchemaExact: nativePidReceiptSchemaExact(budget, operationBinding, spec),
        ...
        receiptBudgetFileHashBindingExact: receipt.run?.budgetReport?.sha256 === await sha256File(budgetPath)
          && receipt.run?.budgetReport?.uri === repoUri(repositoryRoot, budgetPath),
        verification,
  ...
  const fourNativePidReceiptSchemasExact = runAudits.length === 4
    && runAudits.every(row => row.nativePidReceiptSchemaExact && row.receiptBudgetFileHashBindingExact);
  ...
  const nativePidBindingsExact = operationDraft.nativeCompilePidBindingsAvailable === true
    && fourNativePidReceiptSchemasExact;
  ...
  const pidAttackRow = runAudits[0];
  const pidAttackBinding = operationDraft.runBindings?.find(row => row.runId === pidAttackRow.runId);
  for (const attack of [
    { id: 'PID_MISSING', field: 'child.pid', record: deleteAtPath(pidAttackRow.budget, ['child', 'pid']) },
    { id: 'PID_NULL_ON_SUCCESS', field: 'child.pid', record: mutate(pidAttackRow.budget, ['child', 'pid'], null) },
    { id: 'PID_STRING', field: 'child.pid', record: mutate(pidAttackRow.budget, ['child', 'pid'], String(pidAttackRow.nativeChildPid)) },
    { id: 'PID_FRACTIONAL', field: 'child.pid', record: mutate(pidAttackRow.budget, ['child', 'pid'], pidAttackRow.nativeChildPid + 0.5) },
    { id: 'PID_NON_POSITIVE', field: 'child.pid', record: mutate(pidAttackRow.budget, ['child', 'pid'], 0) },
    { id: 'PID_EQUALS_WRAPPER', field: 'child.pid', record: mutate(pidAttackRow.budget, ['child', 'pid'], pidAttackBinding.budget.restrictedWrapperPid) },
    { id: 'BUDGET_RESULT_VERSION', field: 'version', record: mutate(pidAttackRow.budget, ['version'], '0.1.0') },
  ]) {
    attacks.push({
      id: attack.id,
      target: `${pidAttackRow.outputUri}/budget.report.json`,
      changedField: attack.field,
      rejected: !nativePidReceiptSchemaExact(attack.record, pidAttackBinding, spec),
    });
  }
  const pidChangedWithoutReceipt = mutate(pidAttackRow.budget, ['child', 'pid'], pidAttackRow.nativeChildPid + 1);
  const pidChangedSha256 = sha256Bytes(Buffer.from(`${JSON.stringify(pidChangedWithoutReceipt, null, 2)}\n`));
  attacks.push({
    id: 'PID_REPORT_CHANGED_WITHOUT_RECEIPT_HASH',
    target: `${pidAttackRow.outputUri}/compile.receipt.json`,
    changedField: 'bound budget.report.json child.pid',
    rejected: pidChangedSha256 !== pidAttackRow.receipt.run?.budgetReport?.sha256,
  });
  ...
  const gates = {
    ...
    B54_SINGLE_PID_GAP_BOUND_EXACT: parentSingleGapExact,
    SUPERVISOR_CHANGE_MINIMAL_AND_SCHEMA_V0_2: supervisorChangeMinimal,
    ZERO_BLENDER_PID_PROBE_PREFLIGHT_ACCEPTED_AND_PUSHED: preflightAcceptedPushed,
    PID_PROBE_PASS_FAIL_BREACH_AND_SPAWN_ERROR_EXACT: pidProbeAudit.exact,
    ...
    FOUR_NATIVE_PID_RECEIPT_SCHEMAS_EXACT: fourNativePidReceiptSchemasExact,
    ...
    INDEPENDENT_AUDIT_AND_SEMANTIC_ATTACKS_MINIMUM_32: semanticAttacksExact,
  ...
  const auditBody = {
    schemaVersion: 'bfs.budgetedNativeChildPidReceiptIndependentAudit.v0.1',
    experimentId: 'B55-E1',
    status: 'PASS',
    ...
    identities: {
      ...
      parentExact,
      parentSingleGapExact,
      falseParentGates,
      supervisorMinimality,
      supervisorChangeMinimal,
      unchangedProductionExact,
      nodeExact,
      blenderBinaryExact,
      toolRosterExact,
    ...
    processReplay: {
      ...
      semanticCountsExact,
      nativePidBindingsExact,
      nativePidEvidenceBoundary: nativePidBindingsExact ? 'SUPERVISOR_LOCAL_SPAWN_PID_CORROBORATED_BY_CHILD_AUTHORED_PREFLIGHT' : 'NATIVE_PID_SCHEMA_OR_BINDING_FAILED',
  ...
  process.stdout.write(`BFS_B55_E1_AUDIT PASS expected=${expectedScientificVerdict} gates=${audit.gatePassed}/${audit.gateTotal} attacks=${semanticAttacksPassed}/${attacks.length}\n`);
}
