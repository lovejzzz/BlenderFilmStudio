#!/usr/bin/env python3
"""Execute the immutable 65-process B52-D12 formal matrix exactly once."""
...
def main():
    parser = argparse.ArgumentParser(); parser.add_argument("--spec", type=Path, required=True); parser.add_argument("--preflight", type=Path, required=True); args = parser.parse_args()
    root = Path.cwd().resolve(); spec_path, preflight_path = args.spec.resolve(), args.preflight.resolve()
    spec, preflight = json.loads(spec_path.read_text()), json.loads(preflight_path.read_text())
    if sha(spec_path) != SPEC_SHA256: raise RuntimeError("B52-D12 spec identity mismatch")
    if preflight.get("status") != "ACCEPTED" or preflight.get("spec", {}).get("sha256") != SPEC_SHA256 or preflight.get("allFrozenToolsMatchGit") is not True: raise RuntimeError("B52-D12 frozen preflight not accepted")
    output_root = root / spec["formalOutputRoot"]
    if output_root.exists(): raise RuntimeError("refusing to overwrite immutable D12 formal root")
    available = shutil.disk_usage(root).free; projected_after = available - spec["projectedWriteBytes"]
    if projected_after < spec["diskReserveBytes"]: raise RuntimeError("B52-D12 disk admission rejected")
    ...
    tools = {"source": root / "blender/render_b52_d12_projective_source.py", "adapter": root / "scripts/adapt-b52-d12-projective-multipart.py", "pythonReconstructor": root / "scripts/reconstruct-b52-d12-subpixel.py", "nodeReconstructor": root / "scripts/reconstruct-b52-d12-subpixel.mjs", "encoder": root / "scripts/encode-b52-d12-reconstruction.py", "bridge": root / "blender/render_b52_d12_reconstruction_passthrough.py", "analyzer": root / "scripts/analyze-b52-d12-projective-subpixel-holdout.py"}
    runs = []
    ...
    receipt_body = {"schemaVersion": "bfs.blenderProjectiveSubpixelRunReceipt.v0.1", "experimentId": spec["experimentId"], "runnerPid": os.getpid(), "preregistration": {"commit": PREREGISTRATION_COMMIT, "specUri": str(spec_path.relative_to(root)), "specSha256": SPEC_SHA256}, "preflight": {"uri": str(preflight_path), "sha256": sha(preflight_path), "status": preflight["status"], "freezeCommit": preflight["freezeCommit"]}, "tools": preflight["tools"], "diskAdmission": disk, "runs": runs, "preAnalysisOperationCounts": {"totalChildProcesses": len(runs), "uniqueChildPids": len(set(preanalysis_pids)), "stageCounts": stage_counts}}
    ...
    result = json.loads(result_path.read_text()); print(f"BFS_B52_D12_RUN_OK verdict={result['verdict']} result={sha(result_path)} receipt={sha(receipt_path)}")
