#!/usr/bin/env node
/** Independent Node audit for the B52-D12.10-C1 correction result. */
...
function resultOk(candidate, d1, spec) {
  if (!selfOk(candidate, "analysisHash")) return false;
...
if (statSync(paths.output, { throwIfNoEntry: false })) throw new Error("refusing to overwrite D12.10-C1 audit");
...
const attackMutations = [
  ["analysis hash", (value) => { value.analysisHash = "0".repeat(64); }],
...
const checks = [
  ...
  ["RESULT_SELF_HASH", selfOk(result, "analysisHash")],
...
const body = {
  schemaVersion: "bfs.blenderTemporalOwnerSupportLocalizationCorrectionAudit.v0.1",
