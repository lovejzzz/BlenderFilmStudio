...
"""Product-side PC6 validation actions run by the built Film Studio Engine."""
...
def run_negative(root, spec_uri, evidence):
    ...
    status = "PASS" if len(cases) == 9 and all(row["observed"] == row["expected"] and row["sceneUnchanged"] for row in cases) else "FAIL"
    result = {"schemaVersion": "bfs.pc6NegativeControls.v0.1", "status": status, "sceneMutations": 0, "cases": cases}
    write_json(evidence / "negative-controls.json", result)
    print("PC6_NEGATIVE=" + json.dumps(result, sort_keys=True, separators=(",", ":")))
    if status != "PASS":
        raise RuntimeError("PC6 negative controls failed")
...
def run_build(root, spec_uri, evidence, work):
    ...
    inspection_state = {
        ...
        "actorFactory": state.causal_actor_factory,
        "targetFactory": state.causal_target_factory,
        "targetCount": state.causal_target_count,
        "collisionShapes": state.causal_collision_shapes,
        "finalPoseSource": state.causal_final_pose_source,
        "cameraFitSource": state.causal_camera_fit_source,
    }
    ...
    release = json.loads((root / spec_uri).read_text(encoding="utf-8"))["timeline"]["releaseFrame"]
    ...
    blend = work / "PC6_CAUSAL_PRODUCT.blend"
    bpy.ops.wm.save_as_mainfile(filepath=str(blend), check_existing=False)
    output = {
        "schemaVersion": "bfs.pc6CausalProductBuild.v0.1",
        "status": "PASS",
        "inspection": inspection_state,
        "productStatus": state.causal_status,
        "productSummary": state.causal_result_summary,
        "physics": result["physics"],
        "framing": result["framing"],
        "provenance": result["provenance"],
        "semanticRoster": result["semanticRoster"],
        "animation": {
            "actorPoseFrames": actor_pose_frames,
            "actorPoseFramesAfterRelease": [frame for frame in actor_pose_frames if frame >= release],
    ...
    print("PC6_BUILD=" + json.dumps(output, sort_keys=True, separators=(",", ":")))
...
def run_reopen(root, spec_uri, evidence):
    ...
    result = {
        "schemaVersion": "bfs.pc6CausalProductReopen.v0.1",
        "status": "PASS" if response_exact and max(tilt_delta.values()) <= document["acceptance"]["reopenFinalTiltToleranceDegrees"] else "FAIL",
        "responseFramesExact": response_exact,
        "finalTiltDeltaDegrees": tilt_delta,
        "physics": physics,
        "storedFinalPoseSource": saved["provenance"]["finalPoseSource"],
        "networkCalls": 0,
    }
    write_json(evidence / "reopen.json", result)
    print("PC6_REOPEN=" + json.dumps(result, sort_keys=True, separators=(",", ":")))
    if result["status"] != "PASS":
        raise RuntimeError("PC6 reopen mismatch")
