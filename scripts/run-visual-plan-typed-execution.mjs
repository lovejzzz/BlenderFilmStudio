...
requireCondition(context.schemaVersion === 'bfs.visualImprovementExecutionContext.v0.1' && context.contextHash === selfHash(context, 'contextHash'), 'context self hash');
const freezePath = safeRepositoryPath(FREEZE_URI);
const freezeBytes = await readFile(freezePath);
const freeze = JSON.parse(freezeBytes);
requireCondition(freeze.schemaVersion === 'bfs.visualPlanTypedExecutionToolFreeze.v0.1' && freeze.freezeHash === selfHash(freeze, 'freezeHash'), 'freeze self hash');
