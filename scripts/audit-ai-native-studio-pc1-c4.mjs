...
const FREEZE_URI = 'specs/ai-native-studio-pc1-tool-freeze-c4.v0.4.json';
const ROOT_URI = 'experiments/ai-native-studio-post-pb7/PC.1-2026-08-31-mac-m2max-attempt-04';
const WORK_ROOT = '/Users/mengyingli/Documents/ChatGPT/BlenderFilmStudio-PostPB7-workspace/PC.1-2026-08-31-mac-m2max-attempt-04';
...
function pythonNumber(value) {
  if (!Number.isFinite(value)) throw new Error('NONFINITE_PYTHON_CANONICAL');
  if (Number.isInteger(value)) return String(value);
  return String(value).replace(/e([+-]?)(\d+)$/i, (_, sign, digits) => `e${sign || ''}${digits.padStart(2, '0')}`);
}
function pythonCanonical(value) {
  if (value === null) return 'null';
  if (Array.isArray(value)) return `[${value.map(pythonCanonical).join(',')}]`;
  if (typeof value === 'object') return `{${Object.keys(value).sort().map(key => `${JSON.stringify(key)}:${pythonCanonical(value[key])}`).join(',')}}`;
  if (typeof value === 'number') return pythonNumber(value);
  return JSON.stringify(value);
}
const validPythonSelf = (v, f) => v?.[f] === sha(pythonCanonical(without(v, f)));
export { pythonCanonical, validPythonSelf };
...
async function auditPc1(argv = process.argv.slice(2)) {
  ...
  gate('SELF_HASHES', validSelf(spec, 'specHash') && validSelf(freeze, 'specHash') && validPythonSelf(build, 'buildHash') && validSelf(semantic, 'auditHash') && validSelf(receipt, 'receiptHash') && processes.every(row => validSelf(row, 'processHash')), { spec: spec.specHash, freeze: freeze.specHash, build: build.buildHash, buildCanonicalizer: 'PYTHON_JSON_FINITE_NUMBER_V0_1', semantic: semantic.auditHash, receipt: receipt.receiptHash });
