...
void fixtures(const std::string& directory) {
  ...
  for(const std::string mode:{"base","position","point_velocity","flag","grid_velocity","phi_sign","half","truncated","missing_attribute","nonfinite","bad_dimensions","bad_transform"}){
    ...
    auto points=mode=="truncated" ? v::points::createPointDataGrid<v::points::TruncateCodec,Points>(p,*transform) : v::points::createPointDataGrid<v::points::NullCodec,Points>(p,*transform);points->setName("particles");
    if(mode=="truncated") v::points::appendAttribute<int,v::points::TruncateCodec>(points->tree(),"U",1);
    else v::points::appendAttribute<int>(points->tree(),"U",mode=="flag"?2:1);
    if(mode=="truncated")v::points::appendAttribute<v::Vec3f,v::points::TruncateCodec>(points->tree(),"particles_velocity",v::Vec3f(1,-2,.5f));
    else if(mode!="missing_attribute")v::points::appendAttribute<v::Vec3f>(points->tree(),"particles_velocity",v::Vec3f(mode=="point_velocity"?1.25f:1.f,-2,.5f));
  ...
  std::cout<<"{\"fixtures\":12}\n";
}
int main(int argc,char** argv){
  try {
    v::initialize();
    v::points::TypedAttributeArray<int,v::points::TruncateCodec>::registerType();
    v::points::TypedAttributeArray<float,v::points::TruncateCodec>::registerType();
    v::points::TypedAttributeArray<v::Vec3f,v::points::TruncateCodec>::registerType();
