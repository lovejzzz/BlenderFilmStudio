#!/usr/bin/env python3
"""C7-C1 scene adapter: unchanged S08 impulse on the corrected 2 mm margin."""
...
if hashlib.sha256(BASE.read_bytes()).hexdigest() != EXPECTED_BASE_SHA256:
    raise RuntimeError("real-impact C7-C1 scene base identity mismatch")
source = BASE.read_text(encoding="utf-8")
replacements = (
    ('DRIVE_ENDS = {"M02": 9}', 'DRIVE_ENDS = {"S08": 8}', 1, "single impulse cell"),
    ("C6 I09 scene adapter", "C7-C1 corrected-margin I08 scene adapter", 1, "description"),
    ("#C6_CUP_MARGIN_2MM", "#C7_C1_CORRECTED_MARGIN_I08", 1, "compile identity"),
)
for before, after, expected, label in replacements:
    if source.count(before) != expected:
        raise RuntimeError(f"real-impact C7-C1 scene {label} target mismatch")
