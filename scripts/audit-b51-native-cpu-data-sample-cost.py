"""Independent frozen-tool and analyzer-replay audit for B51-D5."""
...
def main() -> None:
    ...
    with tempfile.TemporaryDirectory(prefix="bfs-b51-d5-audit-") as temporary:
        replay = Path(temporary) / "results.json"
        process = subprocess.run([sys.executable, str(analyzer), "--spec", str(args.spec), "--receipt", str(output_root / "run.receipt.json"), "--output", str(replay)], cwd=root, capture_output=True, text=True, check=False)
        if process.returncode:
            raise RuntimeError(f"B51-D5 analyzer replay failed: {process.stdout}\n{process.stderr}")
    ...
    for run in receipt["runs"]:
        ...
        observed = sha256_file(path) if path.is_file() else None
        artifacts.append({"runId": run["runId"], "uri": str(path.relative_to(root)), "expectedSha256": report["artifact"]["sha256"], "observedSha256": observed, "expectedBytes": report["artifact"]["bytes"], "observedBytes": path.stat().st_size if path.is_file() else None, "match": observed == report["artifact"]["sha256"] and path.is_file() and path.stat().st_size == report["artifact"]["bytes"]})

    passed = replay_exact and all(item["match"] for item in frozen_tools + bound_inputs + artifacts) and result["verdict"] in spec["validVerdicts"] and result["attacksPassed"] == len(spec["attacks"])
    audit = {
        "schemaVersion": "bfs.nativeCpuDataPassSampleCostAudit.v0.1",
        "status": "PASS" if passed else "FAIL",
        "verdict": result["verdict"],
        "exactDataSampleFloor": result["exactDataSampleFloor"],
    ...
    print(f"BFS_B51_D5_AUDIT {audit['status']} replayExact={replay_exact} artifacts={sum(item['match'] for item in artifacts)}/{len(artifacts)}", flush=True)
    if not passed:
        raise SystemExit(1)
