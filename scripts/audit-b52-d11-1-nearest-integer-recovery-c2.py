...
CORRECTION_PROTOCOL = "research/2026-08-27-b52-d11-1-c2-receipt-hash-literal-correction.md"
CORRECTION_PROTOCOL_SHA256 = "702d719a1b9d67d0e5319809fd4853bd0d956a0c270d47843f5990677e36dad6"
CORRECTION_PREREGISTRATION_COMMIT = "ac43e9074175a5b6a4192deb826ab448899efc23"
...
RECEIPT_SHA256 = "643717651d4dafb48c87a5925391f06ef30ce97f62a8ab321d4c4aba62d0f443"
C1_TOOL = "scripts/audit-b52-d11-1-nearest-integer-recovery-c1.py"
C1_TOOL_SHA256 = "3acce75131ccbbe8c240d1e8dcd362a7743d41f90340e1ed98739f2c557f90f6"
C1_TOOL_FREEZE_COMMIT = "e7a8ef5cb59e89f0fbc4462a032bf58adcdaf33c"
C2_TOOL = "scripts/audit-b52-d11-1-nearest-integer-recovery-c2.py"
...
def main() -> None:
    ...
    if sha(root / CORRECTION_PROTOCOL) != CORRECTION_PROTOCOL_SHA256 or sha(root / ORIGINAL_AUDIT) != ORIGINAL_AUDIT_SHA256:
        raise RuntimeError("B52-D11.1-C2 correction protocol or original audit identity mismatch")
    if sha(root / C1_TOOL) != C1_TOOL_SHA256:
        raise RuntimeError("B52-D11.1-C2 frozen C1 identity mismatch")
    if sha(result_path) != RESULT_SHA256 or sha(receipt_path) != RECEIPT_SHA256:
        raise RuntimeError("B52-D11.1-C2 immutable formal input identity mismatch")
    frozen_tool = subprocess.run(["git", "show", f"{args.correction_freeze_commit}:{C2_TOOL}"], cwd=root, capture_output=True, check=False)
    ...
    if frozen_tool.returncode != 0 or frozen_tool.stdout != (root / C2_TOOL).read_bytes():
        raise RuntimeError("B52-D11.1-C2 tool does not match its Git freeze commit")
    if frozen_protocol.returncode != 0 or hashlib.sha256(frozen_protocol.stdout).hexdigest() != CORRECTION_PROTOCOL_SHA256:
        raise RuntimeError("B52-D11.1-C2 preregistration Git blob mismatch")
    ...
    correction = {"protocol": {"uri": CORRECTION_PROTOCOL, "sha256": CORRECTION_PROTOCOL_SHA256, "preregistrationCommit": CORRECTION_PREREGISTRATION_COMMIT}, "originalAudit": {"uri": ORIGINAL_AUDIT, "sha256": ORIGINAL_AUDIT_SHA256}, "failedC1Tool": {"uri": C1_TOOL, "sha256": C1_TOOL_SHA256, "freezeCommit": C1_TOOL_FREEZE_COMMIT}, "c2Tool": {"uri": C2_TOOL, "sha256": sha(root / C2_TOOL), "freezeCommit": args.correction_freeze_commit}, "immutableFormalInputs": {"receiptSha256": RECEIPT_SHA256, "resultSha256": RESULT_SHA256}, "permittedChange": "correct malformed receipt SHA-256 literal plus C2 provenance identifiers; retain C1 native-bool cast"}
    body = {"schemaVersion": "bfs.blenderNearestIntegerTemporalRecoveryAuditC2.v0.1", "experimentId": spec["experimentId"], "status": status, "auditorPid": os.getpid(), "correction": correction, "spec": {"uri": str(args.spec), "sha256": sha(args.spec)}, "receipt": {"uri": str(receipt_path), "sha256": sha(receipt_path)}, "result": {"uri": str(result_path), "sha256": sha(result_path), "verdict": result["verdict"], "baseFailure": result["baseFailure"]}, "checks": checks, "replayedEvidence": evidence, "replay": replay_rows}
