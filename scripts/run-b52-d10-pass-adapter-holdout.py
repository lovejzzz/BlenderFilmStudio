#!/usr/bin/env python3
"""Run the immutable 19-process B52-D10 formal matrix exactly once."""
...
def main() -> None:
    ...
    if not (preflight.get("status") == "ACCEPTED" and preflight.get("spec", {}).get("sha256") == SPEC_SHA256 and preflight.get("allFrozenToolsMatchGit") is True):
        raise RuntimeError("frozen D10 preflight not accepted")
    output_root = root / spec["formalOutputRoot"]
    if output_root.exists():
        raise RuntimeError("refusing to overwrite immutable D10 formal root")
    available = shutil.disk_usage(root).free
    projected_after = available - spec["projectedWriteBytes"]
    if projected_after < spec["diskReserveBytes"]:
        raise RuntimeError("D10 disk admission rejected")
    ...
    renderer = root / "blender/render_b52_d10_pass_adapter_source.py"
    adapter = root / "scripts/adapt-b52-d10-multipart.py"
    analyzer = root / "scripts/analyze-b52-d10-pass-adapter-holdout.py"
    ...
    for fixture in spec["fixtures"]:
        for repeat in (1, 2):
            for frame in (0, 1):
                ...
                with tempfile.TemporaryDirectory(prefix="bfs-d10-config-") as config, tempfile.TemporaryDirectory(prefix="bfs-d10-scripts-") as scripts:
                ...
                if result.returncode != 0 or not exr.is_file() or not report.is_file():
                    (output_root / "run.failure.json").write_text(json.dumps({"schemaVersion": "bfs.blenderMultipartTemporalAdapterRunFailure.v0.1", "stage": "SOURCE", "failedCell": record, "sourceRuns": source_runs}, indent=2, sort_keys=True, allow_nan=False) + "\n", encoding="utf-8")
    ...
    for fixture in spec["fixtures"]:
        for repeat in (1, 2):
            ...
            if result.returncode != 0 or not report.is_file():
                (output_root / "run.failure.json").write_text(json.dumps({"schemaVersion": "bfs.blenderMultipartTemporalAdapterRunFailure.v0.1", "stage": "ADAPTER", "failedCell": record, "sourceRuns": source_runs, "adapterRuns": adapter_runs}, indent=2, sort_keys=True, allow_nan=False) + "\n", encoding="utf-8")
                raise SystemExit(result.returncode or 1)
    ...
    receipt_body = {
        "schemaVersion": "bfs.blenderMultipartTemporalAdapterRunReceipt.v0.1",
    ...
    if analysis.returncode != 0 or not result_path.is_file():
        (output_root / "analysis.failure.json").write_text(json.dumps({"schemaVersion": "bfs.blenderMultipartTemporalAdapterAnalysisFailure.v0.1", "exitCode": analysis.returncode, "argv": normalized(argv, root), "stdout": analysis.stdout, "stderr": analysis.stderr}, indent=2, sort_keys=True, allow_nan=False) + "\n", encoding="utf-8")
        raise SystemExit(analysis.returncode or 1)
    result = json.loads(result_path.read_text(encoding="utf-8"))
    print(f"BFS_B52_D10_RUN_OK verdict={result['verdict']} result={sha256_file(result_path)} receipt={sha256_file(receipt_path)}")
