...
def load_multipart(path: Path, render: dict) -> dict:
    ...
    for name, count in (("BFS_MASTER.Combined", 4), ("BFS_MASTER.Depth", 1), ("BFS_MASTER.Vector", 4), ("BFS_MASTER.Object Index", 1)):
...
def main() -> None:
    ...
    if args.output_dir.exists() or args.report.exists():
        raise RuntimeError("refusing to overwrite D10 adapter output")
    ...
    body = {
        "schemaVersion": "bfs.blenderMultipartTemporalAdapterReport.v0.1",
        ...
        "transform": {"source": "current BFS_MASTER.Vector.XY", "destination": "motion.xy32", "formula": "[-X,-Y]", "unusedValidatedPair": "current BFS_MASTER.Vector.ZW"},
        "arrays": records,
    ...
    print(f"BFS_B52_D10_ADAPTER_OK fixture={args.fixture} repeat={args.repeat} motion={records['motion']['sha256']}")
