...
function collectPreflight(spec) {
  ...
  const observed = {
    schemaVersion: 'bfs.pb1ValidationOnlyC2Preflight.v0.7',
    observedAt: new Date().toISOString(),
    status: 'PENDING',
    ...
    runner: { path: 'scripts/run-ai-native-studio-pb1-validation-only-c2.mjs', sha256: sha256File(SCRIPT_PATH) },
    authorization: {
  ...
  if (!observed.roots.externalAbsent || !observed.roots.evidenceAbsent) failures.push('FRESH_ATTEMPT03_ROOTS_REQUIRED');
...
function selfTest(spec) {
  ...
  add('CONTRACT_SCHEMA', ['bfs.pb1ValidationOnlyC2AuthorizationRequest.v0.6', 'bfs.pb1ValidationOnlyC2Execution.v0.7'].includes(spec.schemaVersion));
  ...
  add('FRESH_OBJECT_SYMLINK_FIXED', spec.lfsCorrection.freshObjectsSymlinkTargetMustBeRetainedObjectsSubtree === true && spec.lfsCorrection.retainedStorageWholeTreeMustRemainUnchangedDuringAttempt03 === true && spec.lfsCorrection.symlinkBeforePublicationCheckout === true);
  ...
  const output = { schemaVersion: 'bfs.pb1ValidationOnlyC2SelfTest.v0.7', status: failed.length ? 'FAIL' : 'PASS', checksPassed: checks.length - failed.length, checksTotal: checks.length, checks, failures: failed.map(item => item.id) };
...
async function execute(spec) {
  ...
  try {
    ...
    const negative = writeJsonExclusive(resolve(evidenceRoot, 'negative-controls.json'), { schemaVersion: 'bfs.pb1ValidationOnlyC2NegativeControls.v0.7', observedAt: new Date().toISOString(), status: controls.length === 9 && controls.every(item => item.pass) ? 'PASS' : 'FAIL', externalRootExisted: existsSync(externalRoot), checksPassed: controls.filter(item => item.pass).length, checksTotal: controls.length, controls, counters });
    ...
    tracked(commands, stage, 'disable LFS network URL', GIT, ['-C', source, 'config', 'lfs.url', 'file:///PB1-C2-LFS-NETWORK-DISABLED']);
    ...
    const lfsReceipt = writeJsonExclusive(resolve(evidenceRoot, 'lfs-materialization.json'), { schemaVersion: 'bfs.pb1ValidationOnlyC2LfsMaterialization.v0.7', observedAt: new Date().toISOString(), status: Object.values(lfsChecks).every(Boolean) ? 'PASS' : 'FAIL', localLfsRoot: localLfs, localObjects, retainedBefore: preflight.retainedLfs.wholeTree, retainedAfter: retainedAfterMaterialization, lfs, checks: lfsChecks });
    ...
    const historyReceipt = writeJsonExclusive(resolve(evidenceRoot, 'remote-and-history.json'), { schemaVersion: 'bfs.pb1ValidationOnlyC2RemoteHistory.v0.7', observedAt: new Date().toISOString(), status: Object.values(graphChecks).every(Boolean) ? 'PASS' : 'FAIL', remoteBefore: preflight.remote, source: sourceNow, metric, publication, fsckExitCode: fsck.exitCode, checks: graphChecks });
    ...
    const licenseReceipt = writeJsonExclusive(resolve(evidenceRoot, 'license-and-generated-paths.json'), { schemaVersion: 'bfs.pb1ValidationOnlyC2LicenseGenerated.v0.7', observedAt: new Date().toISOString(), status: Object.values(licenseChecks).every(Boolean) ? 'PASS' : 'FAIL', inventory, checks: licenseChecks });
    ...
    const dependencyReceipt = writeJsonExclusive(resolve(evidenceRoot, 'dependency.json'), { schemaVersion: 'bfs.pb1ValidationOnlyC2Dependency.v0.7', observedAt: new Date().toISOString(), status: Object.values(dependencyChecks).every(Boolean) ? 'PASS' : 'FAIL', dependency, checks: dependencyChecks });
    ...
    const build = await runLogged({ command: MAKE, args: [`BUILD_DIR=${buildRoot}`, 'NPROCS=12'], cwd: source, stdoutPath: buildStdout, stderrPath: buildStderr, timingPath: buildTiming, timeoutMs: spec.resources.maximumBuildWallSeconds * 1000, monitoredRoot: externalRoot, maximumBytes: spec.resources.maximumAttempt03ExternalRootBytes });
    ...
    const buildReceipt = writeJsonExclusive(resolve(evidenceRoot, 'build.json'), { schemaVersion: 'bfs.pb1ValidationOnlyC2Build.v0.7', observedAt: new Date().toISOString(), status: Object.values(buildChecks).every(Boolean) ? 'PASS' : 'FAIL', process: build, timing, artifact: binaryExists ? { app, binary, bytes: statSync(binary).size, sha256: sha256File(binary), fileIdentity, architectures, bundleId: plistRaw(plist, 'CFBundleIdentifier') } : null, logs: { stdout: { bytes: statSync(buildStdout).size, sha256: sha256File(buildStdout) }, stderr: { bytes: statSync(buildStderr).size, sha256: sha256File(buildStderr) }, timing: { bytes: statSync(buildTiming).size, sha256: sha256File(buildTiming) } }, checks: buildChecks });
    ...
    const runtimeReceipt = writeJsonExclusive(resolve(evidenceRoot, 'runtime-identity.json'), { schemaVersion: 'bfs.pb1ValidationOnlyC2Runtime.v0.7', observedAt: new Date().toISOString(), status: Object.values(runtimeChecks).every(Boolean) ? 'PASS' : 'FAIL', productStarts: counters.productStarts, renders: counters.renders, version: { exitCode: version.exitCode, output: versionOutput.trim() }, runtime: { exitCode: runtime.exitCode, elapsedSeconds: runtime.elapsedSeconds, payload, stdoutSha256: sha256File(runtimeStdout), stderrSha256: sha256File(runtimeStderr) }, configuration: { actualOfficial, officialBefore, officialAfter, isolatedOfficial, isolatedOfficialAfter, productRoot, productAfter, expectedPaths }, checks: runtimeChecks });
    ...
    const finalChecks = { receiptsPass: bindings.every(item => item.receiptHashPass && item.status !== 'FAIL'), operationsExact: counters.localEngineClones === 1 && counters.freshObjectsSymlinks === 1 && counters.localLfsMaterializations === 1 && counters.localDependencyClones === 1 && counters.nativeBuilds === 1 && counters.productStarts === 2, forbiddenZero: ['publicEngineNetworkClones','renders','engineRemoteWrites','engineRefUpdates','lfsNetworkDownloads','lfsUploads','releases','signing','notarization','dmg','pb2ThroughPb7','modelCalls'].every(name => counters[name] === 0), sourceExactClean: sourceIdentity(source).head === spec.publicationBaseline.head && sourceIdentity(source).tree === spec.publicationBaseline.tree && sourceIdentity(source).clean, remoteUnchanged: remoteAfter.main === spec.publicationBaseline.head, retainedStorageUnchanged: canonicalJson(preflight.retainedLfs.wholeTree) === canonicalJson(retainedFinal), externalWithin: treeBytes(externalRoot) <= spec.resources.maximumAttempt03ExternalRootBytes, evidenceWithin: treeBytes(evidenceRoot) <= spec.resources.maximumEvidenceRootBytes };
    const verdict = writeJsonExclusive(resolve(evidenceRoot, 'verdict.json'), { schemaVersion: 'bfs.pb1ValidationOnlyC2Verdict.v0.7', gate: 'PB.1', mode: 'VALIDATION_ONLY_C2', observedAt: new Date().toISOString(), status: Object.values(finalChecks).every(Boolean) ? 'PASS' : 'FAIL', publicationHead: spec.publicationBaseline.head, f0Parent: spec.publicationBaseline.soleParent, dependencyCommit: spec.dependency.commit, bindings, counters, resources: { externalBytes: treeBytes(externalRoot), evidenceBytesBeforeVerdict: treeBytes(evidenceRoot) }, checks: finalChecks, failures: Object.entries(finalChecks).filter(([, pass]) => !pass).map(([name]) => name), claimCeiling: 'PB.1 validation-only proves repository/source identity and one clean same-host arm64 build; it does not authorize PB.2-PB.7 or prove distribution, production readiness, cross-platform support, or autonomous filmmaking.', stopRulePreserved: true });
    if (verdict.status !== 'PASS') throw new Error(`VERDICT_FAILED:${verdict.failures.join(',')}`);
    process.stdout.write(`PB1_C2_VALIDATION_PASS receipt=${verdict.receiptHash} buildSeconds=${build.elapsedSeconds.toFixed(3)}\n`);
  } catch (error) {
    if (evidenceCreated) {
      const failurePath = resolve(evidenceRoot, 'failure.json');
      if (!existsSync(failurePath)) writeJsonExclusive(failurePath, { schemaVersion: 'bfs.pb1ValidationOnlyC2Failure.v0.7', gate: 'PB.1', status: 'FAIL', observedAt: new Date().toISOString(), failedStage: stage, error: String(error.message ?? error), stderr: error.stderr ?? null, counters, externalRootExists: existsSync(externalRoot), externalBytes: treeBytes(externalRoot), stopRulePreserved: true });
      const verdictPath = resolve(evidenceRoot, 'verdict.json');
      if (!existsSync(verdictPath)) writeJsonExclusive(verdictPath, { schemaVersion: 'bfs.pb1ValidationOnlyC2Verdict.v0.7', gate: 'PB.1', status: 'FAIL', observedAt: new Date().toISOString(), failedStage: stage, failureReceiptHash: JSON.parse(readFileSync(failurePath, 'utf8')).receiptHash, counters, stopRulePreserved: true });
    }
    process.stderr.write(`PB1_C2_VALIDATION_FAIL stage=${stage} error=${String(error.message ?? error)}\n`);
    process.exitCode = 1;
  }
}
