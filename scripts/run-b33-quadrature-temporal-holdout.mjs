...
const experimentRoot = resolve(repositoryRoot, 'experiments/quadrature-temporal-holdout-v0-1');
const evidenceRoot = resolve(experimentRoot, 'evidence');
const workRoot = resolve(experimentRoot, 'work');
const specPath = resolve(repositoryRoot, 'specs/quadrature-temporal-holdout-spec.v0.1.json');
...
const renderer = resolve(repositoryRoot, 'blender/render_b33_quadrature_temporal_holdout.py');
const analyzer = resolve(repositoryRoot, 'blender/analyze_b33_quadrature_temporal_holdout.py');
const runner = fileURLToPath(import.meta.url);
const blender = resolve(process.env.BLENDER_BIN || '/Applications/Blender.app/Contents/MacOS/Blender');
const derivationSpecPath = resolve(repositoryRoot, 'specs/quadrature-temporal-derivation-spec.v0.1.json');
const derivationResultPath = resolve(repositoryRoot, 'experiments/quadrature-temporal-derivation-v0-1/results.json');
const derivationAnalysisPath = resolve(repositoryRoot, 'experiments/quadrature-temporal-derivation-v0-1/evidence/temporal-analysis.json');
const derivationResearchPath = resolve(repositoryRoot, 'research/2026-08-26-b33-quadrature-temporal-derivation-result.md');
const expectedSpecSha = '79fe033683352c4b8295d47bfca112ace7c7a9440ffdaa992933999fb8468ddf';
...
failUnless(specSha === expectedSpecSha, 'B33 holdout spec changed after preregistration');
...
const fixedInputs = [
  ...
  [derivationSpecPath, spec.evidenceBasis.derivationSpecSha256, 'derivation spec'],
  [derivationResultPath, spec.evidenceBasis.derivationResultSha256, 'derivation result'],
  [derivationAnalysisPath, spec.evidenceBasis.derivationAnalysisSha256, 'derivation analysis'],
  [derivationResearchPath, spec.evidenceBasis.derivationResearchSha256, 'derivation research note'],
];
...
failUnless(derivationResult.decision === spec.evidenceBasis.derivationDecision, 'derivation frozen decision mismatch');
failUnless(derivationResult.status === spec.evidenceBasis.derivationStatus, 'derivation frozen status mismatch');
...
for (const replicateId of frozenSchedule) {
  ...
  const manifestBody = {
    documentType: 'BFS_B33_QUADRATURE_TEMPORAL_HOLDOUT_RENDER_MANIFEST',
  ...
  process.stdout.write(`BFS_B33_HOLDOUT_PROCESS_OK ${replicateId} pid=${launched.processId} seconds=${report.totalRenderSeconds}\n`);
}

failUnless(new Set([...records.values()].map(item => item.processId)).size === spec.design.totalProcesses,
  'B33 holdout official process IDs are missing or duplicated');
const totalRenderCalls = [...records.values()].reduce((sum, item) => sum + item.report.renderCalls, 0);
failUnless(totalRenderCalls === spec.design.totalRenderCalls, 'B33 holdout total render calls mismatch');

const ledgerBody = {
  documentType: 'BFS_B33_QUADRATURE_TEMPORAL_HOLDOUT_PROCESS_LEDGER',
  version: spec.version,
  holdoutSpecSha256: specSha,
...
const indexBody = {
  documentType: 'BFS_B33_QUADRATURE_TEMPORAL_HOLDOUT_ANALYSIS_INDEX',
  version: spec.version,
  holdoutSpecSha256: specSha,
...
const contractHash = sha256Canonical({
  ...
  gates: spec.gates, componentVerdicts: spec.componentVerdicts, overallDecision: spec.overallDecision,
});
const bindingBody = {
  documentType: 'BFS_B33_QUADRATURE_TEMPORAL_HOLDOUT_ANALYSIS_BINDING',
  version: spec.version,
  holdoutSpecSha256: specSha,
...
function validate(overrides = {}) {
  const observed = {
    specSha, derivationResultSha: spec.evidenceBasis.derivationResultSha256,
    ...
    gatesHash: sha256Canonical(spec.gates),
    componentVerdictsHash: sha256Canonical(analysis.componentVerdicts),
    decision: analysis.decision,
    ...overrides,
  };
  failUnless(observed.specSha === expectedSpecSha, 'spec hash mismatch');
  failUnless(observed.derivationResultSha === spec.evidenceBasis.derivationResultSha256, 'derivation evidence hash mismatch');
  failUnless(observed.sceneSha === spec.source.sceneBlendSha256, 'scene identity mismatch');
  failUnless(JSON.stringify(observed.frames) === JSON.stringify([74, 75, 76, 77, 78, 79, 80, 81]), 'frame interval mismatch');
  ...
  failUnless(observed.gatesHash === sha256Canonical(spec.gates), 'formal gates mismatch');
  failUnless(observed.componentVerdictsHash === sha256Canonical(analysis.componentVerdicts), 'component verdict mismatch');
...
const attacks = [
  ...
  validationAttack('derivation_evidence_hash_substitution', { derivationResultSha: '1'.repeat(64) }, 'derivation evidence hash mismatch'),
  validationAttack('scene_identity_substitution', { sceneSha: '2'.repeat(64) }, 'scene identity mismatch'),
  validationAttack('frame_interval_substitution', { frames: [73, 74, 75, 76, 77, 78, 79, 80] }, 'frame interval mismatch'),
  ...
  validationAttack('formal_gate_substitution', { gatesHash: '5'.repeat(64) }, 'formal gates mismatch'),
  validationAttack('component_verdict_substitution', { componentVerdictsHash: '6'.repeat(64) }, 'component verdict mismatch'),
...
}, 'B33 holdout output directory must be empty')) });
...
validate();
const result = {
  documentType: 'BFS_B33_QUADRATURE_TEMPORAL_HOLDOUT_RESULT',
  version: spec.version,
...
process.stdout.write(
  `BFS_B33_HOLDOUT_OK decision=${result.decision} processes=${result.execution.uniqueBlenderProcesses} `
