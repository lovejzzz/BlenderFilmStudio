#!/usr/bin/env python3
"""Bind the independent Data comparator audit to attempt-64."""
...
def transformed_source():
    if hashlib.sha256(BASE.read_bytes()).hexdigest() != EXPECTED_BASE_SHA256:
        raise RuntimeError("timesteps Data-comparison auditor base identity mismatch")
    source = BASE.read_text(encoding="utf-8")
    replacements = [
        ('"""Independently audit attempt-62 copied-VDB subframe comparison."""', '"""Independently audit attempt-64 copied-VDB timestep comparison."""', "docstring"),
        ...
        ('"scripts/run-rc6-moving-liquid-subframes-data-comparison.py"', '"scripts/run-rc6-moving-liquid-timesteps-data-comparison.py"', "runner"),
        ...
        ('{"current": "subframes-2", "baseline": "subframes-1"}', '{"current": "timesteps-min-2", "baseline": "timesteps-min-1"}', "label oracle"),
        ('"bfs.rc6MovingLiquidSubframesDataComparisonIndependentAudit.v0.1"', '"bfs.rc6MovingLiquidTimestepsDataComparisonIndependentAudit.v0.1"', "audit schema"),
    ...
    for before, after, label in replacements:
        ...
        if source.count(before) != expected:
            raise RuntimeError(f"timesteps Data-comparison auditor {label} target mismatch")
        source = source.replace(before, after)
    if source.count("attempt61") != 6:
        raise RuntimeError("timesteps Data-comparison auditor attempt61 key count mismatch")
