#!/usr/bin/env python3
"""Independent formal analyzer for the B52-D12.12-H1 Blender holdout."""
...
def load_array(path: Path, dtype: str, height: int, width: int, channels: int):
    ...
    if len(payload) != expected:
        raise RuntimeError(f"D12.12-H1 array length mismatch: {path}")
...
def main():
    ...
    if sha_file(cli.spec) != SPEC_SHA256 or cli.output.exists() or cli.analysis_receipt.exists():
        raise RuntimeError("D12.12-H1 analyzer identity or fresh output violation")
    ...
    if sha_file(Path(sys.executable)) != runtime["sha256"] or np.__version__ != runtime["numpy"]:
        raise RuntimeError("D12.12-H1 analyzer runtime identity mismatch")
    parent_checks = {}
    ...
    body = {
        "schemaVersion": "bfs.blenderMaterialOwnerOneSidedCurvatureHoldoutResult.v0.1",
    ...
    receipt_body = {
        "schemaVersion": "bfs.blenderMaterialOwnerOneSidedCurvatureHoldoutAnalysisReceipt.v0.1",
    ...
    print(f"BFS_B52_D1212H1_ANALYZER verdict={verdict} hard={sum(hard_checks.values())}/{len(hard_checks)} direction={direction_contract} coverage={coverage_contract}")
