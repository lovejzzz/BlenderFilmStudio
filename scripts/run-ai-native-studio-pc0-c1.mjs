...
const EVIDENCE_URI = 'experiments/ai-native-studio-post-pb7/PC.0-2026-08-31-mac-m2max-attempt-02';
const WORK_ROOT = '/Users/mengyingli/Documents/ChatGPT/BlenderFilmStudio-PostPB7-workspace/PC.0-2026-08-31-mac-m2max-attempt-02';
...
export async function runPc0(argv = process.argv.slice(2)) {
  ...
  const beforeSourceSha256 = await shaFile(sourcePath); const execution = await runProcess(command, timedArgv, { cwd: repositoryRoot, env, stdio: ['ignore', 'pipe', 'pipe'] }, spec.resourceCeilings.pc0WallSeconds * 1000);
  await writeBytes(resolve(evidenceRoot, 'logs/stdout.log'), execution.stdout); await writeBytes(resolve(evidenceRoot, 'logs/stderr.log'), execution.stderr);
  const afterSourceSha256 = await shaFile(sourcePath); const rssMatch = execution.stderr.toString('utf8').match(/([0-9]+)\s+maximum resident set size/); const peakRssBytes = rssMatch ? Number(rssMatch[1]) : null;
  const inventoryExists = await stat(inventoryPath).catch(() => null);
  const processRecord = await writeJson(resolve(evidenceRoot, 'process.json'), { schemaVersion: 'bfs.pc0Process.v0.1', command, argv: timedArgv, blenderArgv: [binary, ...blenderArgv], cwd: repositoryRoot, environment: { HOME: env.HOME, TMPDIR: env.TMPDIR, BLENDER_USER_CONFIG: env.BLENDER_USER_CONFIG, BLENDER_USER_SCRIPTS: env.BLENDER_USER_SCRIPTS, PYTHONNOUSERSITE: env.PYTHONNOUSERSITE, LC_ALL: env.LC_ALL, LANG: env.LANG }, pid: execution.pid, exitCode: execution.code, signal: execution.signal, timedOut: execution.timedOut, wallSeconds: execution.wallSeconds, peakRssBytes, stdout: { uri: `${EVIDENCE_URI}/logs/stdout.log`, sha256: sha(execution.stdout), bytes: execution.stdout.length }, stderr: { uri: `${EVIDENCE_URI}/logs/stderr.log`, sha256: sha(execution.stderr), bytes: execution.stderr.length } }, 'processHash');
