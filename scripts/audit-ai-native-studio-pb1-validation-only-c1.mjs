...
if (selfTestRequested) {
  ...
  const checks = [
    ...
    { id: 'CONTRACT_SCHEMA', pass: ['bfs.pb1ValidationOnlyC1AuthorizationRequest.v0.4', 'bfs.pb1ValidationOnlyC1Execution.v0.5'].includes(spec.schemaVersion) },
  ...
  const output = { schemaVersion: 'bfs.pb1ValidationOnlyC1AuditorSelfTest.v0.5', status: failed.length ? 'FAIL' : 'PASS', checksPassed: checks.length - failed.length, checksTotal: checks.length, checks, failures: failed.map(item => item.id) };
...
const audit = writeJsonExclusive(auditPath, {
  schemaVersion: 'bfs.pb1ValidationOnlyC1IndependentAudit.v0.5',
  gate: 'PB.1',
  mode: 'VALIDATION_ONLY_C1',
  observedAt: new Date().toISOString(),
  status: failed.length ? 'FAIL' : 'PASS',
  auditor: { path: 'scripts/audit-ai-native-studio-pb1-validation-only-c1.mjs', sha256: sha256File(AUDITOR_PATH), importsRunner: false },
