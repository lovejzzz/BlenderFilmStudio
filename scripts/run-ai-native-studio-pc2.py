#!/usr/bin/env python3
"""Fail-closed two-start runner for PC.2 action-complexity validation."""
...
SPEC_URI = "specs/ai-native-studio-pc2-action-complexity-preregistration.v0.1.json"
FREEZE_URI = "specs/ai-native-studio-pc2-tool-freeze.v0.1.json"
EVIDENCE_URI = "experiments/ai-native-studio-post-pb7/PC.2-2026-08-31-mac-m2max-attempt-01"
WORK_ROOT = Path("/Users/mengyingli/Documents/ChatGPT/BlenderFilmStudio-PostPB7-workspace/PC.2-2026-08-31-mac-m2max-attempt-01")
...
def run_process(index, name, argv, env, evidence, ceiling):
    ...
    record = write_self(evidence / "processes" / f"{index:02d}-{name}.json", {
        "schemaVersion": "bfs.pc2Process.v0.1", "name": name, "argv": argv, "exitCode": result.returncode,
...
def execute(args):
    ...
    if not valid_self(spec, "specHash") or not valid_self(freeze, "specHash") or freeze["status"] != "FROZEN_BEFORE_PC2_START": raise RuntimeError("CONTRACT")
    ...
    source = Path(spec["acceptedPc1Baseline"]["sourcePath"]); binary = Path(freeze["binary"]["path"])
    if sha256_file(source) != spec["acceptedPc1Baseline"]["sha256"] or sha256_file(binary) != freeze["binary"]["sha256"]: raise RuntimeError("BASELINE")
    free = os.statvfs(ROOT).f_bavail * os.statvfs(ROOT).f_frsize
    resources = spec["resourceCeilings"]
    ...
    for name in ("logs", "processes"): (evidence / name).mkdir()
    ...
    first_argv = [str(binary), "--background", "--factory-startup", str(source), "--python-exit-code", "1", "--python", str(ROOT / freeze["builder"]["uri"]), "--", *common]
    p1 = run_process(1, "build", first_argv, env, evidence, resources["wallSeconds"])
    ...
    second_argv = [str(binary), "--background", "--factory-startup", str(derived), "--python-exit-code", "1", "--python", str(ROOT / freeze["semanticAuditor"]["uri"]), "--", *common]
    p2 = run_process(2, "semantic-audit", second_argv, env, evidence, resources["wallSeconds"])
    ...
    if source_after != spec["acceptedPc1Baseline"]["sha256"]: raise RuntimeError("SOURCE_DRIFT")
    work_files = tree_files(WORK_ROOT); evidence_files = tree_files(evidence)
    render_artifacts = [str(path) for path in [*work_files, *evidence_files] if path.suffix.lower() in {".exr", ".png", ".jpg", ".jpeg", ".mov", ".mp4"}]
    if render_artifacts: raise RuntimeError("RENDER_ARTIFACTS")
    ...
    if any(row["peakRssBytes"] > resources["peakRssBytesPerProcess"] for row in (p1, p2)): raise RuntimeError("RSS")
    receipt = write_self(evidence / "receipt.json", {
        "schemaVersion": "bfs.pc2ValidationReceipt.v0.1", "status": "PASS", "gate": "PC.2",
        ...
        "source": {"path": str(source), "beforeSha256": spec["acceptedPc1Baseline"]["sha256"], "afterSha256": source_after},
        ...
        "phases": semantic["phaseIds"], "signalTargets": semantic["signalTargets"], "signalChecks": semantic["signalChecks"],
        ...
        "operations": {"BlenderStarts": 2, "renderCalls": 0, "derivedSceneSaves": 1, "sourceSceneSaves": 0, "networkCalls": 0, "modelCalls": 0, "mouseInteractions": 0, "engineSourceEdits": 0, "engineCommits": 0, "engineRemoteWrites": 0},
    }, "receiptHash")
    print(f"BFS_PC2_RUN PASS {receipt['receiptHash']}")
...
except Exception as error: print(f"BFS_PC2_RUN_REJECTED {error}", file=sys.stderr); raise SystemExit(1)
