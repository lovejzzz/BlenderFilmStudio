#!/usr/bin/env python3
"""Independently audit the attempt-67 minimum-particles moving-liquid test."""
...
def transformed_source():
    if hashlib.sha256(BASE.read_bytes()).hexdigest() != EXPECTED_BASE_SHA256:
        raise RuntimeError("moving-liquid particle-minimum auditor base identity mismatch")
    ...
    baseline_after = baseline_before + ' and sha(ATTEMPT65_RESULT) == spec["baseline"]["attempt65ResultFileSha256"] and sha(ATTEMPT65_AUDIT) == spec["baseline"]["attempt65AuditFileSha256"] and sha(ATTEMPT66_AUDIT) == spec["baseline"]["attempt66AuditFileSha256"] and sha(SOURCE_RNA) == spec["implementation"]["rnaFluidSha256"] and sha(SOURCE_DNA) == spec["implementation"]["dnaFluidTypesSha256"] and sha(SOURCE_LIQUID_SCRIPT) == spec["implementation"]["liquidScriptSha256"]'
    ...
    config_after = 'abs(result["configuration"]["particleRadius"] - 1.8) <= 1e-6 and result["configuration"]["particleNumber"] == 2 and result["configuration"]["particleMinimum"] == 12 and result["configuration"]["particleMaximum"] == 16 and abs(result["configuration"]["meshParticleRadius"] - 2.5) <= 1e-6 and abs(result["configuration"]["cupEffectorSurfaceDistanceCells"] - 2.0) <= 1e-6 and result["configuration"]["cupEffectorSubframes"] == 1 and result["configuration"]["timestepsMin"] == 2 and result["configuration"]["timestepsMax"] == 4 and abs(result["configuration"]["cflCondition"] - 2.0) <= 1e-6'
    replacements = [
        ('"""Independently audit the attempt-59 2.0-cell moving-liquid test."""', '"""Independently audit the attempt-67 minimum-particles moving-liquid test."""', "docstring"),
        ("RC6-2026-09-02-moving-liquid-effector-distance-attempt-59", "RC6-2026-09-02-moving-liquid-particle-minimum-attempt-67", "fresh roots"),
        (constants_before, constants_after, "new baseline constants"),
        ('"scripts/run-rc6-moving-liquid-effector-distance-scene.py"', '"scripts/run-rc6-moving-liquid-particle-minimum-scene.py"', "scene tool"),
        ('"scripts/run-rc6-moving-liquid-effector-distance.py"', '"scripts/run-rc6-moving-liquid-particle-minimum.py"', "runner"),
        ('"specs/ai-native-studio-rc6-moving-liquid-effector-distance.v0.70.json"', '"specs/ai-native-studio-rc6-moving-liquid-particle-minimum.v0.78.json"', "spec"),
        (config_before, config_after, "configuration check"),
        (baseline_before, baseline_after, "new baseline checks"),
        ('"logs/01-effector-distance.stdout.log"', '"logs/01-particle-minimum.stdout.log"', "stdout log"),
        ('"logs/01-effector-distance.stderr.log"', '"logs/01-particle-minimum.stderr.log"', "stderr log"),
        ('"processes/01-effector-distance.json"', '"processes/01-particle-minimum.json"', "process receipt"),
        ('"bfs.rc6MovingLiquidEffectorDistanceIndependentAudit.v0.1"', '"bfs.rc6MovingLiquidParticleMinimumIndependentAudit.v0.1"', "audit schema"),
        ('"RC6_MOVING_LIQUID_EFFECTOR_DISTANCE_AUDIT="', '"RC6_MOVING_LIQUID_PARTICLE_MINIMUM_AUDIT="', "audit marker"),
    ]
    for before, after, label in replacements:
        expected = 2 if label == "fresh roots" else 1
        if source.count(before) != expected:
            raise RuntimeError(f"moving-liquid particle-minimum auditor {label} target mismatch")
