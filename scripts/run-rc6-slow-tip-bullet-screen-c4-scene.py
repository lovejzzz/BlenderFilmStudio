#!/usr/bin/env python3
"""C4 damped and limited Bullet hinge correction for the slow-tip screen."""
...
if hashlib.sha256(BASE.read_bytes()).hexdigest() != EXPECTED_BASE_SHA256:
    raise RuntimeError("slow-tip C4 scene base identity mismatch")
...
hinge_pivot_cup_local = cup.matrix_world.inverted() @ hinge_pivot_world''',
        "frame-one hinge rig and physical limits",
    ),
    (
        '''for frame, x in (
    (FRAME_START, -1.10),
    ...
    ('bfs.rc6SlowTipBulletScreenCell.v0.1', 'bfs.rc6SlowTipBulletScreenC4Cell.v0.1', "schema"),
    ('RC6_SLOW_TIP_BULLET_SCREEN=', 'RC6_SLOW_TIP_BULLET_SCREEN_C4=', "marker"),
    ('One Bullet-only slow-tip trajectory screen;', 'One C4 limited and damped Bullet-only slow-tip trajectory screen;', "claim"),
)
for before, after, label in replacements:
    if source.count(before) != 1:
        raise RuntimeError(f"slow-tip C4 scene {label} target mismatch")
    source = source.replace(before, after)
exec(compile(source, str(BASE) + "#C4_LIMITED_DAMPED_HINGE", "exec"), globals(), globals())
