#!/usr/bin/env python3
"""Independent payload, measurement, decision and tamper audit for B52-D12.7."""
...
def main()->None:
 p=argparse.ArgumentParser();p.add_argument("--spec",type=Path,required=True);p.add_argument("--root",type=Path,required=True);p.add_argument("--execution",type=Path,required=True);p.add_argument("--result",type=Path,required=True);p.add_argument("--output",type=Path,required=True);a=p.parse_args()
 if a.output.exists():raise RuntimeError("refusing to overwrite D12.7 audit")
 ...
 expected={"schemaVersion":"bfs.blenderStaticAdaptiveRiskGateResult.v0.1","experimentId":spec["experimentId"],"verdict":verdict,"passed":verdict==spec["decision"]["supportedVerdict"],"analyzerIndependent":analyzer_independent,"checks":checks,"checkPassed":sum(row["passed"] for row in checks),"checkTotal":len(checks),"measurements":measurements,"identities":identities,"parentChecks":parents,"mutationAttackPassed":len(result.get("mutationAttacks",[])),"mutationAttackTotal":len(result.get("mutationAttacks",[])),"operationCounts":{"analyzerProcesses":1,"modelCalls":0,"networkCalls":0},"nonClaims":spec["nonClaims"]}
