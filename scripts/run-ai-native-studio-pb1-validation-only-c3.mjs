...
function collectPreflight(spec) {
  ...
  const retainedDependencyObjects = spec.dependencyLfsCorrection.retainedObjectsSubtree;
  const remote = collectRemote(spec);
  const observed = {
    schemaVersion: 'bfs.pb1ValidationOnlyC3Preflight.v0.9',
    observedAt: new Date().toISOString(),
    status: 'PENDING',
    ...
    runner: { path: 'scripts/run-ai-native-studio-pb1-validation-only-c3.mjs', sha256: sha256File(SCRIPT_PATH) },
    authorization: {
      granted: spec.authorization?.granted === true,
      exactTextMatchesRequest: spec.authorization?.exactTextZhCN === request.exactStandingAuthorizationTextZhCN,
    },
    ...
    dependency: { path: retainedDependency, head: git(retainedDependency, ['rev-parse', 'HEAD']), clean: git(retainedDependency, ['status', '--porcelain=v1']) === '', origin: git(retainedDependency, ['remote', 'get-url', 'origin']), lfsObjects: treeIdentity(retainedDependencyObjects) },
  ...
  if (!observed.roots.externalAbsent || !observed.roots.evidenceAbsent) failures.push('FRESH_ATTEMPT04_ROOTS_REQUIRED');
  ...
  if (observed.dependency.lfsObjects.files !== spec.dependencyLfsCorrection.retainedObjectFiles || observed.dependency.lfsObjects.bytes !== spec.dependencyLfsCorrection.retainedObjectBytes || observed.dependency.lfsObjects.manifestSha256 !== spec.dependencyLfsCorrection.retainedObjectManifestSha256) failures.push('RETAINED_DEPENDENCY_LFS_OBJECTS_MISMATCH');
...
function validationFailures(value, spec) {
  ...
  if (!value.dependencyObjectsSymlinkExact) failures.push('DEPENDENCY_LFS_OBJECT_LINK_MISMATCH');
...
function negativeControls(spec) {
  ...
  const cases = [
    ...
    ['DEPENDENCY_LFS_OBJECT_LINK_REJECTED', { dependencyObjectsSymlinkExact: false }, 'DEPENDENCY_LFS_OBJECT_LINK_MISMATCH'],
...
function selfTest(spec) {
  ...
  add('CONTRACT_SCHEMA', ['bfs.pb1ValidationOnlyC3AuthorizationRequest.v0.8', 'bfs.pb1ValidationOnlyC3Execution.v0.9'].includes(spec.schemaVersion));
  ...
  add('FRESH_OBJECT_SYMLINK_FIXED', spec.lfsCorrection.freshObjectsSymlinkTargetMustBeRetainedObjectsSubtree === true && spec.lfsCorrection.retainedStorageWholeTreeMustRemainUnchangedDuringAttempt04 === true && spec.lfsCorrection.symlinkBeforePublicationCheckout === true);
  add('SYMLINK_CODE_PRECEDES_PUBLICATION_CHECKOUT', runnerSource.indexOf("symlinkSync(retainedObjects, localObjects, 'dir')") > 0 && runnerSource.indexOf("symlinkSync(retainedObjects, localObjects, 'dir')") < runnerSource.indexOf("'checkout exact publication head after objects link'"));
  add('DEPENDENCY_LFS_LOCAL_MATERIALIZATION_FIXED', spec.authorizedOperationsIfOwnerApproves.freshDependencyObjectsSymlinks === 1 && spec.authorizedOperationsIfOwnerApproves.additionalLocalDependencyLfsMaterializations === 1 && spec.dependencyLfsCorrection.trackedPaths === 622 && spec.dependencyLfsCorrection.retainedObjectFiles === 618);
  add('DEPENDENCY_SYMLINK_CODE_PRECEDES_CHECKOUT', runnerSource.indexOf("symlinkSync(retainedDependencyObjects, localDependencyObjects, 'dir')") > 0 && runnerSource.indexOf("symlinkSync(retainedDependencyObjects, localDependencyObjects, 'dir')") < runnerSource.indexOf("'checkout exact dependency after objects link'"));
  ...
  add('TEN_NEGATIVE_CONTROLS', negativeControls(spec).length === 10 && negativeControls(spec).every(item => item.pass));
  ...
  const output = { schemaVersion: 'bfs.pb1ValidationOnlyC3SelfTest.v0.9', status: failed.length ? 'FAIL' : 'PASS', checksPassed: checks.length - failed.length, checksTotal: checks.length, checks, failures: failed.map(item => item.id) };
...
async function execute(spec) {
  ...
  try {
    ...
    const negative = writeJsonExclusive(resolve(evidenceRoot, 'negative-controls.json'), { schemaVersion: 'bfs.pb1ValidationOnlyC3NegativeControls.v0.9', observedAt: new Date().toISOString(), status: controls.length === 10 && controls.every(item => item.pass) ? 'PASS' : 'FAIL', externalRootExisted: existsSync(externalRoot), checksPassed: controls.filter(item => item.pass).length, checksTotal: controls.length, controls, counters });
    ...
    tracked(commands, stage, 'disable LFS network URL', GIT, ['-C', source, 'config', 'lfs.url', 'file:///PB1-C3-LFS-NETWORK-DISABLED']);

    stage = 'FRESH_LOCAL_LFS_PRECHECKOUT_LINK';
    const localLfs = resolve(source, '.git', 'lfs');
    mkdirSync(localLfs, { recursive: true });
    const localObjects = resolve(localLfs, 'objects');
    if (existsSync(localObjects)) throw new Error('FRESH_LFS_OBJECTS_PATH_NOT_ABSENT_AFTER_NO_CHECKOUT_CLONE');
    symlinkSync(retainedObjects, localObjects, 'dir');
    counters.freshObjectsSymlinks += 1;
    tracked(commands, stage, 'checkout exact publication head after objects link', GIT, ['-C', source, 'checkout', '--detach', spec.publicationBaseline.head]);
    ...
    const lfsReceipt = writeJsonExclusive(resolve(evidenceRoot, 'lfs-materialization.json'), { schemaVersion: 'bfs.pb1ValidationOnlyC3LfsMaterialization.v0.9', observedAt: new Date().toISOString(), status: Object.values(lfsChecks).every(Boolean) ? 'PASS' : 'FAIL', localLfsRoot: localLfs, localObjects, retainedBefore: preflight.retainedLfs.wholeTree, retainedAfter: retainedAfterMaterialization, lfs, checks: lfsChecks });
    ...
    const historyReceipt = writeJsonExclusive(resolve(evidenceRoot, 'remote-and-history.json'), { schemaVersion: 'bfs.pb1ValidationOnlyC3RemoteHistory.v0.9', observedAt: new Date().toISOString(), status: Object.values(graphChecks).every(Boolean) ? 'PASS' : 'FAIL', remoteBefore: preflight.remote, source: sourceNow, metric, publication, fsckExitCode: fsck.exitCode, checks: graphChecks });
    ...
    const licenseReceipt = writeJsonExclusive(resolve(evidenceRoot, 'license-and-generated-paths.json'), { schemaVersion: 'bfs.pb1ValidationOnlyC3LicenseGenerated.v0.9', observedAt: new Date().toISOString(), status: Object.values(licenseChecks).every(Boolean) ? 'PASS' : 'FAIL', inventory, checks: licenseChecks });
    ...
    tracked(commands, stage, 'exact dependency local no-checkout clone', GIT, ['clone', '--no-hardlinks', '--no-checkout', retainedDependency, dependencyPath]);
    counters.localDependencyClones += 1;
    tracked(commands, stage, 'disable dependency LFS network URL', GIT, ['-C', dependencyPath, 'config', 'lfs.url', 'file:///PB1-C3-DEPENDENCY-LFS-NETWORK-DISABLED']);
    const localDependencyLfs = resolve(dependencyPath, '.git', 'lfs');
    mkdirSync(localDependencyLfs, { recursive: true });
    const localDependencyObjects = resolve(localDependencyLfs, 'objects');
    if (existsSync(localDependencyObjects)) throw new Error('FRESH_DEPENDENCY_LFS_OBJECTS_PATH_NOT_ABSENT_AFTER_NO_CHECKOUT_CLONE');
    const retainedDependencyBefore = treeIdentity(retainedDependencyObjects);
    symlinkSync(retainedDependencyObjects, localDependencyObjects, 'dir');
    counters.freshDependencyObjectsSymlinks += 1;
    tracked(commands, stage, 'checkout exact dependency after objects link', GIT, ['-C', dependencyPath, 'checkout', '--detach', spec.dependency.commit]);
    tracked(commands, stage, 'single zero-network dependency LFS checkout', GIT, ['-C', dependencyPath, 'lfs', 'checkout']);
    counters.localDependencyLfsMaterializations += 1;
    const dependencyLfs = materializedLfs(dependencyPath);
    const retainedDependencyAfter = treeIdentity(retainedDependencyObjects);
    const dependency = { head: git(dependencyPath, ['rev-parse', 'HEAD']), clean: git(dependencyPath, ['status', '--porcelain=v1']) === '', origin: git(dependencyPath, ['remote', 'get-url', 'origin']), superprojectClean: sourceIdentity(source).clean, bytes: treeBytes(dependencyPath), lfs: dependencyLfs, localLfs: localDependencyLfs, localObjects: localDependencyObjects, retainedObjectsBefore: retainedDependencyBefore, retainedObjectsAfter: retainedDependencyAfter };
    const dependencyChecks = { headExact: dependency.head === spec.dependency.commit, clean: dependency.clean, localOriginExact: dependency.origin === retainedDependency, superprojectClean: dependency.superprojectClean, retainedStillExact: git(retainedDependency, ['rev-parse', 'HEAD']) === spec.dependency.commit && git(retainedDependency, ['status', '--porcelain=v1']) === '', objectsLinkExact: lstatSync(localDependencyObjects).isSymbolicLink() && execRequired('/usr/bin/readlink', [localDependencyObjects]) === retainedDependencyObjects, retainedObjectsUnchanged: canonicalJson(retainedDependencyBefore) === canonicalJson(retainedDependencyAfter), lfsCountExact: dependencyLfs.count === spec.dependencyLfsCorrection.trackedPaths, lfsBytesExact: dependencyLfs.bytes === spec.dependencyLfsCorrection.materializedContentBytes, allDependencyLfsMaterializedExact: dependencyLfs.downloadedCount === dependencyLfs.count && dependencyLfs.checkoutCount === dependencyLfs.count && dependencyLfs.mismatches.length === 0 };
    const dependencyReceipt = writeJsonExclusive(resolve(evidenceRoot, 'dependency.json'), { schemaVersion: 'bfs.pb1ValidationOnlyC3Dependency.v0.9', observedAt: new Date().toISOString(), status: Object.values(dependencyChecks).every(Boolean) ? 'PASS' : 'FAIL', dependency, checks: dependencyChecks });
    ...
    const build = await runLogged({ command: MAKE, args: [`BUILD_DIR=${buildRoot}`, 'NPROCS=12'], cwd: source, stdoutPath: buildStdout, stderrPath: buildStderr, timingPath: buildTiming, timeoutMs: spec.resources.maximumBuildWallSeconds * 1000, monitoredRoot: externalRoot, maximumBytes: spec.resources.maximumAttempt04ExternalRootBytes });
    ...
    const buildReceipt = writeJsonExclusive(resolve(evidenceRoot, 'build.json'), { schemaVersion: 'bfs.pb1ValidationOnlyC3Build.v0.9', observedAt: new Date().toISOString(), status: Object.values(buildChecks).every(Boolean) ? 'PASS' : 'FAIL', process: build, timing, artifact: binaryExists ? { app, binary, bytes: statSync(binary).size, sha256: sha256File(binary), fileIdentity, architectures, bundleId: plistRaw(plist, 'CFBundleIdentifier') } : null, logs: { stdout: { bytes: statSync(buildStdout).size, sha256: sha256File(buildStdout) }, stderr: { bytes: statSync(buildStderr).size, sha256: sha256File(buildStderr) }, timing: { bytes: statSync(buildTiming).size, sha256: sha256File(buildTiming) } }, checks: buildChecks });
    ...
    const runtimeReceipt = writeJsonExclusive(resolve(evidenceRoot, 'runtime-identity.json'), { schemaVersion: 'bfs.pb1ValidationOnlyC3Runtime.v0.9', observedAt: new Date().toISOString(), status: Object.values(runtimeChecks).every(Boolean) ? 'PASS' : 'FAIL', productStarts: counters.productStarts, renders: counters.renders, version: { exitCode: version.exitCode, output: versionOutput.trim() }, runtime: { exitCode: runtime.exitCode, elapsedSeconds: runtime.elapsedSeconds, payload, stdoutSha256: sha256File(runtimeStdout), stderrSha256: sha256File(runtimeStderr) }, configuration: { actualOfficial, officialBefore, officialAfter, isolatedOfficial, isolatedOfficialAfter, productRoot, productAfter, expectedPaths }, checks: runtimeChecks });
    ...
    const finalChecks = { receiptsPass: bindings.every(item => item.receiptHashPass && item.status !== 'FAIL'), operationsExact: counters.localEngineClones === 1 && counters.freshObjectsSymlinks === 1 && counters.localLfsMaterializations === 1 && counters.localDependencyClones === 1 && counters.freshDependencyObjectsSymlinks === 1 && counters.localDependencyLfsMaterializations === 1 && counters.nativeBuilds === 1 && counters.productStarts === 2, forbiddenZero: ['publicEngineNetworkClones','renders','engineRemoteWrites','engineRefUpdates','lfsNetworkDownloads','lfsUploads','releases','signing','notarization','dmg','pb2ThroughPb7','modelCalls'].every(name => counters[name] === 0), sourceExactClean: sourceIdentity(source).head === spec.publicationBaseline.head && sourceIdentity(source).tree === spec.publicationBaseline.tree && sourceIdentity(source).clean, remoteUnchanged: remoteAfter.main === spec.publicationBaseline.head, retainedStorageUnchanged: canonicalJson(preflight.retainedLfs.wholeTree) === canonicalJson(retainedFinal), externalWithin: treeBytes(externalRoot) <= spec.resources.maximumAttempt04ExternalRootBytes, evidenceWithin: treeBytes(evidenceRoot) <= spec.resources.maximumEvidenceRootBytes };
    const verdict = writeJsonExclusive(resolve(evidenceRoot, 'verdict.json'), { schemaVersion: 'bfs.pb1ValidationOnlyC3Verdict.v0.9', gate: 'PB.1', mode: 'VALIDATION_ONLY_C3', observedAt: new Date().toISOString(), status: Object.values(finalChecks).every(Boolean) ? 'PASS' : 'FAIL', publicationHead: spec.publicationBaseline.head, f0Parent: spec.publicationBaseline.soleParent, dependencyCommit: spec.dependency.commit, bindings, counters, resources: { externalBytes: treeBytes(externalRoot), evidenceBytesBeforeVerdict: treeBytes(evidenceRoot) }, checks: finalChecks, failures: Object.entries(finalChecks).filter(([, pass]) => !pass).map(([name]) => name), claimCeiling: 'PB.1 validation-only proves repository/source identity and one clean same-host arm64 build; it does not authorize PB.2-PB.7 or prove distribution, production readiness, cross-platform support, or autonomous filmmaking.', stopRulePreserved: true });
    if (verdict.status !== 'PASS') throw new Error(`VERDICT_FAILED:${verdict.failures.join(',')}`);
    process.stdout.write(`PB1_C3_VALIDATION_PASS receipt=${verdict.receiptHash} buildSeconds=${build.elapsedSeconds.toFixed(3)}\n`);
  } catch (error) {
    if (evidenceCreated) {
      const failurePath = resolve(evidenceRoot, 'failure.json');
      if (!existsSync(failurePath)) writeJsonExclusive(failurePath, { schemaVersion: 'bfs.pb1ValidationOnlyC3Failure.v0.9', gate: 'PB.1', status: 'FAIL', observedAt: new Date().toISOString(), failedStage: stage, error: String(error.message ?? error), stderr: error.stderr ?? null, counters, externalRootExists: existsSync(externalRoot), externalBytes: treeBytes(externalRoot), stopRulePreserved: true });
      const verdictPath = resolve(evidenceRoot, 'verdict.json');
      if (!existsSync(verdictPath)) writeJsonExclusive(verdictPath, { schemaVersion: 'bfs.pb1ValidationOnlyC3Verdict.v0.9', gate: 'PB.1', status: 'FAIL', observedAt: new Date().toISOString(), failedStage: stage, failureReceiptHash: JSON.parse(readFileSync(failurePath, 'utf8')).receiptHash, counters, stopRulePreserved: true });
    }
    process.stderr.write(`PB1_C3_VALIDATION_FAIL stage=${stage} error=${String(error.message ?? error)}\n`);
    process.exitCode = 1;
  }
}
