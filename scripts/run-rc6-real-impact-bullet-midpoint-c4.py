#!/usr/bin/env python3
"""C4 runner adapter for the single I09 real-impact midpoint."""
...
if hashlib.sha256(BASE.read_bytes()).hexdigest() != EXPECTED_BASE_SHA256:
    raise RuntimeError("real-impact C4 runner base identity mismatch")
...
replacements = (
    ...
    ("scripts/run-rc6-real-impact-bullet-speed-screen.py", "scripts/run-rc6-real-impact-bullet-midpoint-c4.py", 1, "runner commit path"),
    ("scripts/audit-rc6-real-impact-bullet-speed-screen.py", "scripts/audit-rc6-real-impact-bullet-midpoint-c4.py", 2, "auditor"),
...
for before, after, expected, label in replacements:
    if source.count(before) != expected:
        raise RuntimeError(f"real-impact C4 runner {label} target mismatch")
