#!/usr/bin/env node
/** Independent scalar Node consumer for B52-D12.14-H1. */
import crypto from 'node:crypto';
import fs from 'node:fs';
import path from 'node:path';

const SPEC_SHA256 = '7ff239d91dca6ea8708ce4cac955dd0b129ae067028a77ec1699a43a236195a8';
const Q24 = 1n << 24n;
const Q30 = 1n << 30n;
...
function parseArgs() {
  ...
  for (let index = 2; index < process.argv.length; index += 2) {
    ...
    if (!key.startsWith('--') || index + 1 >= process.argv.length) throw new Error('invalid D12.14-H1 arguments');
  ...
  if (![1, 2].includes(result.repeat)) throw new Error('invalid D12.14-H1 repeat');
  return result;
}

function readF32(filePath, expectedCount) {
  const payload = fs.readFileSync(filePath);
  if (payload.length !== expectedCount * 4) throw new Error(`D12.14-H1 float32 length mismatch: ${filePath}`);
...
function effectiveFixture(spec, fixture) {
  const camera = spec.sceneContract.camera;
  const result = {
    ...fixture,
    cameraByFrame: Object.fromEntries(['0', '1', '2'].map((frame) => [frame, {
      location: camera.locationByFrame[frame],
      rotationEuler: camera.rotationEulerByFrame[frame],
    }])),
  };
  result.owners = fixture.owners.map((row) => {
    const owner = {...row};
    if (owner.role === 'background') {
      const background = spec.sceneContract.background;
      owner.sizeWorld = background.sizeWorld;
      owner.transformByFrame = background.transformByFrame;
    } else {
...
function main() {
  ...
  if (shaFile(args.spec) !== SPEC_SHA256 || fs.existsSync(args['output-dir']) || fs.existsSync(args.report)) {
    throw new Error('D12.14-H1 spec identity or fresh consumer output violation');
  }
  ...
  if (shaFile(process.execPath) !== spec.runtime.node.sha256 || process.version !== spec.runtime.node.version) {
    throw new Error('D12.14-H1 Node runtime identity mismatch');
  }
  const rawFixture = spec.fixtures.find((row) => row.id === args.fixture);
  if (!rawFixture) throw new Error('unknown D12.14-H1 fixture');
  const fixture = effectiveFixture(spec, rawFixture);
  ...
  if (adapter.reportHash !== canonicalHash(adapterBody) || adapter.fixtureId !== args.fixture || adapter.repeat !== args.repeat) {
    throw new Error('D12.14-H1 adapter report mismatch');
  }
  const arrays = {};
  for (const [name, [filename, channels]] of Object.entries(INPUTS)) {
    ...
    if (shaBytes(payload) !== adapter.arrays[name].sha256) throw new Error(`D12.14-H1 input hash mismatch: ${name}`);
    arrays[name] = value;
  }
  ...
  const body = {
    schemaVersion: 'bfs.blenderMaterialOwnerRigidDirectionalRenderHoldoutConsumerReport.v0.1',
  ...
  console.log(`BFS_B52_D1214H1_NODE fixture=${args.fixture} repeat=${args.repeat} accepted=${acceptedCount}`);
}
