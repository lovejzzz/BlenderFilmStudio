#!/usr/bin/env python3
"""Independently audit the attempt-68 fractional-obstacle-distance test."""
...
def transformed_source():
    if hashlib.sha256(BASE.read_bytes()).hexdigest() != EXPECTED_BASE_SHA256:
        raise RuntimeError("moving-liquid fractions-distance auditor base identity mismatch")
    ...
    baseline_after = baseline_before + ' and sha(ATTEMPT65_RESULT) == spec["baseline"]["attempt65ResultFileSha256"] and sha(ATTEMPT65_AUDIT) == spec["baseline"]["attempt65AuditFileSha256"] and sha(ATTEMPT66_AUDIT) == spec["baseline"]["attempt66AuditFileSha256"] and sha(ATTEMPT67_AUDIT) == spec["baseline"]["attempt67AuditFileSha256"] and sha(SOURCE_RNA) == spec["implementation"]["rnaFluidSha256"] and sha(SOURCE_DNA) == spec["implementation"]["dnaFluidTypesSha256"] and sha(SOURCE_LIQUID_SCRIPT) == spec["implementation"]["liquidScriptSha256"]'
    ...
    config_after = 'abs(result["configuration"]["particleRadius"] - 1.8) <= 1e-6 and result["configuration"]["particleNumber"] == 2 and result["configuration"]["particleMinimum"] == 8 and result["configuration"]["particleMaximum"] == 16 and abs(result["configuration"]["fractionsThreshold"] - 0.05) <= 1e-6 and abs(result["configuration"]["fractionsDistance"] - 0.25) <= 1e-6 and abs(result["configuration"]["meshParticleRadius"] - 2.5) <= 1e-6 and abs(result["configuration"]["cupEffectorSurfaceDistanceCells"] - 2.0) <= 1e-6 and result["configuration"]["cupEffectorSubframes"] == 1 and result["configuration"]["timestepsMin"] == 2 and result["configuration"]["timestepsMax"] == 4 and abs(result["configuration"]["cflCondition"] - 2.0) <= 1e-6'
    replacements = [
        ('"""Independently audit the attempt-59 2.0-cell moving-liquid test."""', '"""Independently audit the attempt-68 fractional-obstacle-distance test."""', "docstring"),
        ...
        ('"scripts/run-rc6-moving-liquid-effector-distance.py"', '"scripts/run-rc6-moving-liquid-fractions-distance.py"', "runner"),
        ...
        ('"bfs.rc6MovingLiquidEffectorDistanceIndependentAudit.v0.1"', '"bfs.rc6MovingLiquidFractionsDistanceIndependentAudit.v0.1"', "audit schema"),
        ('"RC6_MOVING_LIQUID_EFFECTOR_DISTANCE_AUDIT="', '"RC6_MOVING_LIQUID_FRACTIONS_DISTANCE_AUDIT="', "audit marker"),
    ]
    for before, after, label in replacements:
        expected = 2 if label == "fresh roots" else 1
        if source.count(before) != expected:
            raise RuntimeError(f"moving-liquid fractions-distance auditor {label} target mismatch")
