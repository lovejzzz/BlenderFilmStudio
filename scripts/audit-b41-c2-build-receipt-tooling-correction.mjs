...
import { analyzeB41C2Evidence, readB41C2Spec } from './lib/b41-c2-build-receipt-tooling-correction.mjs';

const root = resolve(repositoryRoot, 'experiments/linux-amd64-blender-runtime-canary-v0-3');
...
const analysis = analyzeB41C2Evidence(result, c2Spec, c1Spec, baseSpec);
...
const audit = {
  schemaVersion: 'bfs.linuxAmd64BlenderRuntimeIndependentAudit.v0.3', experimentId: 'B41-C2', analysis,
...
process.stdout.write(`BFS_B41_C2_AUDIT ${audit.passed ? 'PASS' : 'FAIL'} tools=${toolsMatch ? 'MATCH' : 'MISMATCH'} artifacts=${artifactsMatch ? 'MATCH' : 'MISMATCH'} timeoutPromotable=${result.timeout?.promotable}\n`);
