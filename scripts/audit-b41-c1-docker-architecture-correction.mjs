...
import { analyzeB41C1Evidence, readB41C1Spec } from './lib/b41-c1-docker-architecture-correction.mjs';

const root = resolve(repositoryRoot, 'experiments/linux-amd64-blender-runtime-canary-v0-2');
const result = JSON.parse(await readFile(resolve(root, 'results.json'), 'utf8'));
const baseSpec = await readB41Spec();
const correctionSpec = await readB41C1Spec();
const analysis = analyzeB41C1Evidence(result, correctionSpec, baseSpec);
...
const audit = {
  schemaVersion: 'bfs.linuxAmd64BlenderRuntimeIndependentAudit.v0.2', experimentId: 'B41-C1', analysis,
...
process.stdout.write(`BFS_B41_C1_AUDIT ${audit.passed ? 'PASS' : 'FAIL'} tools=${toolsMatch ? 'MATCH' : 'MISMATCH'} artifacts=${artifactsMatch ? 'MATCH' : 'MISMATCH'} timeoutPromotable=${result.timeout?.promotable}\n`);
