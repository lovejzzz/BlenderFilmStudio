#!/usr/bin/env python3
"""Zero-render admission preflight for B52-D12.11-I1."""
...
def main():
    ...
    if sha_file(h1_path) != H1_SPEC_SHA256:
        raise RuntimeError("D12.11-I1 H1 spec identity mismatch")
    spec = json.loads(h1_path.read_text())
    analysis_spec = json.loads(json.dumps(spec))
    for fixture in analysis_spec["fixtures"]:
        for owner in fixture["owners"]:
            owner["passIndex"] = intervention["materialOwnerTokens"]["assignments"][owner["analyticOwnerId"]]
    expected_output = (repo / intervention["diskAdmission"]["preflightRoot"] / "frozen-tool-preflight.json").resolve()
    if output != expected_output or output.parent.exists() or (repo / intervention["diskAdmission"]["formalRoot"]).exists():
        raise RuntimeError("D12.11-I1 preflight/formal freshness rejected")
    ...
    prereg_tools_absent = all(subprocess.run(["git", "cat-file", "-e", f"{prereg_commit}:{uri}"], cwd=repo, capture_output=True).returncode != 0 for uri in intervention["freshness"]["newFormalToolPaths"])
    parent_checks = {name: sha_file(repo / row["uri"]) == row["sha256"] for name, row in intervention["parents"].items() if "uri" in row and "sha256" in row}
    runtime_checks = {
    ...
    disk = {
        ...
        "projectedWriteBytes": intervention["diskAdmission"]["projectedWriteBytes"],
        "minimumReserveBytes": intervention["diskAdmission"]["minimumReserveBytes"],
        "freeAfterProjectedBytes": available - intervention["diskAdmission"]["projectedWriteBytes"],
    }
    disk["passed"] = disk["freeAfterProjectedBytes"] >= disk["minimumReserveBytes"]
    analyzer_tree = ast.parse((repo / "scripts/analyze-b52-d12-11-material-owner.py").read_text())
    imports = []
    ...
    with tempfile.TemporaryDirectory(prefix="bfs-d1211-i1-preflight-") as temporary:
        temp = Path(temporary)
        for fixture in spec["fixtures"]:
            ...
            for key, suffix in (("TMPDIR", "tmp"), ("BLENDER_USER_CONFIG", "config"), ("BLENDER_USER_SCRIPTS", "scripts")):
                ...
                env[key] = str(target)
            row = run([spec["runtime"]["blender"]["executable"], *spec["runtime"]["blender"]["launchFlags"], "--python", str(repo / "blender/render_b52_d12_11_material_owner_source.py"), "--", "--spec", str(spec_path), "--fixture", fixture["id"], "--frame", "1", "--repeat", "1", "--report", str(report), "--probe-only"], repo, env)
            ...
            probe_rows.append({
                ...
                "materialPassIndices": [owner["materialPassIndex"] for owner in payload.get("sceneStructure", {}).get("owners", [])],
        ...
        module_spec = importlib.util.spec_from_file_location("d1211_consumer", repo / "scripts/reconstruct-b52-d12-11-material-owner.py")
        module = importlib.util.module_from_spec(module_spec)
        module_spec.loader.exec_module(module)
        fixture = next(row for row in analysis_spec["fixtures"] if row["id"] == "STATIC_FREQUENCY_CONTROL_131X89")
        ...
        for y in range(height):
            for x in range(width):
                oracle = module.oracle_pixel(analysis_spec, fixture, x, y)
        ...
        if risk_target is None:
            raise RuntimeError("D12.11-I1 synthetic risk target absent")
        ...
        adapter_body = {"schemaVersion": "bfs.d1211SyntheticAdapter.v0.1", "experimentId": intervention["experimentId"], "fixtureId": fixture["id"], "repeat": 1, "pid": 0, "arrays": records}
        ...
        reports = {}
        for producer, executable, tool in (("python", spec["runtime"]["python"]["executable"], repo / "scripts/reconstruct-b52-d12-11-material-owner.py"), ("node", spec["runtime"]["node"]["executable"], repo / "scripts/reconstruct-b52-d12-11-material-owner.mjs")):
    ...
    expected_material_by_fixture = {fixture["id"]: [intervention["materialOwnerTokens"]["assignments"][owner["analyticOwnerId"]] for owner in fixture["owners"]] for fixture in spec["fixtures"]}
    expected_object_by_fixture = {fixture["id"]: [owner["passIndex"] for owner in fixture["owners"]] for fixture in spec["fixtures"]}
    probe_ok = all(row["exitCode"] == 0 and row["reportHashValid"] and row["probeOnly"] is True and row["output"] is None and row["ownerCount"] == 2 and row["renderCalls"] == 0 and row["passIndices"] == expected_object_by_fixture[row["fixtureId"]] and row["materialPassIndices"] == expected_material_by_fixture[row["fixtureId"]] for row in probe_rows)
    ...
    checks = {
        "specIdentity": True,
        "formalRootAbsent": not (repo / intervention["diskAdmission"]["formalRoot"]).exists(),
        "preflightRootFresh": not output.parent.exists(),
        "toolHashesMatchHead": all(git_checks.values()),
        "preregisteredBeforeTools": prereg_tools_absent and prereg_commit != head,
    ...
    body = {
        "schemaVersion": "bfs.blenderMaterialIndexOwnerIntegrationPreflight.v0.1",
    ...
    print(f"BFS_B52_D1211_PREFLIGHT status={report['status']} checks={sum(checks.values())}/{len(checks)} riskRejected={synthetic['riskRejectedPixels']}")
