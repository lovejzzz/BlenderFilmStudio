...
const CORRECTION_URI = 'specs/b62-camera-quality-bounded-search-c1-rotation-primitive.v0.1.json';
const CORRECTION_PROTOCOL_URI = 'research/2026-08-29-b62-d3-c1-rotation-primitive-correction.md';
const ROOT_URI = 'experiments/b62-camera-quality-bounded-candidate-search-v0-2';
const RETAINED_ROOT = 'experiments/b62-camera-quality-bounded-candidate-search-v0-1';
const PARENT_ROOT = 'experiments/b62-camera-quality-material-aware-framing-v0-1';
const PREREGISTRATION_COMMIT = 'ae089e5';
const CORRECTION_COMMIT = 'ab66df5';
...
async function processReceipt(root, id, command, args, result) { return writeHashed(resolve(root, 'processes', `${id}.json`), { schemaVersion: 'bfs.b62CameraQualityProcessReceipt.v0.1', experimentId: 'B62-Q1-D3', processId: id, command, args, result }, 'processHash'); }
...
async function blenderChild(root, id, tool, output, master, masterSha, spec) {
  ...
  const result = await runBudgetedProcess({ command, args, cwd: repositoryRoot, env: { PATH: '/usr/bin:/bin', LANG: 'C.UTF-8', LC_ALL: 'C.UTF-8', OCIO: resolve(repositoryRoot, 'color/ocio/cg-config-v4.0.0_aces-v2.0_ocio-v2.5.ocio') }, outputRoot: root, budgets: { wallTimeMs: spec.processBudget.maximumWallSecondsPerBlender * 1000, maxRssBytes: spec.processBudget.maximumPeakResidentSetSizeBytesPerBlender, maxLogBytes: spec.processBudget.maximumCombinedLogBytesPerChild, maxOutputFiles: 64, maxOutputBytes: spec.processBudget.projectedWriteBytes, sampleIntervalMs: 100 } });
...
async function main() {
  ...
  const correction = JSON.parse(await readFile(localPath(CORRECTION_URI), 'utf8'));
  requireValue(spec.experimentId === 'B62-Q1-D3' && spec.statusBeforeToolCreation === 'PREREGISTERED', 'spec mismatch');
  requireValue(correction.correctionId === 'B62-Q1-D3-C1' && correction.statusBeforeToolChange === 'PREREGISTERED' && correction.retainedFailure.root === spec.output.formalRoot && correction.authorizedChanges.retryRoot === ROOT_URI, 'correction mismatch');
  ...
  requireValue(await hashFile(localPath(CORRECTION_URI)) === await committedHash(CORRECTION_COMMIT, CORRECTION_URI), 'correction drift');
  requireValue(await hashFile(localPath(CORRECTION_PROTOCOL_URI)) === await committedHash(CORRECTION_COMMIT, CORRECTION_PROTOCOL_URI), 'correction protocol drift');
  const toolHashes = {};
  ...
  requireValue((await git(['status', '--porcelain=v1', '--', SPEC_URI, PROTOCOL_URI, CORRECTION_URI, CORRECTION_PROTOCOL_URI, PARENT_ROOT, RETAINED_ROOT, ...TOOLS])).trim() === '', 'scoped dirty');
  const retainedTree = await treeIdentity(RETAINED_ROOT);
  requireValue(canonicalJson(retainedTree) === canonicalJson(correction.retainedFailure.tree), 'retained v0.1 tree drift');
  requireValue(await hashFile(localPath(correction.retainedFailure.failure.uri)) === correction.retainedFailure.failure.sha256, 'retained failure drift');
  requireValue(toolHashes['blender/search_b62_q1_close_camera_candidates.py'] === correction.frozen.primaryBlenderTool.sha256, 'primary tool changed under C1');
  const parentTree = await treeIdentity(PARENT_ROOT);
  requireValue(canonicalJson(parentTree) === canonicalJson(spec.parentEvidence.d2.tree), 'D2 tree drift');
  const parentReceiptPath = localPath(spec.parentEvidence.d2.receipt.uri);
  requireValue(await hashFile(parentReceiptPath) === spec.parentEvidence.d2.receipt.sha256, 'D2 receipt drift');
  const parentReceipt = JSON.parse(await readFile(parentReceiptPath, 'utf8'));
  requireValue(parentReceipt.receiptHash === spec.parentEvidence.d2.receipt.receiptHash && parentReceipt.scientificVerdict === spec.parentEvidence.d2.verdict, 'D2 receipt invalid');
  const master = localPath(spec.parentEvidence.masterScene.uri), masterSha = await hashFile(master);
  requireValue(masterSha === spec.parentEvidence.masterScene.sha256 && await hashFile(localPath(spec.parentEvidence.generatorSource.uri)) === spec.parentEvidence.generatorSource.sha256, 'source identity drift');
  requireValue(await hashFile(spec.runtime.blender.executable) === spec.runtime.blender.sha256, 'Blender drift');
  const filesystem = await statfs(repositoryRoot); const available = Number(filesystem.bavail) * Number(filesystem.bsize);
  requireValue(available - spec.processBudget.projectedWriteBytes >= spec.processBudget.minimumFreeReserveBytes, 'disk reserve');
  const root = localPath(ROOT_URI); await mkdir(resolve(root, 'processes'), { recursive: true, mode: 0o700 });
  const admission = await writeHashed(resolve(root, 'admission.json'), { schemaVersion: 'bfs.b62CameraQualityAdmission.v0.1', experimentId: 'B62-Q1-D3', status: 'ADMITTED', preregistrationCommit: PREREGISTRATION_COMMIT, correctionCommit: CORRECTION_COMMIT, toolFreezeCommit: freeze, bindings: { spec: { uri: SPEC_URI, sha256: await hashFile(localPath(SPEC_URI)) }, protocol: { uri: PROTOCOL_URI, sha256: await hashFile(localPath(PROTOCOL_URI)) }, correction: { uri: CORRECTION_URI, sha256: await hashFile(localPath(CORRECTION_URI)) }, correctionProtocol: { uri: CORRECTION_PROTOCOL_URI, sha256: await hashFile(localPath(CORRECTION_PROTOCOL_URI)) }, retainedFailureTree: retainedTree, parentTree, parentReceiptSha256: await hashFile(parentReceiptPath), master: { uri: spec.parentEvidence.masterScene.uri, sha256: masterSha }, generatorSha256: await hashFile(localPath(spec.parentEvidence.generatorSource.uri)), blenderSha256: await hashFile(spec.runtime.blender.executable), tools: toolHashes }, resources: { availableBytesBefore: available, projectedWriteBytes: spec.processBudget.projectedWriteBytes, minimumFreeReserveBytes: spec.processBudget.minimumFreeReserveBytes }, operations: { blenderStartsBeforeAdmission: 0, renderCalls: 0, modelCalls: 0, networkCalls: 0, dockerProcesses: 0 } }, 'admissionHash');
  try {
    ...
    const result = await runBudgetedProcess({ command: process.execPath, args: auditorArgs, cwd: repositoryRoot, env: { PATH: '/usr/bin:/bin', LANG: 'C.UTF-8', LC_ALL: 'C.UTF-8' }, outputRoot: root, budgets: { wallTimeMs: 60000, maxRssBytes: 1073741824, maxLogBytes: 1048576, maxOutputFiles: 64, maxOutputBytes: spec.processBudget.projectedWriteBytes, sampleIntervalMs: 100 } });
    ...
    requireValue(result.outcome === 'PASS' && result.child.exitCode === 0, 'auditor failed');
    ...
    const receipt = await writeHashed(resolve(root, 'receipt.json'), { schemaVersion: 'bfs.b62CameraQualityBoundedSearchReceipt.v0.1', experimentId: 'B62-Q1-D3', status: 'PASS', scientificVerdict: audit.scientificVerdict, selectedCandidateId: audit.selectedCandidateId, admission: { uri: `${ROOT_URI}/admission.json`, sha256: await hashFile(resolve(root, 'admission.json')), admissionHash: admission.admissionHash }, comparison: { uri: `${ROOT_URI}/comparison.json`, sha256: await hashFile(comparisonPath), comparisonHash: comparison.comparisonHash }, audit: { uri: `${ROOT_URI}/audit.json`, sha256: await hashFile(auditPath), auditHash: audit.auditHash }, processes: { primary: { uri: `${ROOT_URI}/processes/PRIMARY.json`, sha256: await hashFile(resolve(root, 'processes/PRIMARY.json')), processHash: primaryProcess.processHash }, independent: { uri: `${ROOT_URI}/processes/INDEPENDENT.json`, sha256: await hashFile(resolve(root, 'processes/INDEPENDENT.json')), processHash: independentProcess.processHash }, auditor: { uri: `${ROOT_URI}/processes/AUDITOR.json`, sha256: await hashFile(resolve(root, 'processes/AUDITOR.json')), processHash: auditorProcess.processHash } }, operations: { runnerProcesses: 1, blenderStarts: 2, renderCalls: 0, nodeAuditorProcesses: 1, modelCalls: 0, networkCalls: 0, dockerProcesses: 0 }, nonClaims: spec.nonClaims }, 'receiptHash');
    console.log(`BFS_B62_Q1_D3 PASS ${receipt.scientificVerdict} ${receipt.selectedCandidateId ?? 'NONE'} ${receipt.receiptHash}`);
  } catch (error) {
    if (!await exists(resolve(root, 'failure.json'))) await writeHashed(resolve(root, 'failure.json'), { schemaVersion: 'bfs.b62CameraQualityFailure.v0.1', experimentId: 'B62-Q1-D3', status: 'INVALIDATED', scientificVerdict: null, toolFreezeCommit: freeze, reason: error.message }, 'failureHash');
    throw error;
  }
}
