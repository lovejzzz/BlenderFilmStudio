#!/usr/bin/env python3
"""C2 adapter: actual parent OID and fresh roots only."""
...
if hashlib.sha256(BASE.read_bytes()).hexdigest() != EXPECTED_BASE_SHA256:
    raise RuntimeError("real-impact C2 runner base identity mismatch")
...
replacements = (
    ...
    ("scripts/run-rc6-real-impact-bullet-speed-screen.py", "scripts/run-rc6-real-impact-bullet-speed-screen-c2.py", 1, "runner commit path"),
    ("scripts/audit-rc6-real-impact-bullet-speed-screen.py", "scripts/audit-rc6-real-impact-bullet-speed-screen-c2.py", 2, "auditor"),
...
for before, after, expected, label in replacements:
    if source.count(before) != expected:
        raise RuntimeError(f"real-impact C2 runner {label} target mismatch")
