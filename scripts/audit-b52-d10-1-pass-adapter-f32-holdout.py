#!/usr/bin/env python3
"""Independent artifact and array replay audit for B52-D10.1."""
...
def main() -> None:
    ...
    if args.output.exists():
        raise RuntimeError("refusing to overwrite D10.1 audit")
    ...
    for record in receipt["adapterRuns"]:
        ...
        expected = {
            "previousRgba": previous["BFS_F32_MASTER.Combined"],
            "currentRgba": current["BFS_F32_MASTER.Combined"],
            "previousDepth": previous["BFS_F32_MASTER.Depth"][..., 0],
            "currentDepth": current["BFS_F32_MASTER.Depth"][..., 0],
            "previousLayer": previous["BFS_F32_MASTER.Object Index"][..., 0],
            "currentLayer": current["BFS_F32_MASTER.Object Index"][..., 0],
            "motion": np.negative(current["BFS_F32_MASTER.Vector"][..., :2], dtype=np.float32),
        }
        arrays = {}
    ...
    body = {
        "schemaVersion": "bfs.blenderMultipartTemporalAdapterF32Audit.v0.1",
    ...
    print(f"BFS_B52_D10_1_AUDIT status={audit['status']} sources={sum(item['passed'] for item in source_artifacts)}/12 adapters={sum(item['passed'] for item in adapter_replay)}/6 diagnostics={sum(item['passed'] for item in diagnostics)}/12 attacks={len(attacks)} output={sha256_file(args.output)}")
    if not passed:
        raise SystemExit(2)
