...
import {
  B40_C1_PREREG_COMMIT,
  B40_C1_SPEC_SHA256,
  analyzeB40C1Evidence,
  parseB40C1Binfmt,
  readB40C1Spec,
  runB40C1Attacks,
} from './lib/b40-c1-binfmt-parser-correction.mjs';

const experimentRoot = resolve(repositoryRoot, 'experiments/worker-host-capacity-admission-v0-2');
const runnerPath = resolve(repositoryRoot, 'scripts/run-b40-c1-binfmt-parser-correction.mjs');
const libraryPath = resolve(repositoryRoot, 'scripts/lib/b40-c1-binfmt-parser-correction.mjs');
const auditPath = resolve(repositoryRoot, 'scripts/audit-b40-c1-binfmt-parser-correction.mjs');
...
if (spawnSync('git', ['merge-base', '--is-ancestor', B40_C1_PREREG_COMMIT, 'HEAD'], { cwd: repositoryRoot }).status !== 0) throw new Error('B40-C1 preregistration is not an ancestor');
const toolFreezeCommit = probe('git', ['rev-parse', 'HEAD'], 'tool freeze identity');
if (probe('git', ['status', '--porcelain', '--untracked-files=no'], 'tracked status') !== '') throw new Error('B40-C1 tracked worktree must be clean');
if (process.version !== baseSpec.runtime.nodeVersion || process.execPath !== baseSpec.runtime.nodeBinary) throw new Error('B40-C1 Node identity differs');
const nodeBinarySha256 = await sha256File(process.execPath);
if (nodeBinarySha256 !== baseSpec.runtime.nodeBinarySha256) throw new Error('B40-C1 Node SHA differs');
const invalidResultSha256 = await sha256File(resolve(repositoryRoot, 'experiments/worker-host-capacity-admission-v0-1/results.json'));
const invalidAuditSha256 = await sha256File(resolve(repositoryRoot, 'experiments/worker-host-capacity-admission-v0-1/audit.json'));
if (invalidResultSha256 !== correctionSpec.invalidAttempt.resultSha256 || invalidAuditSha256 !== correctionSpec.invalidAttempt.auditSha256) throw new Error('B40-C1 invalid parent evidence differs');
...
if (JSON.stringify(ancestry) !== JSON.stringify(baseSpec.ancestry)) throw new Error('B40-C1 base ancestry differs');
...
const evidence = {
  schemaVersion: 'bfs.workerHostCapacityEvidence.v0.2', experimentId: 'B40-C1',
  status: 'CORRECTED_READ_ONLY_CAPACITY_ADMISSION_COMPLETE_NO_RUNTIME_OPERATION',
  preregistration: { commit: B40_C1_PREREG_COMMIT, specSha256: B40_C1_SPEC_SHA256 },
  correction: {
    baseSpecSha256: correctionSpec.baseProtocol.specSha256,
    invalidResultSha256,
    invalidAuditSha256,
    changedImplementationExact: structuredClone(correctionSpec.changedImplementationExact),
  },
  ancestry,
  toolFreezeCommit,
  runtime: { nodeVersion: process.version, nodeBinary: process.execPath, nodeBinarySha256 },
  tools: {
    runner: { uri: 'scripts/run-b40-c1-binfmt-parser-correction.mjs', sha256: await sha256File(runnerPath) },
    library: { uri: 'scripts/lib/b40-c1-binfmt-parser-correction.mjs', sha256: await sha256File(libraryPath) },
    audit: { uri: 'scripts/audit-b40-c1-binfmt-parser-correction.mjs', sha256: await sha256File(auditPath) },
  },
  ...
  probeTrace: structuredClone(baseSpec.probeTraceExact),
...
const analysis = await analyzeB40C1Evidence(evidence, correctionSpec, baseSpec);
const attacks = analysis.passed ? await runB40C1Attacks(evidence, correctionSpec, baseSpec) : [];
const attacksPassed = attacks.length === 14 && attacks.every(attack => attack.passed);
const result = { ...evidence, analysis, attacks, attacksPassed, verdict: analysis.passed && attacksPassed ? correctionSpec.acceptedVerdict : analysis.decision, nonClaims: correctionSpec.nonClaims };
...
process.stdout.write(`BFS_B40_C1_RESULT verdict=${result.verdict} blockers=${result.decision.reasons.join(',')} flags=${observations.vm.emulator.flags} attacks=${attacks.filter(a => a.passed).length}/14 runtimeOps=0\n`);
