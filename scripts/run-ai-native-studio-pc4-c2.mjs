...
const FREEZE_URI = 'specs/ai-native-studio-pc4-tool-freeze-c2.v0.3.json';
const EVIDENCE_URI = 'experiments/ai-native-studio-post-pb7/PC.4-2026-08-31-mac-m2max-attempt-03';
const WORK = '/Users/mengyingli/Documents/ChatGPT/BlenderFilmStudio-PostPB7-workspace/PC.4-2026-08-31-mac-m2max-attempt-03';
...
async function runProcess(executable, argv, logPath, timeoutMs) {
  ...
  clearTimeout(timer); await writeFile(logPath, output, { flag: 'wx', mode: 0o600 }); return { ...result, wallSeconds: (Date.now() - started) / 1000 };
}
...
async function main() {
  ...
  if (!validSelf(freeze, 'specHash') || freeze.status !== 'FROZEN_C2_BEFORE_PC4_ATTEMPT_03' || freeze.preregistration.specHash !== spec.specHash) throw new Error('FREEZE');
  for (const tool of freeze.tools) if (await hashFile(resolve(root, tool.uri)) !== tool.sha256) throw new Error(`TOOL_${tool.uri}`);
  for (const item of Object.values(freeze.retainedAttempts)) if (await hashFile(resolve(root, item.uri)) !== item.sha256) throw new Error(`RETAINED_${item.uri}`);
  ...
  const buildArgv = ['--background', spec.source.path, '--disable-autoexec', '--python-exit-code', '1', '--python', resolve(root, 'scripts/build-ai-native-studio-pc4-c2.py'), '--', '--spec', specPath, '--evidence-root', evidence, '--work-root', WORK];
  ...
  const receipt = await writeSelf(resolve(evidence, 'receipt.json'), {
    schemaVersion: 'bfs.pc4HeroRedesignReceipt.v0.3', status: 'MACHINE_PASS_VISUAL_REVIEW_REQUIRED', gate: 'PC.4', attempt: 'attempt-03',
    preregistration: { uri: SPEC_URI, sha256: await hashFile(specPath), specHash: spec.specHash }, toolFreeze: { uri: FREEZE_URI, sha256: await hashFile(freezePath), specHash: freeze.specHash }, retainedAttempts: freeze.retainedAttempts,
    source: { path: spec.source.path, sha256: await hashFile(spec.source.path) }, binary: { path: spec.binary.path, sha256: await hashFile(spec.binary.path) },
  ...
  process.stdout.write(`BFS_PC4_C2_RUN ${receipt.status} ${receipt.receiptHash}\n`);
}

main().catch(error => { process.stderr.write(`BFS_PC4_C2_RUN_REJECTED ${error.message}\n`); process.exitCode = 1; });
