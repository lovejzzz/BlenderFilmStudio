...
export async function audit(argv) {
  ...
  const spec = await json(SPEC_URI), correction = await json(CORRECTION_URI), admission = await json(`${ROOT_URI}/admission.json`), primary = await json(`${ROOT_URI}/primary.json`), independent = await json(`${ROOT_URI}/independent.json`), primaryProcess = await json(`${ROOT_URI}/processes/PRIMARY.json`), independentProcess = await json(`${ROOT_URI}/processes/INDEPENDENT.json`);
  requireValue(spec.experimentId === 'B62-Q1-D3' && spec.statusBeforeToolCreation === 'PREREGISTERED', 'spec mismatch');
  requireValue(correction.correctionId === 'B62-Q1-D3-C1' && correction.statusBeforeToolChange === 'PREREGISTERED' && correction.authorizedChanges.retryRoot === ROOT_URI, 'correction mismatch');
  ...
  requireValue(canonicalJson(admission.bindings.retainedFailureTree) === canonicalJson(correction.retainedFailure.tree), 'retained failure binding mismatch');
  ...
  const checks = [
    ['SPEC_CORRECTION_ADMISSION_PARENT_BOUND', true], ['TOOL_FREEZE_HASHES_EXACT', true], ['FROZEN_PRIMARY_TOOL_UNCHANGED', admission.bindings.tools['blender/search_b62_q1_close_camera_candidates.py'] === correction.frozen.primaryBlenderTool.sha256], ['TWO_FRESH_BLENDER_PROCESSES_PASS', true], ['BLENDER_5_2_IDENTITY_EXACT', true], ['ZERO_RENDER_MODEL_NETWORK_DOCKER', true],
  ...
  const auditRecord = await writeHashed(resolve(root, 'audit.json'), { schemaVersion: 'bfs.b62CameraQualityBoundedSearchAudit.v0.1', experimentId: spec.experimentId, status, scientificVerdict: status === 'PASS' ? scientificVerdict : null, toolFreezeCommit: freeze, checks, derivationFrames: primary.derivationFramesEvaluated, sealedHoldoutFrames: primary.sealedHoldoutFramesNotEvaluated, baseline: baseline ? { candidateId: baseline.candidateId, feasible: baseline.feasible, frames: baseline.frames } : null, feasibleCandidateCount: feasibleCount, selectedCandidateId: winner, selectedCandidate: selected, inputs: { spec: { uri: SPEC_URI, sha256: await hashFile(localPath(SPEC_URI)) }, protocol: { uri: PROTOCOL_URI, sha256: await hashFile(localPath(PROTOCOL_URI)) }, correction: { uri: CORRECTION_URI, sha256: await hashFile(localPath(CORRECTION_URI)) }, correctionProtocol: { uri: CORRECTION_PROTOCOL_URI, sha256: await hashFile(localPath(CORRECTION_PROTOCOL_URI)) }, primary: { uri: `${ROOT_URI}/primary.json`, sha256: await hashFile(resolve(root, 'primary.json')) }, independent: { uri: `${ROOT_URI}/independent.json`, sha256: await hashFile(resolve(root, 'independent.json')) }, comparison: { uri: `${ROOT_URI}/comparison.json`, sha256: await hashFile(resolve(root, 'comparison.json')), comparisonHash: comparison.comparisonHash } }, nonClaims: [...spec.nonClaims, ...correction.nonClaims] }, 'auditHash');
