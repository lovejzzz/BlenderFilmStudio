#!/usr/bin/env node
/** Frozen scalar Node consumer with D12.11-I1 Material Index ownership. */
import fs from 'node:fs';
import path from 'node:path';
import crypto from 'node:crypto';

const SPEC_SHA256 = '89dd3637ffe5af3544e8cd8aca8869eedd8b1a1867d41e08a354e5cd0c3b2a0e';
const H1_SPEC_SHA256 = 'c2756a20e314cf470698ef7af6160154b8d7e2d5e8531ce6591b2509a8730dbc';
...
function parseArgs() {
  ...
  for (let index = 2; index < process.argv.length; index += 2) {
    ...
    if (!key.startsWith('--') || index + 1 >= process.argv.length) throw new Error('invalid D12.11-I1 arguments');
  ...
  if (![1, 2].includes(result.repeat)) throw new Error('invalid D12.11-I1 repeat');
  return result;
}
function readF32(filePath, expectedCount) {
  const payload = fs.readFileSync(filePath);
  if (payload.length !== expectedCount * 4) throw new Error(`D12.11-I1 float32 length mismatch: ${filePath}`);
...
function exactScaled(value, scale, label) { const scaled = value * Number(scale); if (!Number.isInteger(scaled)) throw new Error(`D12.11-I1 non-canonical ${label}: ${value}`); return BigInt(scaled); }
function ceilDiv(numerator, denominator) { return (numerator + denominator - 1n) / denominator; }
function main() {
  const args = parseArgs(), intervention = JSON.parse(fs.readFileSync(args.spec, 'utf8'));
  const h1Path = intervention.parents.h1Spec.uri;
  if (shaFile(args.spec) !== SPEC_SHA256 || shaFile(h1Path) !== H1_SPEC_SHA256) throw new Error('D12.11-I1 spec identity mismatch');
  const spec = JSON.parse(fs.readFileSync(h1Path, 'utf8'));
  if (shaFile(process.execPath) !== spec.runtime.node.sha256 || process.version !== spec.runtime.node.version) throw new Error('D12.11-I1 Node identity mismatch');
  const sourceFixture = spec.fixtures.find(row => row.id === args.fixture), fixture = sourceFixture ? JSON.parse(JSON.stringify(sourceFixture)) : null;
  if (fixture) for (const owner of fixture.owners) owner.passIndex = intervention.materialOwnerTokens.assignments[owner.analyticOwnerId];
  if (!fixture || fs.existsSync(args['output-dir']) || fs.existsSync(args.report)) throw new Error('D12.11-I1 consumer fixture or output invalid');
  ...
  if (adapter.reportHash !== canonicalHash(adapterBody) || adapter.fixtureId !== args.fixture || adapter.repeat !== args.repeat) throw new Error('D12.11-I1 adapter identity mismatch');
  const [width, height] = fixture.resolution, pixels = width * height, arrays = {};
  for (const [name, [filename, channels]] of Object.entries(INPUTS)) { const [value, payload] = readF32(path.join(args['input-dir'], filename), pixels * channels); if (shaBytes(payload) !== adapter.arrays[name].sha256) throw new Error(`D12.11-I1 input hash mismatch: ${name}`); arrays[name] = value; }
