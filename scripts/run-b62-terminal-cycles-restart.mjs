...
const JOB_URI = 'experiments/b62-terminal-cycles-restart-job-v0-1';
const FORMAL_URI = 'experiments/b62-terminal-cycles-restart-v0-1';
const JOB_ID = 'B62-TERMINAL-CYCLES-RESTART-J01';
const OCIO_URI = 'color/ocio/cg-config-v4.0.0_aces-v2.0_ocio-v2.5.ocio';
const TOOL_PATHS = [SPEC_URI, PROTOCOL_URI, RENDER_URI, EXR_AUDITOR_URI, ORCHESTRATOR_URI, FINAL_AUDITOR_URI, LEDGER_URI, BUDGET_URI];
...
async function createStart(parsed, spec) {
  ...
  const preflight = await writeExclusiveDurableHashed(resolve(repositoryRoot, PREFLIGHT_URI, 'preflight.json'), { schemaVersion: 'bfs.b62TerminalCyclesRestartPreflight.v0.1', status: 'ACCEPTED', toolFreezeCommit: freeze.commit, originMain: freeze.origin, toolHashes: freeze.hashes, capacity, source: spec.parents.sceneCompilation.scene, stageDag: spec.stageDag }, 'preflightHash');
  const jobRoot = resolve(repositoryRoot, JOB_URI);
  await createManifest(jobRoot, { jobId: JOB_ID, experimentId: spec.experimentId, toolFreezeCommit: freeze.commit, preflight: { uri: repoUri(preflight.path), sha256: preflight.file.sha256, preflightHash: preflight.record.preflightHash }, source: spec.parents.sceneCompilation.scene, spec: { uri: SPEC_URI, sha256: await sha256File(resolve(repositoryRoot, SPEC_URI)) }, stageDag: spec.stageDag });
